...
CBloodCastle::CBloodCastle()
{
	...
	for ( int i =0; i<MAX_BLOOD_CASTLE_LEVEL ; i++ )
	{
		...
		this->m_BridgeData[i].m_iMapNumber = this->GetMap(i);
...
void CBloodCastle::Init(bool bEVENT_ENABLE)
{
	...
	for ( int n =0; n<gMSetBase.m_Count;n++)
	{
		if ( BC_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber ) != FALSE )
		{
			...
			BYTE btBloodCastleIndex = this->GetIndex(btMapNumber);

			if ( btMonsterType == 232 )
			{
				this->m_BridgeData[this->GetIndex(gMSetBase.m_Mp[n].m_MapNumber)].m_iAngelKingPosNum = n;
				continue;
			}
...
void CBloodCastle::ProcState_Playing(int iBridgeIndex)
{
	...
	if ( iTICK_MSEC >= 1000 )
	{
		...
		{
			LogAddTD("[Blood Castle] CheckWinnerExist == true");
			if ( this->CheckWinnerValid(iBridgeIndex) == true )
			{
				LogAddTD("[Blood Castle] CheckWinnerValid == true");
				if ( this->CheckWinnerPartyComplete(iBridgeIndex) == true )
				{
					LogAddTD("[Blood Castle] CheckWinnerPartyComplete == true");
...
void CBloodCastle::SetState_None(int iBridgeIndex)
{
	...
	{
		if ( gObj[n].MapNumber == this->GetMap(iBridgeIndex) && gObj[n].Connected == PLAYER_PLAYING )
		{
			gObjMoveGate(n, 22);
		}
	}
...
void CBloodCastle::SetState_Closed(int iBridgeIndex)
{
	...
	for (int n=OBJ_STARTUSERINDEX;n<OBJMAX;n++)
	{
		if ( gObj[n].MapNumber == this->GetMap(iBridgeIndex) && gObj[n].Connected > PLAYER_LOGGED )
		{
...
bool CBloodCastle::BloodCastleChaosMix(int iIndex, int iLEVEL)
{
	...
	if ( BC_CLOACK_LEVEL_RANGE(iLEVEL-1)  == FALSE )
	{
		return false;
	}
...
int  CBloodCastle::CheckQuestItem(int iIndex)
{
	...
	if ( this->m_BridgeData[this->GetIndex(gObj[iIndex].MapNumber)].m_nBC_QUESTITEM_SERIAL == -1 )
	{
		return -1;
	}
	...
	for ( int x=0;x<MAIN_INVENTORY_SIZE;x++)
	{
		if ( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			...
			{
				if ( gObj[iIndex].pInventory[x].m_Number	== this->m_BridgeData[this->GetIndex(gObj[iIndex].MapNumber)].m_nBC_QUESTITEM_SERIAL )
				{
					iITEM_LEVEL = gObj[iIndex].pInventory[x].m_Level;
...
bool CBloodCastle::CheckWalk(int iIndex, int iMoveX, int iMoveY)
{
	...
	if ( this->GetCurrentState( this->GetIndex(gObj[iIndex].MapNumber) ) == TRUE )	// Apply Deathway fix here : Add -MAP_INDEX_BLOODCASTLE1
	{
...
void CBloodCastle::ClearMonster(int iBridgeIndex, bool bClearCastleDoor)
{
	...
	{
		if ( gObj[n].MapNumber == (this->GetMap(iBridgeIndex)))
		{
...
void CBloodCastle::SetMonster(int iBridgeIndex)
{
	...
	for ( int n=0;n<gMSetBase.m_Count;n++)
	{
		if ( BC_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber) != FALSE  )
		{
			...
			BYTE btBridgeIndex = this->GetIndex(btMap);
	...
	if ( nSaintStatuePosNum != -1 )
	{
		...
		else
		{
			LogAddTD("[Blood Castle][Bug Tracer] (%d) It's not Saint Statue( Map:%d, BloodCastleIndex:%d, Type:%d )",
				iBridgeIndex+1, btMapNumber, this->GetIndex(gMSetBase.m_Mp[nSaintStatuePosNum].m_MapNumber), btMonsterType);
		}
	}
...
int  CBloodCastle::LevelUp(int iIndex, int iAddExp, int iEventType)
{
	...
	LogAddTD("Experience : Map[%d]-(%d,%d) [%s][%s](%d) %u %d MonsterIndex : %d, EventType : %d",
		gObj[iIndex].MapNumber, gObj[iIndex].X, gObj[iIndex].Y,	// #error gObj[iIndex].X -> gObj[iIndex].Y
...
void CBloodCastle::CheckUsersOnConnect(int iBridgeIndex)
{
	...
	for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
	{
		if ( this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex != -1 )
		{
			...
			{
				if ( gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].MapNumber != (this->GetMap(iBridgeIndex)) )
				{
					this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex = -1;
				}
			}
...
void CBloodCastle::BlockCastleEntrance(int iBridgeIndex)
{
	...
	for ( int i= ::g_btCastleEntranceMapXY[iBridgeIndex].btStartX;i<= ::g_btCastleEntranceMapXY[iBridgeIndex].btEndX;i++)
	{
		...
		{
			MapC[this->GetMap(iBridgeIndex)].m_attrbuf[j * 256 + i] |= 4;
		}
	}
}
...
void CBloodCastle::BlockCastleBridge(int iBridgeIndex)
{
	...
	for ( int i= ::g_btCastleBridgeMapXY[iBridgeIndex].btStartX;i<= ::g_btCastleBridgeMapXY[iBridgeIndex].btEndX;i++)
	{
		...
		{
			MapC[this->GetMap(iBridgeIndex)].m_attrbuf[j * 256 + i] |= 8;
		}
	}
}
...
void CBloodCastle::BlockCastleDoor(int iBridgeIndex)
{
	...
	for ( int x=0;x<3;x++)
	{
		for ( int y=::g_btCastleDoorMapXY[iBridgeIndex][x].btStartX; y <= ::g_btCastleDoorMapXY[iBridgeIndex][x].btEndX ;y++)
		{
			...
			{
				MapC[this->GetMap(iBridgeIndex)].m_attrbuf[z * 256 + y] |= 4;
			}
		}
	}
}
...
void CBloodCastle::SendCastleEntranceBlockInfo(int iBridgeIndex, bool bLive)
{
	...
	{
		if ( gObj[i].MapNumber == (this->GetMap(iBridgeIndex)) )
		{
		...
		if ( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			if ( gObj[iIndex].pInventory[x].m_Type == ITEMGET(13,19) )
			{
				...
				if ( BC_WEAPON_LEVEL_RANGE(iLEVEL) != FALSE )
				{
					...
					this->DropItemDirectly(this->GetIndex(gObj[iIndex].MapNumber),
						gObj[iIndex].m_Index, ITEMGET(13, 19), x);

					if ( BC_MAP_RANGE(gObj[iIndex].MapNumber) != FALSE )
					{
						int iBC_INDEX = this->GetIndex(gObj[iIndex].MapNumber);
...
void CBloodCastle::DropChaosGem(int iIndex)
{
	...
	int iType = ::ItemGetNumberMake(12, 15);	// Chaos Gem
...
bool CBloodCastle::CheckAngelKingExist(int iBridgeIndex)
{
	...
	if ( bRET_VAL == false )
	{
		int result = gObjAddMonster(this->GetMap(iBridgeIndex));

		if ( result >= 0 )
		{
			gObjSetPosMonster(result, this->m_BridgeData[iBridgeIndex].m_iAngelKingPosNum);
			gObjSetMonster(result, 232);
			gObj[result].m_cBloodCastleIndex = this->GetIndex(gObj[result].MapNumber);
			bRET_VAL = true;
		}
		else
		{
...
{
	if ( BC_MAP_RANGE(this->GetIndex(iBridgeIndex)) == FALSE )
	{
	...
	if ( MapC[map_num].ItemDrop(type, level, dur, gObj[iIndex].X, gObj[iIndex].Y,Option1, Option2, Option3, NOption, SOption, item_number, aAntiLootIndex, PetLevel, PetExp, ItemEffectEx,0) == TRUE )
	{
