...
#define BC_MAP_RANGE(value) (value < MAP_INDEX_BLOODCASTLE1 ? FALSE : ((value > MAP_INDEX_BLOODCASTLE7 && value != MAP_INDEX_BLOODCASTLE8) ? FALSE : TRUE))
...
public:
	...
	int GetMap ( int iLevel )
	{
		if(iLevel == 7)
		{
			return MAP_INDEX_BLOODCASTLE8;
		}
		else
		{
			return MAP_INDEX_BLOODCASTLE1 + iLevel;
		}
	}

	int GetIndex ( int Map )
	{
		if(Map == MAP_INDEX_BLOODCASTLE8) return 7;
		return Map - MAP_INDEX_BLOODCASTLE1;
	}
