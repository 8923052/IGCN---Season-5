...
void CCastleSiege::ResetAllUserCsJoinSide()
{
	...
	for(int iUSER = OBJ_STARTUSERINDEX; iUSER < OBJMAX; iUSER++)
	{
		if(::gObjIsConnected(iUSER) == FALSE)
			continue;
//		BuffEffectC.DisableSiegeBuff(iUSER);
...
void CCastleSiege::NotifySelfCsJoinSide(int iIndex)
{
	...
	switch( btCsJoinSide )
	{
		case 1:
			iState =
			iViewState = 0x40000;
			if(this->m_iCastleSiegeState == CASTLESIEGE_STATE_STARTSIEGE)BuffEffectC.EnableBuff(iIndex, 14, -1, 0, 0, 0);
			break;
		case 2:
		{
			iState = 1;
			iViewState = 0x80000;
			if( gObj[iIndex].m_btCsJoinSide == 2 )
			{
				if(this->m_iCastleSiegeState == CASTLESIEGE_STATE_STARTSIEGE)BuffEffectC.EnableBuff(iIndex, 15, -1, 0, 0, 0);
			}
			else if( gObj[iIndex].m_btCsJoinSide == 3 )
			{
				if(this->m_iCastleSiegeState == CASTLESIEGE_STATE_STARTSIEGE)BuffEffectC.EnableBuff(iIndex, 16, -1, 0, 0, 0);
			}
			else if( gObj[iIndex].m_btCsJoinSide == 4 )
			{
				if(this->m_iCastleSiegeState == CASTLESIEGE_STATE_STARTSIEGE)BuffEffectC.EnableBuff(iIndex, 17, -1, 0, 0, 0);
			}
		}
			break;
		default:
		{
			iState = 0;
			iViewState = 0xC0000;
			BuffEffectC.DisableSiegeBuff(iIndex);
		}
		break;
	}
	...
	while(it != m_vtNpcData.end())
	{
		...
		if(pNpcData.m_bIN_USE == 1 && pNpcData.m_iNPC_LIVE == 1)
		{
			...
			if(iNPC_INDEX >= 0)
			{
				...
				if(gObj[iNPC_INDEX].Class == 283)
				{
					statuy++;
					LogAddC(4, "ZBUDOWANO %d STATUL %d %d", statuy, gObj[iNPC_INDEX].X, gObj[iNPC_INDEX].Y);
				}
				...
				it->m_iCsNpcExistVal = rand()%65535;
				gObj[iNPC_INDEX].m_iCsNpcExistVal = it->m_iCsNpcExistVal;
...
void CCastleSiege::SetState_STARTSIEGE(int bSetRemainMsec)
{
	...
	else
	{
		...
		{
			SetCastleTowerAccessable(FALSE);
		}
		//CheckReviveGuardianStatue(); // usuniete w 1.00.66 i wyzej
		AdjustDbNpcLevel();
	...
	while( it != this->m_vtNpcData.end() )
	{
		...
		if( (NpcData.m_iNPC_NUM == iNpcType) && (NpcData.m_iNPC_INDEX == iNpcIndex) )
		{
			...
			if( iObjIndex >= 0 )
			{
				...
				it->m_iCsNpcExistVal = ( rand() % 65535);
				gObj[iObjIndex].m_iCsNpcExistVal = it->m_iCsNpcExistVal;
...
BOOL CCastleSiege::SetCastleNpcData(CSP_CSINITDATA* lpMsg, int iCOUNT)
{
	...
	for( int iNPC_COUNT = 0; iNPC_COUNT < iCOUNT; iNPC_COUNT++ )
	{
		...
		if( bNpcLoaded == FALSE )
		{
			...
			m_NpcData.m_iNPC_BASE_MAXHP = lpMsg[iNPC_COUNT].iNpcMaxHp;
			m_NpcData.m_iNPC_MAXHP = m_NpcData.m_iNPC_BASE_MAXHP;
			m_NpcData.m_iNPC_BASE_HP = lpMsg[iNPC_COUNT].iNpcHp;
			m_NpcData.m_iNPC_HP = m_NpcData.m_iNPC_BASE_HP;
			m_NpcData.m_iNPC_BASE_SX = lpMsg[iNPC_COUNT].btNpcX;
			m_NpcData.m_iNPC_SX	= m_NpcData.m_iNPC_BASE_SX;
			m_NpcData.m_iNPC_BASE_SY = lpMsg[iNPC_COUNT].btNpcY;
			m_NpcData.m_iNPC_SY	= m_NpcData.m_iNPC_BASE_SY;
			m_NpcData.m_iNPC_BASE_DX = -1;
			m_NpcData.m_iNPC_DX = -1;
			m_NpcData.m_iNPC_BASE_DY = -1;
			m_NpcData.m_iNPC_DY = -1;
...
void CCastleSiege::SetCalcRegGuildList(LPBYTE lpMsg, int iCOUNT)
{
	if( lpMsg == NULL )
	{
		return;
	...
	CSP_CALCREGGUILDLIST *lpCalcRegGuildList = (CSP_CALCREGGUILDLIST*)lpMsg;

	int iGCNT;
	int dwHighScore;
	for(iGCNT = 0; iGCNT < iCOUNT; iGCNT++ )
	{
		lpCalcRegGuildList = (CSP_CALCREGGUILDLIST*)lpMsg+(iGCNT*sizeof(CSP_CALCREGGUILDLIST));
		_CS_REG_GUILD_DATA GuildData;

		char szGuildName[16] = {0};

		memcpy(szGuildName, lpCalcRegGuildList->szGuildName, MAX_GUILD_LEN);
		
		GuildData.m_bIN_USE = TRUE;
		GuildData.m_strGuildName = szGuildName;
		GuildData.m_iRegMarkCount = lpCalcRegGuildList->iRegMarkCount;
		GuildData.m_iGuildMasterLevel = lpCalcRegGuildList->iGuildMasterLevel;
		GuildData.m_iGuildMemberCount = lpCalcRegGuildList->iGuildMemberCount;
		GuildData.m_iSeqNum = 0x100000 - lpCalcRegGuildList->iSeqNum; 

		dwHighScore = ( ( lpCalcRegGuildList->iRegMarkCount * 5 ) + lpCalcRegGuildList->iGuildMemberCount + (lpCalcRegGuildList->iGuildMasterLevel / 4));

		#define MACRO3(value) (((__int64) value & 0xFFFFFFFF ))

		GuildData.m_i64TotolScore = ((__int64)(DWORD)MACRO3(lpCalcRegGuildList->iSeqNum)) | (((__int64)(DWORD)MACRO3(dwHighScore))<<32);
	...
	for( iGCNT = 0; iGCNT < iGUILD_COUNT; iGCNT++ )
	{
		...
		GuildData.m_iTotalScore = ( ( lpCalcRegGuildList->iRegMarkCount * 5 ) + lpCalcRegGuildList->iGuildMemberCount + (lpCalcRegGuildList->iGuildMasterLevel / 4));
...
void CCastleSiege::MakeCsTotalGuildInfo(LPBYTE lpMsg, int iCOUNT)
{
	if( lpMsg == NULL )
		return;
	...
	CSP_CSGUILDUNIONINFO *lpCSGuildInfo = (CSP_CSGUILDUNIONINFO*)lpMsg;

	EnterCriticalSection(&m_critCsTotalGuildInfo);

	for( int iGCNT = 0; iGCNT < iCOUNT; iGCNT++ )
	{
		lpCSGuildInfo = (CSP_CSGUILDUNIONINFO*)lpMsg+(iGCNT*sizeof(CSP_CSGUILDUNIONINFO));

		char szGuildName[16] = {0};

		memcpy(szGuildName, lpCSGuildInfo->szGuildName, MAX_GUILD_LEN);
		...
		GuildData.m_bIN_USE = TRUE;
		GuildData.m_iCsGuildID = lpCSGuildInfo->iCsGuildID;
...
void CCastleSiege::Run()
{
	...
	if( (GetTickCount() - g_iLastSiegeUpdate) > 3000 )
	{
		...
		DWORD m_iLogTick = GetTickCount();
		if( iLeftSiegeDate >= 0 && (m_iLogTick > this->m_iRunLogTime + 300000) )
		{
			...
			this->m_iRunLogTime = GetTickCount();
		}
		else if(m_iLogTick > this->m_iRunLogTime + 300000)
		{
...
void CCastleSiege::ProcState_IDLE_1()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if(GetTickCount() > this->m_iLogTime + 60000)
		{
			LogAddC(3, "RUN ProcState_IDLE_1()\tLEFT-MIN:%d", this->m_iCS_REMAIN_MSEC / 60000);
			this->m_iLogTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_REGSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if(GetTickCount() > this->m_iLogTime + 60000)
		{
			LogAddC(3, "RUN ProcState_REGSIEGE()\tLEFT-MIN:%d", this->m_iCS_REMAIN_MSEC / 60000);
			this->m_iLogTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_IDLE_2()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if(GetTickCount() > this->m_iLogTime + 60000)
		{
			LogAddC(3, "RUN ProcState_IDLE_2()\tLEFT-MIN:%d", this->m_iCS_REMAIN_MSEC / 60000);
			this->m_iLogTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_REGMARK()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if(GetTickCount() > this->m_iLogTime + 60000)
		{
			LogAddC(3, "RUN ProcState_REGMARK()\tLEFT-MIN:%d", this->m_iCS_REMAIN_MSEC / 60000);
			this->m_iLogTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_IDLE_3()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if(GetTickCount() > this->m_iLogTime + 60000)
		{
			LogAddC(3, "RUN ProcState_IDLE_3()\tLEFT-MIN:%d", this->m_iCS_REMAIN_MSEC / 60000);
			this->m_iLogTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_NOTIFY()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if(GetTickCount() > this->m_iLogTime + 60000)
		{
			LogAddC(3, "RUN ProcState_NOTIFY()\tLEFT-MIN:%d", this->m_iCS_REMAIN_MSEC / 60000);
			this->m_iLogTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_READYSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		{
			if( ( GetTickCount() - this->m_dwCS_STARTTIME_TICK_COUNT ) > 300000 )
			{
				char szBuff[256]={0};
		...
		if(GetTickCount() > this->m_iLogTime + 60000)
		{
			LogAddC(3, "RUN ProcState_READYSIEGE()\tLEFT-MIN:%d", this->m_iCS_REMAIN_MSEC / 60000);
			this->m_iLogTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_STARTSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if(GetTickCount() > this->m_iLogTime + 60000)
		{
			LogAddC(3, "RUN ProcState_STARTSIEGE()\tLEFT-MIN:%d", this->m_iCS_REMAIN_MSEC / 60000);
			this->m_iLogTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_ENDSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if(GetTickCount() > this->m_iLogTime + 60000)
		{
			LogAddC(3, "RUN ProcState_ENDSIEGE()\tLEFT-MIN:%d", this->m_iCS_REMAIN_MSEC / 60000);
			this->m_iLogTime = GetTickCount();
		}
	}
...
void CCastleSiege::ProcState_ENDCYCLE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if(GetTickCount() > this->m_iLogTime + 60000)
		{
			LogAddC(3, "RUN ProcState_ENDCYCLE()\tLEFT-MIN:%d", this->m_iCS_REMAIN_MSEC / 60000);
			this->m_iLogTime = GetTickCount();
		}
	}
...
BOOL CCastleSiege::CheckUnionGuildMaster(int iIndex)
{
	...
	{
		if( (lpGuildInfo->Number == lpGuildInfo->iGuildUnion) || ( lpGuildInfo->Number == 0 ) )
		{
			return TRUE;
		}
	}
...
int CCastleSiege::GetCsAttkGuildList(PMSG_CSATTKGUILDLIST* lpMsgBody, int & iCount)
{
	...
	if( (this->m_iCastleSiegeState != CASTLESIEGE_STATE_READYSIEGE) && (this->m_iCastleSiegeState != CASTLESIEGE_STATE_NOTIFY) )
	{
		return 2;
	}
	...
	for( map<basic_string<char> , _CS_TOTAL_GUILD_DATA>::iterator it = (map<basic_string<char> , _CS_TOTAL_GUILD_DATA>::iterator)m_mapCsTotalGuildInfo.begin(); it != this->m_mapCsTotalGuildInfo.end(); it++)
	{
		...
		lpMsgBody[iCount].dwScore = it->second.m_iTotalScore;
...
void CCastleSiege::SaveCsTotalGuildInfo()
{
	...
	char cBUFFER[2684];
	BYTE GuildInfoBuf[2600];
	int lOfs = 0;
	int BufOfs = 0;
	int count = 0;

	CSP_REQ_CSSAVETOTALGUILDINFO pMsg;
	...
	EnterCriticalSection(&m_critCsTotalGuildInfo);
	std::map<basic_string<char>, _CS_TOTAL_GUILD_DATA>::iterator it;
	for( it = (map<basic_string<char>, _CS_TOTAL_GUILD_DATA>::iterator)m_mapCsTotalGuildInfo.begin(); it != m_mapCsTotalGuildInfo.end(); it++)
	{
		...
		memcpy(GuildInfo.szGuildName, &szGuildName[0], MAX_GUILD_LEN);
		
		GuildInfo.iCsGuildID = it->second.m_iCsGuildID;
		GuildInfo.iGuildInvolved = it->second.m_bGuildInvolved;
		memcpy(&GuildInfoBuf[BufOfs], &GuildInfo, sizeof(GuildInfo));
		BufOfs += sizeof(GuildInfo);
		count++;
	}
	LeaveCriticalSection(&m_critCsTotalGuildInfo);
	pMsg.iCount = count;
	PHeadSetW((LPBYTE)&pMsg, 0x87, ( sizeof(pMsg) + (sizeof(GuildInfo) * count)));
	memcpy(&cBUFFER[lOfs], &pMsg, sizeof(pMsg));
	lOfs += sizeof(pMsg);
	memcpy(&cBUFFER[lOfs], &GuildInfoBuf, BufOfs);
	lOfs += BufOfs;
	cDBSMng.Send(cBUFFER, lOfs );
...
void CCastleSiege::OperateGmCommand(int iIndex, int iGmCommand, void* lpParam)
{
	...
	switch( iGmCommand )
	{
		case 0:
			{
				...
				else
				{
					if( strcmp(szNewOwnerGuild, "") != 0 )
					{
						...
						MsgOutput(iIndex, "?ö???? ±ãµ??Ý°ã: %s", this->m_szCastleOwnerGuild);
					}
					else
					{
		...
		case 3:
			{
				...
				this->SetState(CASTLESIEGE_STATE_IDLE_3, 1);
				MsgOutput(iIndex, "»ó???Ý°ã ??°? - °????±Ã÷");
			}
			break;
		case 4:
			{
				this->SetState(CASTLESIEGE_STATE_READYSIEGE, 1);
				MsgOutput(iIndex, "»ó???Ý°ã ??°? - °???????");		
			}
			break;
		...
		case 6:
			{
				if( strcmp(this->m_szCastleOwnerGuild, "") == 0 )
				{
					MsgOutput(iIndex, "?ö?? ?ö???? ????");
				}
				else
				{
					MsgOutput(iIndex, "?ö?? ?ö???? ±ãµ?  - %s", this->m_szCastleOwnerGuild);
				}
			}
			break;
		case 7:	/* Get Siege State Time Left */
			{
				...
				else
				{
					...
					case CASTLESIEGE_STATE_NONE:
						...
						break;
					case CASTLESIEGE_STATE_IDLE_1:
						MsgOutput(iIndex, "?ö?? °???»ó?? - °??? ?Ö±? ?÷?ü (?????ÆµÆ)");
						break;
					case CASTLESIEGE_STATE_REGSIEGE:
						MsgOutput(iIndex, "?ö?? °???»ó?? - °????ü µ?·?±?°Ù : %d??°Ù:%d??:%d?Æ ùž??",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60, 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_IDLE_2:
						MsgOutput(iIndex, "?ö?? °???»ó?? - °????ü µ?·??÷ÈÄ (È??Ä±?°Ù) : %d??°Ù:%d??:%d?Æ ùž??",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_REGMARK:
						MsgOutput(iIndex, "?ö?? °???»ó?? - à®?? µ?·?±?°Ù : %d??°Ù:%d??:%d?Æ ùž??",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_IDLE_3:
						MsgOutput(iIndex, "?ö?? °???»ó?? - à®?? µ?·??÷ÈÄ  (È??Ä±?°Ù) : %d??°Ù:%d??:%d?Æ ùž??",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_NOTIFY:
						MsgOutput(iIndex, "?ö?? °???»ó?? - °????ü ?üý©±ãµ? àß?À±?°Ù : %d??°Ù:%d??:%d?Æ ùž??",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_READYSIEGE:
						...
						break;
					case CASTLESIEGE_STATE_STARTSIEGE:
						MsgOutput(iIndex, "?ö?? °???»ó?? - °????ü ???ñ±?°Ù : %d??°Ù:%d??:%d?Æ ùž??",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
					case CASTLESIEGE_STATE_ENDSIEGE:
						MsgOutput(iIndex, "?ö?? °???»ó?? - °????ü ?ü?? ?ß : %d??°Ù:%d??:%d?Æ ùž??",
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600), 
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60,	
							((this->m_iCS_REMAIN_MSEC / 1000) % 3600) / 60);
						break;
		...
		case 8:
			{
				...
				if( this->m_iCastleSiegeState != CASTLESIEGE_STATE_STARTSIEGE )
				{
					MsgOutput(iIndex, "°??ö±ù?? ??Ã?...?ö?? °???»ó?? - °????ü ?Ö±???·?");
				}
		...
		case 9:
			{
				this->SetState(CASTLESIEGE_STATE_STARTSIEGE, 1);
				MsgOutput(iIndex, "Castle siege started!");
			}
			break;
		default:
			MsgOutput(iIndex, "°??ö±ù?? ??Ã?...?ö?? °???»ó?? - °????ü ?Ö±???·?");
...
void CCastleSiege::NotifyCrownState(BYTE btState)
{
	...
	if( !btState )
	{
		BuffEffectC.EnableBuff(m_iCrownIndex, 20, -1, 0, 0, 0);
	}
	else
	{
		BuffEffectC.DelEffect(&gObj[m_iCrownIndex], 20);
	}
...
BOOL CCastleSiege::PayForUpgradeDbNpc(int iIndex, int iNpcType, int iNpcIndex, int iNpcUpType, int iNpcUpValue, int iNpcUpIndex)
{
	...
	_CS_NPC_DATA pNpcData;
	int bIsLive = GetNpcData(iNpcType, iNpcIndex, pNpcData);
		...
		case 277:
		{
			switch ( iNpcUpType )
			{
				case 1:
				{
					...
					if(!DelGemOfDefend(iIndex, iNEED_GEMOFDEFEND))
					{
