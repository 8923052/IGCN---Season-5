...
public:
	...
	BOOL SetCastleNpcData(CSP_CSINITDATA* lpMsg, int iCOUNT);
	int CheckSync();
	...
	BOOL PayForUpgradeDbNpc(int iIndex, int iNpcType, int iNpcIndex, int iNpcUpType, int iNpcUpValue, int iNpcUpIndex);
	void StoreDbNpc();
	...
	void SetCalcRegGuildList(LPBYTE lpMsg, int iCOUNT);
	void MakeCsTotalGuildInfo(LPBYTE lpMsg, int iCOUNT);
...
/*this+0x1F8*/ int m_iLogTime;
/*this+0x1FC*/ int m_iRunLogTime;
}; 
