	...
	for (int n=0;n<CHAOS_BOX_SIZE;n++)
	{
		...
		else
		{
			...
			lpObj->pChaosBox[n].Convert(lpObj->pChaosBox[n].m_Type, lpObj->pChaosBox[n].m_Option1,
				...
				lpObj->pChaosBox[n].m_SetOption, lpObj->pChaosBox[n].m_ItemOptionEx, CURRENT_DB_VERSION, &lpObj->pChaosBox[n].m_ItemSocket[0]);
		}
	}
...
{
	LogAddTD("[DevilSquare] Item request [%s][%s]", AccountId, Name);	// #warning Translation
...
extern int gChaosMix13Rate;*/
	...
	if ( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		...
		Item.Convert(lpObj->pChaosBox[PlusItemPos].m_Type, lpObj->pChaosBox[PlusItemPos].m_Option1, lpObj->pChaosBox[PlusItemPos].m_Option2,
			...
			lpObj->pChaosBox[PlusItemPos].m_SetOption, lpObj->pChaosBox[PlusItemPos].m_ItemOptionEx, CURRENT_DB_VERSION, &lpObj->pChaosBox[PlusItemPos].m_ItemSocket[0]);
...
BOOL WingChaosMix(LPOBJ lpObj)
{
	...
	for ( int n=0;n<CHAOS_BOX_SIZE;n++)
	{
		if ( lpObj->pChaosBox[n].IsItem() == TRUE )
		{
			...
			if ( lpObj->pChaosBox[n].m_Type >= ITEMGET(12,0) && lpObj->pChaosBox[n].m_Type <= ITEMGET(12,2) || lpObj->pChaosBox[n].m_Type  >= ITEMGET(12,3) && lpObj->pChaosBox[n].m_Type <= ITEMGET(12,6) || lpObj->pChaosBox[n].m_Type == ITEMGET(14,30) || lpObj->pChaosBox[n].m_Type == ITEMGET(12,41))
			{
				WingCount++;
				WingIndex = n;
	...
	if ( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		...
		{
			iItemType = 12;
			iItemSubType = WingNormal[rand()%1 + 1];//rand()%4 + 3;
		}
...
BOOL Wing3ChaosMix(LPOBJ lpObj)
{
	...
	if ( ChoasGemCount == 1 )
	{
		if ( ChoasGemCount != 1 || CondorCrystalCount != 1 || CondorFeatherCount != 1 || BundleOfBlessCount != 1 || BundleOfSoulCount != 1 || JewelOfCreationCount != 1 )
		{
			pMsg.Result = CB_INCORRECT_MIX_ITEMS;
			DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
			lpObj->ChaosLock = FALSE;
	...
	lpObj->ChaosSuccessRate += 95;

	if ( lpObj->ChaosSuccessRate == 0 && ChoasGemCount != 1 && BundleOfSoulCount != 1 && JewelOfCreationCount != 1 )
	{
		pMsg.Result = CB_INCORRECT_MIX_ITEMS;
		DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
	...
	if ( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		...
		::ItemSerialCreateSend(lpObj->m_Index, -1, 0, 0, iWingNum, iWingLevel, 0, 0, iOption1, iOption2, 220, iOption3, 0);
		::gObjInventoryCommit(lpObj->m_Index);
...
BOOL CondorFeatherChaosMix(LPOBJ lpObj) //Created by HermeX Only Lack SetItem Checkers
{
	...
	if ( WingCount == 1 )
	{
		if ( WingCount != 1 || ChoasGemCount != 1 || BundleOfSoulCount != 1 || JewelOfCreationCount != 1 )
		{
			lpObj->ChaosLock = FALSE;
			pMsg.Result = CB_INCORRECT_MIX_ITEMS;
			DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
	...
	if ( lpObj->ChaosSuccessRate == 0 && WingCount != 1 && ChoasGemCount != 1 && BundleOfSoulCount != 1 && JewelOfCreationCount != 1 )
	{
		pMsg.Result = CB_INCORRECT_MIX_ITEMS;
		DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
...
void SetItemChaosMix(LPOBJ lpObj)
{
	...
	if ( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		...
		Item.Convert(lpObj->pChaosBox[MixSetItemIndex].m_Type, lpObj->pChaosBox[MixSetItemIndex].m_Option1, lpObj->pChaosBox[MixSetItemIndex].m_Option2,
			...
			lpObj->pChaosBox[MixSetItemIndex].m_SetOption, lpObj->pChaosBox[MixSetItemIndex].m_ItemOptionEx, CURRENT_DB_VERSION,&lpObj->pChaosBox[MixSetItemIndex].m_ItemSocket[0]);
...
{
	MsgOutput(lpObj->m_Index,"Lotery mix is disabled");
	return;
	PMSG_CHAOSMIXRESULT pMsg;
...
int  CheckIllusionTempleItem(OBJECTSTRUCT* lpObj, int & eventitemcount, int & itemlevel)
{
	BOOL bChaoseGem = FALSE;
	BOOL bScrollOfIllusion = FALSE;
	BOOL bPotionOfIllusion = FALSE;
	eventitemcount = 0;
	BOOL FoundOtherItem = FALSE;
	int level = -1;
	BOOL bLevelCheck = FALSE;
	...
	LogAddTD("[Illusion Temple] Search Illusion Temple items");
	...
	if ( FoundOtherItem != FALSE )
	{
		LogAddTD("[Illusion Temple] Others Illusion items found");
		LogAddC(5, "[Illusion Temple] Others Illusion items found");
		return 0;
	}

	if ( bLevelCheck != FALSE )
	{
		LogAddTD("[Illusion Temple] DiffLevel Illusion items Found");
		LogAddC(5, "[Illusion Temple] DiffLevel Illusion items Found");
		return 3;
	}

	if ( bCharmOfLuckOver == TRUE )
	{
		LogAddTD("[Illusion Temple] Charm Of Luck Count Over");
		return 4;
	}

	if ( bChaoseGem != FALSE && bPotionOfIllusion != FALSE && bScrollOfIllusion != FALSE )
	{
		LogAddTD("[Illusion Temple] Items Found");
		return 1;
	}

	if ( bPotionOfIllusion != FALSE && bScrollOfIllusion != FALSE )
	{
		LogAddTD("[Illusion Temple] Items Found");
		return 2;
	}

	LogAddTD("[IllusionMix] Illusion Items not found");
	LogAddC(5, "[IllusionMix] Illusion Items not found");
...
BOOL IllusionTempleEventChaosMix(OBJECTSTRUCT* lpObj, BOOL bCheckType, int iItemLevel)
{
	BOOL fail = TRUE;
	int aIndex = lpObj->m_Index;
	PMSG_CHAOSMIXRESULT pMsg;
	...
	LogAddTD("[Illusion Temple] Chaos Mix Start");
	LogAddC(3, "[Illusion Temple] Chaos Mix Start");
	...
	switch ( iItemLevel )
	{
		...
		default:
			LogAddTD("[Illusion Temple] [%s][%s] Invalid ITItem Level [%d]",
				lpObj->AccountID, lpObj->Name, iItemLevel);
	...
	if ( (lpObj->Money - nChaosNeedMoney) < 0 )
	{
		LogAddTD("[Illusion Temple] [%s][%s] CBMix Not Enough Money [%d] need zen [%d]",
			lpObj->AccountID, lpObj->Name, lpObj->Money, nChaosNeedMoney);

		LogAddC(3, "[Illusion Temple] [%s][%s] CBMix Not Enough Money [%d] need zen [%d]",
			lpObj->AccountID, lpObj->Name, lpObj->Money, nChaosNeedMoney);
	...
	if ( bCheckType == TRUE )
	{
		if ( iRate < lpObj->ChaosSuccessRate )
		{
			...
			ItemSerialCreateSend(lpObj->m_Index, -1, 0, 0, IllusionInv, iItemLevel, 0, 0, 0, 0, -1, 0, 0);
			fail = FALSE;
		}
	}
	...
	if ( fail == TRUE )
	{
		...
		LogAddTD("[Illusion Temple Invite,%d] [%s][%s] CBMix Fail %d Money : %d-%d", iItemLevel,
			lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate,
			lpObj->Money, nChaosNeedMoney);

		LogAddC(3, "[Illusion Temple Invite,%d] [%s][%s] CBMix Fail %d Money : %d-%d", iItemLevel,
			lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate,
			lpObj->Money, nChaosNeedMoney);
	...
	{
		LogAddTD("[Illusion Temple Invite,%d] [%s][%s] CBMix Success Rate:%d Money : %d", iItemLevel,
			lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate,
			nChaosNeedMoney);

		LogAddC(3, "[Illusion Temple Invite,%d] [%s][%s] CBMix Success Rate:%d Money : %d", iItemLevel,
			lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate,
			nChaosNeedMoney);
	}
...
void IllusionTempleItemChaosMix(OBJECTSTRUCT* lpObj)
{
	...
	int eventitemcount;
	int itemlevel;
	int Ret = ::CheckIllusionTempleItem(lpObj, eventitemcount, itemlevel);
	...
	if ( Ret != FALSE )
	{
		if ( Ret == 3 )
		{
			pMsg.Result = CB_INCORRECT_MIX_ITEMS;
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.h.size);
			LogAddTD("[Illusion Temple] DiffLevel Illusion Potion or Scroll [%d]", eventitemcount);
			lpObj->ChaosLock = FALSE;
		...
		if ( Ret == 2 )
		{
			pMsg.Result = CB_INCORRECT_MIX_ITEMS;
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.h.size);
			LogAddTD("[Illusion Temple] Not Found Chaos Gem [%d]", eventitemcount);
			lpObj->ChaosLock = FALSE;
		...
		if ( Ret == 4 )
		{
			pMsg.Result = 0xF0;
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.h.size);
			LogAddTD("[Illusion Temple] Charm Of Luck Over 10% [%d]", eventitemcount);
			lpObj->ChaosLock = FALSE;
		...
		if ( eventitemcount > 2 )
		{
			pMsg.Result = CB_TOO_MANY_ITEMS;
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.h.size);
			LogAddTD("[Illusion Temple] Too many Illusions's Potions or Scrolls [%d]", eventitemcount);
			lpObj->ChaosLock = FALSE;
...
void SecromiconMix(LPOBJ lpObj)
{
	...
	if(first == 1 && second == 1 && third == 1 && fourth == 1 && fifth == 1 && sixth == 1)
	{
		...
		lpObj->Money -= iMixCost;
		g_CastleSiegeSync.AddTributeMoney(iMixTax);
		GCMoneySend(lpObj->m_Index, lpObj->Money);

		ItemSerialCreateSend(lpObj->m_Index, 255, 0, 0,ITEMGET(14,109), 0, 0, 0, 0, 0, lpObj->m_Index, 0, 0);
		gObjInventoryCommit(lpObj->m_Index);

		LogAddTD("[SecromiconMix] (%s)(%s) Success ItemID: %d", lpObj->AccountID, lpObj->Name, ITEMGET(14,109));
	}
	lpObj->ChaosLock = FALSE;
}