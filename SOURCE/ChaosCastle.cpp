...
CChaosCastle::CChaosCastle()
{
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_LEVEL;i++)
	{
		...
		this->m_stChaosCastleData[i].m_iMapNumber = this->GetCCMap(i);
...
void CChaosCastle::PullObjInnerPlace(int iChaosCastleIndex, int iTRAP_STEP)
{
	...
	for ( int iAXIS = 0; iAXIS < MAX_CC_TRAP_STEP ; iAXIS++)
	{
		for ( int iMAPX = g_rtPOINT_TRAP[iAXIS].left ; iMAPX <= g_rtPOINT_TRAP[iAXIS].right ; iMAPX ++ )
		{
			...
			{
				BYTE btMapAttr = MapC[this->GetCCMap(iChaosCastleIndex)].GetAttr(iMAPX, iMAPY);
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		...
		{
			if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) ==TRUE && gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == (this->GetCCMap(iChaosCastleIndex)) )
			{
				...
				BYTE btMapAttr = MapC[this->GetCCMap(iChaosCastleIndex)].m_attrbuf[iSY * 256 + iSX] & 0x08;
	...
	{
		if ( MapC[this->GetCCMap(iChaosCastleIndex)].m_cItem[iMAP_ITEM].IsItem() )
		{
			int x = MapC[this->GetCCMap(iChaosCastleIndex)].m_cItem[iMAP_ITEM].px;
			int y = MapC[this->GetCCMap(iChaosCastleIndex)].m_cItem[iMAP_ITEM].py;
			...
			{
				MapC[this->GetCCMap(iChaosCastleIndex)].m_cItem[iMAP_ITEM].m_State = 8;
				MapC[this->GetCCMap(iChaosCastleIndex)].m_cItem[iMAP_ITEM].Give = true;
				MapC[this->GetCCMap(iChaosCastleIndex)].m_cItem[iMAP_ITEM].live = false;
			}
		}
	}
}
...
void CChaosCastle::SetMapAttrHollow(int iChaosCastleIndex, int iTRAP_STEP)
{
	...
	for ( int a=0;a<MAX_CC_HOLLOW_ZONE;a++)
	{
		for ( int b=::g_iChaosCastle_MapHollowZone[iTRAP_STEP][a][0] ; b<=::g_iChaosCastle_MapHollowZone[iTRAP_STEP][a][2] ; b++)
		{
			...
			{
				MapC[this->GetCCMap(iChaosCastleIndex)].m_attrbuf[ c * 256 + b] |= 8;
			}
		}
	}
...
void CChaosCastle::SetState_Closed(int iChaosCastleIndex)
{
	...
	{
		if ( gObj[n].MapNumber == (this->GetCCMap(iChaosCastleIndex)) )
		{
...
int  CChaosCastle::SetMonster(int iChaosCastleIndex)
{
	...
	for ( int n=0;n<gMSetBase.m_Count;n++)
	{
		...
		if ( CC_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber) != FALSE )
		{
			...
			BYTE btChaosCastleIndex = this->GetCCIndex(btMapNumber);
...
int  CChaosCastle::GetMonsterListCount(int iChaosCastleIndex)
{
	...
	int iMapNumber = this->GetCCMap(iChaosCastleIndex);
...
void CChaosCastle::ProcessFallUser(int iChaosCastleIndex)
{
	...
	for ( int iFALL=0;iFALL<MAX_CHAOSCASTLE_USER;iFALL++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_lFallUser[iFALL] != -1 )
		{
			...
			if ( (gObj[iFALL_INDEX].MapNumber) == (this->GetCCMap(iChaosCastleIndex)) &&
...
int  CChaosCastle::GetUserLevelToEnter(int iUserIndex)
{
	...
	for(int i=0;i<MAX_CHAOSCASTLE_LEVEL;i++)
	{
		...
		{
			if(gObjIsMasteringLevel(iUserIndex) == FALSE)
			{
				return 5;
			}
			else
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
			if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) == TRUE &&
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == ( this->GetCCMap(iChaosCastleIndex)) &&	
				this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iUserState == 0 )
			{
...
void CChaosCastle::SearchNBlowObjs(int iMapNumber, int iX, int iY)
{
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if ( this->m_stChaosCastleData[this->GetCCIndex(iMapNumber)].m_UserData[i].m_iIndex == -1 )
			continue;

		int iIndex = this->m_stChaosCastleData[this->GetCCIndex(iMapNumber)].m_UserData[i].m_iIndex;
		...
		if ( gObj[iIndex].MapNumber == iMapNumber && gObj[iIndex].Connected > PLAYER_LOGGED )
		{
				 ...
				 gObj[iIndex].Y >= iMIN_Y && gObj[iIndex].Y <= iMAX_Y )
			{
				...
				if ( btMapAttr == 8 )
				{
					this->AddFallUser(this->GetCCMap(iMapNumber), iIndex);

					LogAddTD("[Chaos Castle] (%d) [%s][%s] User Dead In Chaos Castle : Fall from Castle (X:%d, Y:%d)",
						this->GetCCIndex(iMapNumber)+1, gObj[iIndex].AccountID, gObj[iIndex].Name, iX, iY);
				}
			}
		}
	}
...
void CChaosCastle::SafetyCastleZone(int iChaosCastleIndex)
{
	...
	for ( int i=g_btCastleSafetyZoneMapXY[iChaosCastleIndex][0] ; i <= g_btCastleSafetyZoneMapXY[iChaosCastleIndex][2] ; i++ )
	{
		...
		{
			MapC[this->GetCCMap(iChaosCastleIndex)].m_attrbuf[ j * 256 + i] |= 1;
		}
	}
	...
	for ( int iHOLE_NUM = 0; iHOLE_NUM <2;iHOLE_NUM++)
	{
		for(int i=g_btCastleCenterHollowZoneMapXY[iChaosCastleIndex][iHOLE_NUM][0] ; i<= g_btCastleCenterHollowZoneMapXY[iChaosCastleIndex][iHOLE_NUM][2] ; i++ )
		{
			...
			{
				MapC[this->GetCCMap(iChaosCastleIndex)].m_attrbuf[ j * 256 + i] |= 8;
			}
		}
	}
}
...
void CChaosCastle::SendCastleZoneSafetyInfo(int iChaosCastleIndex, bool bDoSet)
{
	...
	for (int i=OBJ_STARTUSERINDEX;i<OBJMAX;i++)
	{
		if ( gObj[i].MapNumber == (this->GetCCMap(iChaosCastleIndex)) )
		{
		...
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
				 ...
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == (this->GetCCMap(iChaosCastleIndex)) )
			{
					   ...
					   iSY > g_iChaosCastle_DamageAxis[iTRAP_STEP][3] ) )
				{
					...
					if ( BuffEffectC.BuffSearch(&gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex], 56) != 0 )
					{
						...
						BuffEffectC.EnableBuff(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex, 56, 3, 0, 0, 0);
					}
...
BOOL CChaosCastle::CheckWearingMOPH(int iUserIndex)
{
	...
	if ( gObj[iUserIndex].pInventory[10].IsItem()  )
	{
		...
		if ( gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,40) ) // Hallowin
		{
			return TRUE;
		}

		if ( gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,41) ) // Santa Ring
		{
			return TRUE;
		}

		if ( gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,42) ) // GM Ring
		{
			return TRUE;
		}

		if ( gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,68) ) // Snowman Ring
		{
			return TRUE;
		}

		if ( gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,76) ) // Panda Ring
		{
			return TRUE;
		}
	}
	...
	if ( gObj[iUserIndex].pInventory[11].IsItem() )
	{
		...
		if ( gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,40) ) // Hallowin
		{
			return TRUE;
		}

		if ( gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,41) ) // Santa Ring
		{
			return TRUE;
		}

		if ( gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,42) ) // GM Ring
		{
			return TRUE;
		}

		if ( gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,68) ) // Snowman Ring
		{
			return TRUE;
		}

		if ( gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,76) ) // Panda Ring
		{
			return TRUE;
		}
	}
		...
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
			if ( gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Connected == PLAYER_PLAYING )
			{
				...
				LogAddTD("[Chaos Castle][Bug Tracer] (%d) [%s][%s] Reword ZettoMoney(Map:%d) Send Message",
					iChaosCastleIndex+1, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].AccountID,
					gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Name,
					this->GetCCIndex(gObj[iZettoWinnerIndex].MapNumber));
			}
		}
	}
