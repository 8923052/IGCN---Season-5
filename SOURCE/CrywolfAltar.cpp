...
void CCrywolfAltar::SetAltarValidContract(int iAltarObjIndex, int iClass)
{
	...
	{
		MsgOutput(AltarInfo->m_iUserIndex, "You contracted %d Altar Successfuly", iAltarNumber);
		UTIL.SendCrywolfUserAnyMsg(2, "%s contracted Altar Nr. %d", gObj[AltarInfo->m_iUserIndex].Name, iAltarNumber);
		...
		BuffEffectC.ClearBuff(&gObj[iAltarObjIndex]);
		BuffEffectC.EnableBuff(iAltarObjIndex, 23, -1, 0, 0, 0);
	}
}
...
BOOL CCrywolfAltar::SetAltarUserIndex(int iAltarObjIndex, int iClass, int iUserIndex)
{
	...
	BuffEffectC.ClearBuff(&gObj[iAltarObjIndex]);
	BuffEffectC.EnableBuff(iAltarObjIndex, 24, -1, 0, 0, 0);
...
void CCrywolfAltar::ResetAltarUserIndex(int iAltarObjIndex, int iClass)
{
	...
	if ( AltarInfo->m_iContractCount >= MAX_ALTAR_CONTRACTS )
	{
		...
		BuffEffectC.ClearBuff(&gObj[iAltarObjIndex]);
		BuffEffectC.EnableBuff(iAltarObjIndex, 22, -1, 0, 0, 0);
	}
	else
	{
		BuffEffectC.ClearBuff(&gObj[iAltarObjIndex]);
		BuffEffectC.EnableBuff(iAltarObjIndex, 21, -1, 0, 0, 0);
...
//..	GCStateInfoSend(&gObj[AltarInfo->m_iAltarIndex], 0, 0,0,0); // CHECAR
	BuffEffectC.ClearBuff(&gObj[AltarInfo->m_iAltarIndex]);
	AltarInfo->Reset();
}
...
void CCrywolfAltar::SetAltarViewState(int iClass, int iAltarState)
{
	...
	BuffEffectC.ClearBuff(&gObj[AltarInfo->m_iAltarIndex]);
	BuffEffectC.EnableBuff(AltarInfo->m_iAltarIndex, 21, -1, 0, 0, 0);
}
