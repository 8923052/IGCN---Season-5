...
BOOL CDarkSpirit::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, int criticaldamage, int iActionType)
{
	...
	int DurabilityVal = 800 + lpObj->MasterSkillTreeEffect.DecreasePetDurabilityWane;
	...
	if ( lpObj->pInventory[1].m_DurabilitySmall >= DurabilityVal )
	{
		...
		if ( lpObj->pInventory[1].m_Durability < 1.0f )
		{
			...
			LogAddTD("[%s][%s] Dark Raven Item is Broken because durability is exhausted [%d]",
	...
	if ( AttackDamage >= 1 )
	{
		...
		if ( lpTargetObj->m_SkillHarden )
		{
			...
			if ( lpTargetObj->m_SkillHarden <= 0 )
			{
				...
				BuffEffectC.DelEffect(lpTargetObj, 57);
