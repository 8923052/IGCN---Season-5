...
{
	//FinalChestItemBag->DropGoldenChestItem(lpChest->m_Index, lpChest->MapNumber, lpChest->X, lpChest->Y);
	lpChest->Life = 0.0;
	gObjLifeCheck(lpChest, lpChest, 1, 0, 0, 0, 0, 0);
}
...
bool CDoppelGanger::SetEventLevel()
{
	...
	for(int i=0;i<5;++i)
	{
		if((level = (gObj[this->m_DGData.PlayerIndex[i]].Level + gObj[this->m_DGData.PlayerIndex[i]].m_uMasterLevel)) > temp)
		{
			temp = level;
		}
	}
