	...
	for(int i = 0; i < MAX_DUEL_ROOMS; i++)
	{
		if(this->m_Rooms[i].bFree == FALSE)
		{
			...
			for(int u = 0; u < MAX_DUEL_LEARNERS; u++)
			{
				...
				{
					if(this->m_Rooms[i].lpLearners[u]->MapNumber != MAP_INDEX_DUELROOM)
					{
						this->SendEndDuel(this->m_Rooms[i].lpLearners[u]);
						BuffEffectC.DelEffect(this->m_Rooms[i].lpLearners[u], 18);
						BuffEffectC.DelEffect(this->m_Rooms[i].lpLearners[u], 27);
						BuffEffectC.DelEffect(this->m_Rooms[i].lpLearners[u], 98);
			...
			if(this->m_Rooms[i].lpObj01 != NULL && this->m_Rooms[i].lpObj02 != NULL)
			{
				...
				else if ((this->m_Rooms[i].lpObj01->MapNumber != MAP_INDEX_DUELROOM ||
					this->m_Rooms[i].lpObj02->MapNumber != MAP_INDEX_DUELROOM) && 
					this->m_Rooms[i].bHasWinner == FALSE)
				{
					this->RoomReset(i);
					continue;
...
void CDuelManager::RoomReset(int iRoom, bool dontMove, bool dontSendEnd)
{
	...
	for(int i = 0; i < MAX_DUEL_LEARNERS; i++)
	{
		if(this->m_Rooms[iRoom].lpLearners[i] != NULL)
		{
			...
			BuffEffectC.DelEffect(this->m_Rooms[iRoom].lpLearners[i], 18);
			BuffEffectC.DelEffect(this->m_Rooms[iRoom].lpLearners[i], 27);
			BuffEffectC.DelEffect(this->m_Rooms[iRoom].lpLearners[i], 98);
...
{ 
	...
	if(lpTarget->m_btDuelScore >= DUEL_WIN_POINT_COUNT)
	{
		lpObj->m_btDuelScore = 0;
		lpTarget->m_btDuelScore = 0;
		this->m_Rooms[iDuelRoom].bHasWinner = TRUE;

		this->SendEndDuel(lpTarget);
		this->SendEndDuel(lpObj);

		this->SendEndDuelNotification(lpTarget, lpTarget->Name, lpObj->Name);
		this->SendEndDuelNotification(lpObj, lpTarget->Name, lpTarget->Name);
		...
		wsprintf(szMsg,lMsg.Get(1217),lpTarget->Name);
		...
		lpTarget->m_ViewSkillState[eVS_DUEL_MEDAL] = 1;
		lpTarget->m_GladiatorGloryBuffTime = 3600;
		BuffEffectC.EnableBuff(lpObj->m_Index,eVS_DUEL_MEDAL, 3600, 0, 0,103);
		this->m_Rooms[iDuelRoom].dwTickCount = GetTickCount() + 10000;
		gObjMoveGate(lpObj->m_Index,294);
		gObjMoveGate(lpTarget->m_Index,294);
		LogAdd("[Duel] [%s][%s] Win Duel, Loser [%s][%s]",lpTarget->AccountID,lpTarget->Name,lpObj->AccountID,lpObj->Name);
		...
		for(int i = 0; i < MAX_DUEL_LEARNERS; i++)
		{
			if(this->m_Rooms[lpMsg->btRoomIndex].lpLearners[i] == NULL)
			{
				if(gObjMoveGate(lpObj->m_Index, g_DuelGates[lpMsg->btRoomIndex].LearnerGate))
				{
					...
					BuffEffectC.EnableBuff(lpObj->m_Index, 98, -1, 0, 0, 0);
					BuffEffectC.EnableBuff(lpObj->m_Index, 18, -1, 0, 0, 0);
