...
void CGMMng::Init()
{
	...
	this->cCommand.Add("/item", 390);
	this->cCommand.Add("/online",400);
//	this->cCommand.Add("/info",401);
	this->cCommand.Add("/i",402);
	this->cCommand.Add("/ware",403);
...
void CGMMng::GMDataSend(unsigned char* szMsg, int size)
{
	for ( int n=0;n<MAX_GAME_MASTER;n++ )
	{
		if ( this->ManagerIndex[n] != -1 )
...
char * SwearFilter(char *szText)
{
	...
	char *fix;
	fix = strpbrk(szText,swear1);
	fix = strpbrk(szText,swear2);
	fix = strpbrk(szText,swear3);
	return fix;
}
...
int CGMMng::ManagementProc(LPOBJ lpObj, char* szCmd, int aIndex)
{
	...
	switch ( command_number )
	{
		...
		case 402:
			{
				...
				if ( iTargetIndex >= 0 )
				{
					...
					MsgOutput(lpObj->m_Index,"[INFO] User ACC = %s", lpTargetObj->AccountID);
		...
		case 403:
			{
				...
				WareId = this->GetTokenNumber();
				if(WareId > 3 || WareId < 0)
				{
					MsgOutput(lpObj->m_Index,"[WareHouse switch 0 - 3 only]");
					return 0;
				}

				lpObj->m_btWareHouseId = WareId;
				MsgOutput(lpObj->m_Index,"Warehouse %d loaded",WareId);
			}
			break;
		...
		case 390:
		{
			...
			{
				MsgOutput(aIndex,"You are not allowed to use admin cmd");
				return FALSE;
			}
		...
		case 401:
			{
				...
				char *cName = {0};
				cName = this->GetTokenString();

				for(int i=OBJ_STARTUSERINDEX;i<OBJMAX;i++)
				{
					if(gObj[i].Name == cName)
					{
						MsgOutput(aIndex,"AccId: %s, CharName: %s",gObj[i].AccountID,gObj[i].Name);
						return true;
					}
				}
				return true;
		...
		{
			int Level = 50;
			if(lpObj->Level<gPostLevel)
			{
				char levelmsg[100];
				sprintf(levelmsg,"[Warning]: to use /post you need level %d ",gPostLevel);
				GCServerMsgStringSend(levelmsg,aIndex,1);
				return FALSE;
			}
			if(lpObj->Money < gPostMoney)
			{
				char moneymsg[100];
				sprintf(moneymsg,"[Warning]: You need %d money for post cmd",gPostMoney);
				GCServerMsgStringSend(moneymsg,aIndex,1);
				return FALSE;
			}
			
			lpObj->Money -= gPostMoney;
			GCMoneySend(lpObj->m_Index,lpObj->Money);

			MsgSrv("~[POST] %s: %s",lpObj->Name,(char*)szCmd+strlen("/post"));		
		//	MakeRewardSetItem(lpObj->m_Index, lpObj->X, lpObj->Y,1, lpObj->MapNumber);
			return TRUE;
		}
		break;
		...
		case 392:
		{
			if(gPKClearEnabled == 0)
			{
				GCServerMsgStringSend("PK Clear Disabled",lpObj->m_Index,1);
				return true;
			}
			if(lpObj->m_PK_Level <= 3) 
			{
			char Msg[100] = "you are not PK!";
			GCServerMsgStringSend(Msg,lpObj->m_Index,1);
			return TRUE;
			}
			else if(lpObj->Money < gPKClearCost)
			{
			char Msg[100] = "Infusicent zen for clear PK!";
			GCServerMsgStringSend(Msg,lpObj->m_Index,1);
			return TRUE;
			}
			else if(lpObj->Money >= gPKClearCost)
			{
			lpObj->Money -= gPKClearCost;
			GCMoneySend(lpObj->m_Index,lpObj->Money);
			lpObj->m_PK_Level = 3;
			GCPkLevelSend(lpObj->m_Index, 3);
			char Msg[100] = "[PKCLear] PK Cleared";
		...
		case 393:
		{
			...
			if(Pontos > 200)
			{
				GCServerMsgStringSend("[CmdAdd]: Dont add more than 200 point in time",lpObj->m_Index,1);
				return FALSE;
			}

			for(int p=0;p<Pontos;p++)
			{
				gObjLevelUpPointAdd(0,lpObj);
				PMSG_LVPOINTADDRESULT pMsg;
				PHeadSubSetB((LPBYTE)&pMsg, 0xF3, 0x06, sizeof(pMsg));
				pMsg.ResultType = 0x10;
				pMsg.ResultType += 0;
				pMsg.wMaxShield = gObj[lpObj->m_Index].iMaxShield + gObj[lpObj->m_Index].iAddShield;
				gObjSetBP(aIndex);
				pMsg.MaxBP = gObj[lpObj->m_Index].MaxBP + gObj[aIndex].AddBP;
		...
		case 394:
		{
			...
			{
				GCServerMsgStringSend("[CmdAdd]: Cant add more than 65000!.",lpObj->m_Index,1);
				return FALSE;
			}
		...
		case 400:
			{
				...
				MsgOutput(lpObj->m_Index,"[Online] Players Online %d, GM Online %d",oncount,gmoncount);
				return TRUE;
			}
			break;
