...
#include "TNotice.h"
CImperialBattleOfErkanne::CImperialBattleOfErkanne()
{
	this->ResetData();
}
...
void CImperialBattleOfErkanne::ResetData()
{
	this->m_BattleData.Reset();
}

void CImperialBattleOfErkanne::SetState(int iState)
{
	switch ( iState )
	{
		case 0:
...
void CImperialBattleOfErkanne::SetState_NONE()
{
	LogAddC(2, "[ IMPERIAL FORT ] [ BattleOfErkanne ] State(%d) -> NONE", this->m_BattleData.CurrentState);
	this->ResetData();
	TMonsterAIGroup::DelGroupInstance(27);
	TMonsterAIGroup::DelGroupInstance(28);
	TMonsterAIGroup::DelGroupInstance(34);
	this->m_BattleData.CurrentState = BATTLEOFERKANNE_STATE_NONE;
}


void CImperialBattleOfErkanne::SetState_STANDBY1()
{
	LogAddC(2, "[ IMPERIAL FORT ] [ BattleOfErkanne ] State(%d) -> STANDBY1", this->m_BattleData.CurrentState);
	for(int i=0;i<3;++i)
	{
		...
		if(this->m_BattleData.GateIndex[i] >= 0)
		{
			...
			LogAddTD("[ IMPERIAL FORT ] [ BattleOfErkanne ] Create Gate Index: %d Class: %d",
				this->m_BattleData.GateIndex[i], g_VermontGate[i].Class);
		}
	}
	this->m_BattleData.Time = 180000;
	...
	this->m_BattleData.CurrentState = BATTLEOFERKANNE_STATE_STANDBY1;
	this->SendEventState();
}

void CImperialBattleOfErkanne::SetState_STANDBY2()
{
	LogAddC(2, "[ IMPERIAL FORT ] [ BattleOfErkanne ] State(%d) -> STANDBY2", this->m_BattleData.CurrentState);
	for(int i=3;i<6;++i)
	{
		this->m_BattleData.GateIndex[i] = gObjAddMonster(70);
		if(this->m_BattleData.GateIndex[i] >= 0)
	...
	this->m_BattleData.CurrentState = BATTLEOFERKANNE_STATE_STANDBY2;
	this->SendEventState();
}

void CImperialBattleOfErkanne::SetState_STANDBY3()
{
	LogAddC(2, "[ IMPERIAL FORT ] [ BattleOfErkanne ] State(%d) -> STANDBY3", this->m_BattleData.CurrentState);
	...
	this->m_BattleData.TickCount = GetTickCount();
	TMonsterAIGroup::DelGroupInstance(28);
	TMonsterAIGroup::Init(34);
	this->m_BattleData.MonsterCount = 0;
	this->m_BattleData.CurrentLeaderIndex = TMonsterAIGroup::FindGroupLeader(34);
	...
	this->m_BattleData.CurrentAIGroup = 34;
	this->m_BattleData.CurrentState = BATTLEOFERKANNE_STATE_STANDBY3;
	this->SendEventState();
}

void CImperialBattleOfErkanne::SetState_PHAZE1()
{
	LogAddC(2, "[ IMPERIAL FORT ] [ BattleOfErkanne ] State(%d) -> PHAZE1", this->m_BattleData.CurrentState);
	...
	this->m_BattleData.CurrentState = BATTLEOFERKANNE_STATE_PHAZE1;
}

void CImperialBattleOfErkanne::SetState_PHAZE2()
{
	LogAddC(2, "[ IMPERIAL FORT ] [ BattleOfErkanne ] State(%d) -> PHAZE2", this->m_BattleData.CurrentState);
	...
	TNotice::MakeNoticeMsg(&pNotice, 0, "Quest Started!");
	this->SendImperialPlayers((LPBYTE)&pNotice, pNotice.h.size);
	this->m_BattleData.CurrentState = BATTLEOFERKANNE_STATE_PHAZE2;
}

void CImperialBattleOfErkanne::SetState_PHAZE3()
{
	LogAddC(2, "[ IMPERIAL FORT ] [ BattleOfErkanne ] State(%d) -> PHAZE3", this->m_BattleData.CurrentState);
	...
	this->m_BattleData.Time = 600000;
	this->m_BattleData.TickCount = GetTickCount();
	TMonsterAIGroup::ChangeAIOrder(34, 1);
	this->m_BattleData.MonsterCount = TMonsterAIGroup::s_iMonsterAIGroupMemberCount[34];
	this->m_BattleData.PhazeStep = 0;
	this->m_BattleData.NotifyCount = -1;
	PMSG_NOTICE pNotice;
	TNotice::MakeNoticeMsg(&pNotice, 0, "Quest Started!");
	this->SendImperialPlayers((LPBYTE)&pNotice, pNotice.h.size);
	this->m_BattleData.CurrentState = BATTLEOFERKANNE_STATE_PHAZE3;
}

void CImperialBattleOfErkanne::SetState_END()
{
	LogAddC(2, "[ IMPERIAL FORT ] [ BattleOfErkanne ] State(%d) -> END", this->m_BattleData.CurrentState);
	this->m_BattleData.CurrentState = BATTLEOFERKANNE_STATE_END;
	return;
}



void CImperialBattleOfErkanne::Run()
{
...
void CImperialBattleOfErkanne::ProcState_STANDBY1()
{
	...
	if(Tick >= 1000)
	{
		...
		{
			this->SetState(BATTLEOFERKANNE_STATE_PHAZE1);
		}
	}
}

void CImperialBattleOfErkanne::ProcState_PHAZE1()
{
	int Tick = GetTickCount() - this->m_BattleData.TickCount;
	if(Tick >= 1000)
	{
		...
		for(int i=0;i<12;++i)
		{
			for(int n=0;n<g_ImperialBattleUserMng.GetUserCount();++n)
			{
				...
				{
					gObjAttack(&gObj[this->m_BattleData.TrapIndex], &gObj[g_ImperialBattleUserMng.m_BattleUser[n].GetIndex()], NULL, 0, 0, 0, 0);
				}
			}
		}
...
void CImperialBattleOfErkanne::ProcState_STANDBY2()
{
	int Tick = GetTickCount() - this->m_BattleData.TickCount;
	if(Tick >= 1000)
	{
		...
		if(this->m_BattleData.Time <= 0)
		{
			this->SetState(BATTLEOFERKANNE_STATE_PHAZE2);
		}
	}
}

void CImperialBattleOfErkanne::ProcState_PHAZE2()
{
	int Tick = GetTickCount() - this->m_BattleData.TickCount;
	if(Tick >= 1000)
	{
...
void CImperialBattleOfErkanne::ProcState_STANDBY3()
{
	int Tick = GetTickCount() - this->m_BattleData.TickCount;
	if(Tick >= 1000)
	{
		...
		if(this->m_BattleData.Time <= 0)
		{
			this->SetState(BATTLEOFERKANNE_STATE_PHAZE3);
		}
	}
}

void CImperialBattleOfErkanne::ProcState_PHAZE3()
{
	int Tick = GetTickCount() - this->m_BattleData.TickCount;
	if(Tick >= 1000)
	{
		...
		if(gObj[this->m_BattleData.CurrentLeaderIndex].Live == false)
		{
			...
			this->SetState(BATTLEOFERKANNE_STATE_END);
		}
	}
}

void CImperialBattleOfErkanne::ProcState_END()
{
	return;
}

int CImperialBattleOfErkanne::GetEventTime()
{
	return this->m_BattleData.Time / 1000;
}

int CImperialBattleOfErkanne::GetEventState()
{
...
void CImperialBattleOfErkanne::SendImperialPlayers(LPBYTE lpMsg, int iSize)
{
	int index;
...
void CImperialBattleOfErkanne::SendEventInterface()
{
...
void CImperialBattleOfErkanne::SetMapAttr(int Gate)
{
	char Buff[256];
	short PacketSize = 0;
...
void CImperialBattleOfErkanne::MonsterProc()
{
	short alivecount = 0;
...
void CImperialBattleOfErkanne::OperatePortalGate(bool Block, bool ForAll, int aIndex)
{
	return;
}

void CImperialBattleOfErkanne::SetNextState()
{
	int iNextState = this->m_BattleData.CurrentState + 1;

	if ( iNextState >= BATTLEOFERKANNE_STATE_END )
	{
		iNextState = BATTLEOFERKANNE_STATE_NONE;
	}

	this->SetState(iNextState);
}

bool CImperialBattleOfErkanne::EnterPortal(LPOBJ lpObj)
{
	if(this->m_BattleData.MonsterCount > 0)
		return false;
	
	if(this->m_BattleData.PhazeStep != 4)
		return false;
...
void CImperialBattleOfErkanne::SendEventState()
{
