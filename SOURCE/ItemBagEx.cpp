...
void CItemBagEx::Init(char* name)
{
	...
	char Path[256];
	sprintf(Path,"./EventBags/%s",name);

	this->LoadItem(gDirPath.GetNewPath(Path));
}
...
void CItemBagEx::LoadItem(char* script_file)
{
	int Token;
	char Path[256];
	sprintf(Path,"/EventBags/%s",script_file);
	this->m_bLoad = FALSE;
...
BOOL CItemBagEx::DropGameMasterBoxItem(int aIndex)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		else
		{
			...
			LogAddTD("[GM BOX] GameMaster BOX Event ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, this->m_sEventName,
				lpObj->MapNumber, x, y, ItemAttribute[type].Name,
				type, level, Option1, Option2, Option3);
		}
...
BOOL CItemBagEx::DropGreenChaosBoxItem(int aIndex)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		else
		{
			...
			LogAddTD("[Chaos Box] Green Chaos BOX Event ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, this->m_sEventName,
				lpObj->MapNumber, x, y, ItemAttribute[type].Name,
				type, level, Option1, Option2, Option3);
		}
...
BOOL CItemBagEx::DropRedChaosBoxItem(int aIndex)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		else
		{
			...
			LogAddTD("[GM BOX] Red Chaos BOX Event ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, this->m_sEventName,
				lpObj->MapNumber, x, y, ItemAttribute[type].Name,
				type, level, Option1, Option2, Option3);
		}
...
BOOL CItemBagEx::DropPurpleChaosBoxItem(int aIndex)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		else
		{
			...
			LogAddTD("[GM BOX] Purple Chaos BOX Event ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, this->m_sEventName,
				lpObj->MapNumber, x, y, ItemAttribute[type].Name,
				type, level, Option1, Option2, Option3);
		}
...
BOOL CItemBagEx::DropZaikanItem(int aIndex)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		DropItemRate = rand() % 1000;

		if ( DropItemRate < g_ZaikanZenDropRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(g_iDeepBlueCandyBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[Zaikan Item Drop] Zaikan Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				g_iDeepBlueCandyBoxDropZen);
		}
		else
		{
			...
			LogAddTD("[Zaikan] Zaikan Event ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, this->m_sEventName,
				lpObj->MapNumber, x, y, ItemAttribute[type].Name,
				type, level, Option1, Option2, Option3);
		}
...
BOOL CItemBagEx::DropPhoenixItem(int aIndex)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < g_PhoenixZenDropRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(g_iDeepBlueCandyBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[Phoenix Item Drop] Phoenix Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				g_iDeepBlueCandyBoxDropZen);
		}
		else
		{
			...
			LogAddTD("[Phoenix] Phoenix Event ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, this->m_sEventName,
				lpObj->MapNumber, x, y, ItemAttribute[type].Name,
				type, level, Option1, Option2, Option3);
		}
...
BOOL CItemBagEx::DropHellMainItem(int aIndex)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < g_HellMainZenDropRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(g_iDeepBlueCandyBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[Hell Main Item Drop] Zaikan Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				g_iDeepBlueCandyBoxDropZen);
		}
		else
		{
			...
			LogAddTD("[Hell Main] Hell Main Event ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, this->m_sEventName,
				lpObj->MapNumber, x, y, ItemAttribute[type].Name,
				type, level, Option1, Option2, Option3);
		}
...
BOOL CItemBagEx::DropMedusaItem(int aIndex)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < g_HellMainZenDropRate )
		{
			...
			LogAddTD("[Medusa Item Drop] Medusa droped [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				g_iDeepBlueCandyBoxDropZen);
		}
		else
		{
			...
			LogAddTD("[Medusa] Meduse Event ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, this->m_sEventName,
				lpObj->MapNumber, x, y, ItemAttribute[type].Name,
				type, level, Option1, Option2, Option3);
		}
...
BOOL CItemBagEx::DropSantaClausItem(int aIndex)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
			...
			LogAddTD("[XMAS EVENT] Santa ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, this->m_sEventName,
				lpObj->MapNumber, x, y, ItemAttribute[type].Name,
				type, level, Option1, Option2, Option3);
		}
...
void CItemBagEx::LoadBagList(LPSTR filename)
{

	if ( (SMDFile = fopen(filename, "r")) == NULL )
		 {
			MessageBoxA(0,"CItemBagEx::LoadBagList() error","CRITICAL ERROR",0);
			ExitProcess(1);
			return;
		 }
		SMDToken Token;
		...
		while(true)
		 {
			 ...
			 if(Token == NUMBER)
			 {
				 ...
				 while(true)
				 {
					...
					Token = GetToken();
					memcpy(Path,TokenString,sizeof(Path));

					Token = GetToken();
					isEffect = TokenNumber;
					gObjItemBags[BagCount].isEffect = isEffect;
					gObjItemBags[BagCount].ItemID = ITEMGET(itemCat,itemIdx);
					strcpy_s(gObjItemBags[BagCount].Path,Path);
					BagCount++;
				 }
			 }
			 }
...
BOOL CItemBagEx::DynamicItemDrop(int aIndex)
{
	if ( this->m_bLoad == FALSE )
		return FALSE;
	...
	if ( this->GetBagCount() > 0 )
	{
			...
			LogAddTD("[ExEventBag] ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, this->m_sEventName,
				lpObj->MapNumber, x, y, ItemAttribute[type].Name,
				type, level, Option1, Option2, Option3);
		}
