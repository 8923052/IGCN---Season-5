...
struct P_ItemBags
{
	...
	char Path[MAX_PATH];
	bool isEffect;
}; extern P_ItemBags gObjItemBags[200]; 
extern int BagCount;
...
public:
	...
	static void LoadBagList(LPSTR filename);
	...
	int DropGameMasterBoxItem(int aIndex);
	int DropPurpleChaosBoxItem(int aIndex);
	int DropRedChaosBoxItem(int aIndex);
	int DropGreenChaosBoxItem(int aIndex);
	int DropHellMainItem(int aIndex);
	int DropPhoenixItem(int aIndex);
	int DropZaikanItem(int aIndex);
	int DropHallowinEventItem(LPOBJ lpObj);
	int DropSantaClausItem(int aIndex);
	int DynamicItemDrop(int aIndex);
	BYTE GetMinLevel(int n);
	BYTE GetMaxLevel(int n);
	int PickItem(CItem& objItem, int& nItemIndex);
	BOOL DropMedusaItem(int aIndex);
