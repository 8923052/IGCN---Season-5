...
fclose(SMDFile);//#pragma message("Put fclose here CJewelOfHarmony::LoadScript")
	return TRUE;
}
...
fclose(SMDFile);//#pragma message("Put fclose here CJewelOfHarmony::LoadScriptSmelt")
	return TRUE;
}
...
BOOL CJewelOfHarmonySystem::StrengthenItemByJewelOfHarmony(LPOBJ lpObj, int source, int target)
{
	...
	lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type,
		...
		lpObj->pInventory[target].m_SetOption, lpObj->pInventory[target].m_ItemOptionEx, 3, &lpObj->pInventory[target].m_ItemSocket[0]);
...
BYTE CJewelOfHarmonySystem::ShowStrengthenOption(CItem *pItem)
{
	...
	if ( this->IsStrengthenByJewelOfHarmony(pItem) == FALSE )
	{
		LogAddTD("ShowStrengthenOption() FAIL on IsStrengthenByJewelOfHarmony(pItem) returns false");
		return bResult;
	}
