...
void CMagicDamage::LogSkillList(char * filename)
{
	...
	while ( true )
	{
		...
		if ( Token == 1 )
		{
			...
			this->SetMaster(number, IsMaster, iRank, iGroup, iMasterP);
...
BOOL CMagicDamage::CheckStatus(int iSkill, int iGuildStatus)
{
	...
	if ( this->m_iSkillUseType[iSkill] == 0 || this->m_iSkillUseType[iSkill] == 4 )
	{
		return TRUE;
	}
...
BOOL CMagicDamage::GetBrandOfSkill(int iSkill)
{
	if ( iSkill < 0 || iSkill > MAX_SKILL-1 )
	{
		LogAdd(lMsg.Get(MSGGET(1, 201)), __FILE__, __LINE__);
...
void CMagicDamage::SetMaster(int iSkill, bool IsMaster, int Rank, int Group, int MasterPoint)
{
	if ( iSkill < 0 || iSkill > MAX_SKILL-1 )
	{
		LogAdd(lMsg.Get(MSGGET(1,201)), __FILE__, __LINE__);
	...
	if(!IsMaster)
	{
		this->m_iMasterSkill[iSkill] = 0;
		this->m_iRank[iSkill] = 0;
		this->m_iGroup[iSkill] = 0;
		this->m_iMasterPoint[iSkill] = 0;
	}
	else
	{
		this->m_iMasterSkill[iSkill] = 1;
		this->m_iRank[iSkill] = Rank;
		this->m_iGroup[iSkill] = Group;
		this->m_iMasterPoint[iSkill] = MasterPoint;
	}
}
int CMagicDamage::GetMasterGroup(int skill)
{
	return this->m_iGroup[skill];
}
int CMagicDamage::GetMasterPoint(int skill)
{
	return this->m_iMasterPoint[skill];
}
int CMagicDamage::GetMasterSkillType(int skill)
{
