...
MapClass::MapClass()
{
	...
	SetRect(&this->gRegenRect[51], 51, 224, 54, 227);
	SetRect(&this->gRegenRect[57],222,211,225,212);
	this->m_ItemCount=0;
}
...
int MapClass::MonsterItemDrop(int type, int level, float dur,  int x, int y, BYTE Option1, BYTE Option2, BYTE Option3, BYTE NOption, BYTE SOption,  int aIndex, DWORD number, BYTE ItemEffectEx,BYTE* SOCKETOPT)
{
	int count;
	int counttot=0;
	...
	while ( true )
	{
		...
		{
			this->m_cItem[count].CreateItem(type, level, x, y, dur, Option1, Option2, Option3, NOption, SOption, number, ItemEffectEx, &SOCKETOPT[0]);
