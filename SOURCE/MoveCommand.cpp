...
int CMoveCommand::Load(char* filename)
{
	...
	while ( true )
	{
		...
		if ( Token == 1)
		{
			...
			Token = GetToken();
			this->m_MoveCommandData[counter].MaxLevel = TokenNumber;
			...
			Token = GetToken();
			strcpy(this->m_MoveCommandData[m_MoveCommandData[counter].Index].Name, TokenString);

			Token = GetToken();
			strcpy(this->m_MoveCommandData[m_MoveCommandData[counter].Index].EngName, TokenString);

			Token = GetToken();
			this->m_MoveCommandData[m_MoveCommandData[counter].Index].NeedLevel = TokenNumber;

			Token = GetToken();
			this->m_MoveCommandData[m_MoveCommandData[counter].Index].MaxLevel = TokenNumber;

			Token = GetToken();
			this->m_MoveCommandData[m_MoveCommandData[counter].Index].NeedZen = TokenNumber;

			Token = GetToken();
			this->m_MoveCommandData[m_MoveCommandData[counter].Index].GateNumber = TokenNumber;
			counter++;
		}
		else
		{
...
int CMoveCommand::Load(char* Buffer, int iSize)
{
	...
	while ( true )
	{
		...
		if ( Token == 1)
		{
			...
			Token = WzMemScript.GetToken();
			this->m_MoveCommandData[counter].MaxLevel = WzMemScript.GetNumber();
...
BOOL CMoveCommand::Move(LPOBJ lpObj, LPSTR mapname)
{
	...
	if ( GateNumber >= 0 )
	{
		if ( NeedLevel <= lpObj->Level )
		{
			if ( (lpObj->Money - NeedZen ) >= 0 )
			{
				if ( gObjMoveGate(lpObj->m_Index, GateNumber ) != FALSE )
				{
					...
					LogAddTD("[%s][%s] Use [%s %s] Success : reduces money %d - %d"
						...
						"/move", mapname, lpObj->Money, NeedLevel);
			...
			{
				::GCServerMsgStringSend("you are short of zen", lpObj->m_Index, 1);
			}
		}
		else
		{
			...
			wsprintf(szTemp,"You need level %d to  warp there", NeedLevel);
