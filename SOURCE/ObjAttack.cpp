...
BOOL CObjAttack::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf* lpMagic,  int magicsend, unsigned char MSBFlag, int AttackDamage, BOOL bCombo)
{
	...
	if ( lpObj->MapNumber != lpTargetObj->MapNumber )
		return FALSE;
	if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER && lpObj->MapNumber == MAP_INDEX_RAKLIONBOSS)
	{
		return FALSE;
	}
	...
	BOOL bIsOnDuel = g_DuelManager.DuelCheck(lpObj, lpTargetObj);
...
//			lpTargetObj->m_ViewSkillState |= 0x20;
	...
	if ( AttackDamage >= 1 )
	{
		if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER )
		{
			...
			else if ( PvPGens.GensCheck(lpObj, lpTargetObj) )
			{
				selfdefense = 0;
			}
...
//				lpTargetObj->m_ViewSkillState &= -33;
	...
	/*if (lpTargetObj->Class == 409 && gObj[lpObj->m_Index].m_Quest[1] == 0xf6)
	{
		gObj[lpObj->m_Index].m_Quest[4] += 1;
	}

	if (lpTargetObj->Class == 410 && gObj[lpObj->m_Index].m_Quest[1] == 0xf6)
	{
		gObj[lpObj->m_Index].m_Quest[5] += 1;
	}

	if (lpTargetObj->Class == 411 && gObj[lpObj->m_Index].m_Quest[1] == 0xf6)
	{
		gObj[lpObj->m_Index].m_Quest[6] += 1;
	}


	if (lpTargetObj->Class == 412 && gObj[lpObj->m_Index].m_Quest[1] == 0xda)
	{
		gObj[lpObj->m_Index].m_Quest[7] += 1;
	}*/
