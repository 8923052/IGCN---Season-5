		...
		for ( int n =0;n<lpQuestInfo->QuestSubInfoCount;n++)
		{
			...
			if ( lpSubInfo != NULL )
			{
				...
				if ( lpSubInfo->QuestType == 2 && lpSubInfo->NeedTargetMinLevel == -1 ) //Third Quest for Kill Count
				{
					if ( lpObj->Class == lpSubInfo->NeedTargetMaxLevel) //READS Monster Class on TargetMax Level Category (for Original Quest.txt)
					{
						if(lpTarget->PartyNumber != -1)
						{
							int pindex;
							int dbnumber;
							for(int i=0;i<MAX_USER_IN_PARTY;++i)
							{
								gParty.GetIndexUser(lpTarget->PartyNumber, i, (int &)pindex, (int &)dbnumber);
								if(pindex != -1)
								{
									if ( this->GetQuestState(&gObj[pindex], lpQuestInfo->QuestIndex) == TRUE )
									{
										if( gObj[pindex].m_Quest[1] == 0xF6 && gObj[pindex].MapNumber == 41 )
										{
											if(lpObj->Class == 409 && gObj[pindex].m_Quest[30] < 20)
											{
												gObj[pindex].m_Quest[30] += 1;
												MsgOutput(pindex, "[Quest] (%d/20) Balram(Hero)", gObj[pindex].m_Quest[30]);
												return true;
											}

											if(lpObj->Class == 410 && gObj[pindex].m_Quest[31] < 20)
											{
												gObj[pindex].m_Quest[31] += 1;
												MsgOutput(pindex, "[Quest] (%d/20) Death Spirit(Hero)", gObj[pindex].m_Quest[31]);
												return true;
											}

											if(lpObj->Class == 411 && gObj[pindex].m_Quest[32] < 20)
											{
												gObj[pindex].m_Quest[32] += 1;
												MsgOutput(pindex, "[Quest] (%d/20) Soram(Hero).", gObj[pindex].m_Quest[32]);
												return true;
											}
										}
										if( gObj[pindex].m_Quest[1] == 0xDA && gObj[pindex].MapNumber == 42 )
										{
											if(lpObj->Class == 412 && gObj[pindex].m_Quest[34] < 1)
											{
												gObj[pindex].m_Quest[34] += 1;
												MsgOutput(pindex, "[Quest] (%d/1) Dark Elf(Hero)", gObj[pindex].m_Quest[34]);
												return true;
											}
										}
									}
						...
						else
						{
							if ( this->GetQuestState(lpTarget, lpQuestInfo->QuestIndex) == TRUE )
							{
								...
								{
									if(lpObj->Class == 409 && lpTarget->m_Quest[30] < 20)
									{
										lpTarget->m_Quest[30] += 1;
										MsgOutput(lpTarget->m_Index, "[Quest] (%d/20) Balram(Hero)", lpTarget->m_Quest[30]);
										return true;
									}

									if(lpObj->Class == 410 && lpTarget->m_Quest[31] < 20)
									{
										lpTarget->m_Quest[31] += 1;
										MsgOutput(lpTarget->m_Index, "[Quest] (%d/20) Death Spirit(Hero)", lpTarget->m_Quest[31]);
										return true;
									}

									if(lpObj->Class == 411 && lpTarget->m_Quest[32] < 20)
									{
										lpTarget->m_Quest[32] += 1;
										MsgOutput(lpTarget->m_Index, "[Quest] (%d/20) Soram(Hero).", lpTarget->m_Quest[32]);
										return true;
									}
								}
