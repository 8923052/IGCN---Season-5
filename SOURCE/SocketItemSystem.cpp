...
void CItemSocket::SetInfo(ITEM_SOCKET_INFO info)
{
	...
	{
		LogAddC( 2, "[SocketSystem] Number of Max Sockets exceeded %d %d ).", info.Index, MAX_SOCKET_OPTION );
		//CloseClient(aIndex);
	    return;
	}
...
void CItemSocket::ApplyEffect(int aIndex)
{
...
void CItemSocket::SeedExtract(int aIndex) //GOOD
{
	LPOBJ lpObj = &gObj[aIndex];
    int ChaosCount = 0;
	int HarmonyCount = 0;
...
void CItemSocket::SeedSphere(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
    int ChaosCount = 0;
	int CreationCount = 0;
...
void CItemSocket::SeedCalc(int aIndex,BYTE info)
{
	LPOBJ lpObj = &gObj[aIndex];
    int ChaosCount = 0;
	int CreationCount = 0;
	...
	BYTE option = this->CalcOption(SeedSphereType,SeedSphereLevel+1,SeedSphereOption);
...
void CItemSocket::NewMix4(int aIndex,BYTE info)
{
	LPOBJ lpObj = &gObj[aIndex];
    int ItemCount = 0;
	int ItemSlot = 0;
