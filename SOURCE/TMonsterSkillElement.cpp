...
void TMonsterSkillElement::ForceSkillElement(int iIndex, int iTargetIndex)
{
	...
	switch ( this->m_iElementType )
	{
		...
		case MSE_TYPE_NORMALATTACK:
			...
			break;
		default:
			LogAddC(2, "[TMonsterSkillElement] ForceSkillElement(): m_iElementType unknown value: %d", this->m_iElementType);
			break;
	}
}
...
BOOL TMonsterSkillElement::ApplyElementStun(int iIndex, int iTargetIndex)
{
	...
	BuffEffectC.EnableBuff(lpTargetObj->m_Index, 61, this->m_iContinuanceTime, 0, 0, 0);
...
BOOL TMonsterSkillElement::ApplyElementSummon(int iIndex, int iTargetIndex)
{
	...
	if(lpObj->Class == 459 && lpObj->Connected == PLAYER_PLAYING &&
...
BOOL TMonsterSkillElement::ApplyElementPush(int iIndex, int iTargetIndex)
{
	...
	if ( lpObj->Class == 459 && lpObj->Connected == 3 && lpObj->Class == 58 && iIndex == iTargetIndex )
	{
		return FALSE;
	}
...
BOOL TMonsterSkillElement::ApplyElementImmuneSkill(int iIndex, int iTargetIndex)
{
	...
	BuffEffectC.EnableBuff(lpTargetObj->m_Index, 63, this->m_iContinuanceTime, 0, 0, 0);
	return FALSE;
}
	...
	if ( lpTargetObj->m_SkillInfo.PoisonType == 0 )
	{
		if ( this->m_iNullifiedSkill  < 1 )
		{
			if ( retResistance(lpTargetObj, 1) == 0 )
			{
				...
				BuffEffectC.EnableBuff(lpTargetObj->m_Index, 55, this->m_iIncAndDecValue, 1, 0, 0);
			}
		}
		else
		{
