...
BOOL gObjMonsterRegen(LPOBJ lpObj)
{
	...
	/*
	else if ( lpObj->Class >= 386 && lpObj->Class <= 403 && 
		g_IllusionTemple.GetCurrentState(g_IllusionTemple.ReturnIndex(lpObj->MapNumber)) == IT_STATE_PLAYING)
	{
		lpObj->MTX = lpObj->X;
		lpObj->MTY = lpObj->Y;
		lpObj->TX = lpObj->X;
	...
	else if ( lpObj->Class >= 460 && lpObj->Class <= 462 )
	{
		if(g_Raklion.GetRaklionState() != 10)
		{
			return FALSE;
		}
	}
	...
	{
		if(g_Raklion.GetRaklionState() != 2 || g_Raklion.GetRaklionState() != 4)
		{
			return FALSE;
		}
	}
	else if ( (lpObj->Class >= 457 && lpObj->Class <= 458) && lpObj->MapNumber== 58 )
	{
		return FALSE;
	}
...
BOOL gObjMonsterMoveCheck(LPOBJ lpObj, int tx, int ty)
{
	...
	if ( lpObj->MapNumber == MAP_INDEX_KANTURU_BOSS && lpObj->m_PosNum < 0)
	{
		tx -= lpObj->StartX;
		ty -= lpObj->StartY;
		//float sqrtres = atof((char*)((tx*tx) + (ty*ty)));
		...
		case 1:
			...
			if (  BC_MAP_RANGE(lpObj->MapNumber) != FALSE && lpObj->Type >= OBJ_MONSTER)
			{
				if ( lpObj->Class == 89 || lpObj->Class == 95 || lpObj->Class == 112 || lpObj->Class == 118 || lpObj->Class == 124 || lpObj->Class == 130 || lpObj->Class == 143 || lpObj->Class == 433)
				{
					...
					{
						g_BloodCastle.m_BridgeData[7].m_iBC_BOSS_MONSTER_KILL_COUNT++;
					}
					else 
					{
				...
				{	if(lpObj->MapNumber == MAP_INDEX_BLOODCASTLE8)
					{
						g_BloodCastle.m_BridgeData[7].m_iBC_MONSTER_KILL_COUNT++;
					}
					else
					{
				...
				if ( g_BloodCastle.CheckMonsterKillCount(lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1) != false && lpObj->MapNumber != MAP_INDEX_BLOODCASTLE8 )
				{
				...
				else if ( g_BloodCastle.CheckMonsterKillCount(7) != false && lpObj->MapNumber == MAP_INDEX_BLOODCASTLE8)
				{
					if (g_BloodCastle.m_BridgeData[7].m_bBC_MONSTER_KILL_COMPLETE == false )
					{
						g_BloodCastle.m_BridgeData[7].m_bBC_MONSTER_KILL_COMPLETE = true;
						g_BloodCastle.m_BridgeData[7].m_iBC_MONSTER_MAX_COUNT = -1;
						...
						g_BloodCastle.SendBridgeAnyMsg( (UCHAR *)&pMsg, pMsg.h.size, 7);
						g_BloodCastle.ReleaseCastleBridge(7);
						g_BloodCastle.m_BridgeData[7].m_dwBC_TICK_DOOR_OPEN = GetTickCount() + 3000;

						LogAddTD("[Blood Castle] (%d) All of the Monster Terminated -> %d", 7+1,
							g_BloodCastle.m_BridgeData[7].m_iBC_MONSTER_KILL_COUNT);
					...
					if (g_BloodCastle.m_BridgeData[7].m_iBC_MONSTER_SUCCESS_MSG_COUNT < 1 )
					{
						g_BloodCastle.m_BridgeData[7].m_iBC_MONSTER_SUCCESS_MSG_COUNT++;
						g_BloodCastle.SendNoticeMessage(7, lMsg.Get(MSGGET(4, 144))); 
					}
				...
				if ( g_BloodCastle.m_BridgeData[7].m_bBC_MONSTER_KILL_COMPLETE != false )
				{
					if ( g_BloodCastle.CheckBossKillCount(7) != false )
					{
						if ( g_BloodCastle.m_BridgeData[7].m_bBC_BOSS_MONSTER_KILL_COMPLETE == false )
						{
							g_BloodCastle.m_BridgeData[7].m_bBC_BOSS_MONSTER_KILL_COMPLETE = true;
							g_BloodCastle.m_BridgeData[7].m_iBC_BOSS_MONSTER_MAX_COUNT = -1;

							g_BloodCastle.SetSaintStatue(7);

							LogAddTD("[Blood Castle] (%d) All of the Boss Monster Terminated -> %d",
								7+1, g_BloodCastle.m_BridgeData[7].m_iBC_BOSS_MONSTER_KILL_COUNT);
						}

						if ( g_BloodCastle.m_BridgeData[7].m_iBC_BOSS_MONSTER_SUCCESS_MSG_COUNT < 1 )
						{
							g_BloodCastle.m_BridgeData[7].m_iBC_BOSS_MONSTER_SUCCESS_MSG_COUNT++;
							g_BloodCastle.SendNoticeMessage(7, lMsg.Get(MSGGET(4, 156)));
						}
					}
				}
			}
		...
		case 56:
			{
				...
				if ( lpTargetObj->m_PoisonType == 0 )
				{
					if ( retResistance(lpTargetObj, 1) == 0 )
					{
						...
						BuffEffectC.EnableBuff(lpTargetObj->m_Index, 55, aMsgSubCode, 1, 0, 0);
					}
				}
			}
			break;
...
void gObjMonsterProcess(LPOBJ lpObj)
{
	...
	if ( RAKLION_EGG_CLASS_RANGE(lpObj->Class) )
	{
		return;
	}
...
void gObjMonsterAttack(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	{
		PMSG_MAGICATTACK pAttackMsg;
		pAttackMsg.MagicNumberH = SET_NUMBERH(rand()%6+1);
		pAttackMsg.MagicNumberL = SET_NUMBERL(rand()%6+1);
		pAttackMsg.NumberH = SET_NUMBERH(lpObj->TargetNumber);
		pAttackMsg.NumberL = SET_NUMBERL(lpObj->TargetNumber);
		pAttackMsg.Dis = 0;
	...
	{
		PMSG_MAGICATTACK pAttackMsg;
		pAttackMsg.MagicNumberH = SET_NUMBERH(rand()%2+1);
		pAttackMsg.MagicNumberL = SET_NUMBERL(rand()%2+1);
		pAttackMsg.NumberH = SET_NUMBERH(lpObj->TargetNumber);
		pAttackMsg.NumberL = SET_NUMBERL(lpObj->TargetNumber);
		pAttackMsg.Dis = 0;
...
#pragma message("Delete this unuseful Code with ADDON gObjMonsterAttack")
	...
	else if ( lpObj->Class >= 534 && lpObj->Class <= 539 )
	{
		...
		pAttackMsg.NumberH = SET_NUMBERH(lpObj->TargetNumber);
		pAttackMsg.NumberL = SET_NUMBERL(lpObj->TargetNumber);
		pAttackMsg.Dis = gObjCalDistance(lpObj, lpTargetObj);
		pAttackMsg.Dir = lpObj->Dir;
		pAttackMsg.X = lpTargetObj->X;
		pAttackMsg.Y = lpTargetObj->Y;
		CGDurationMagicRecv(&pAttackMsg, lpObj->m_Index);
		...
		if ( item_drop )
		{
			...
			else
			{
				MapC[lpObj->MapNumber].MonsterItemDrop(type, level, dur, x, y,
					Option1, Option2, Option3, NOption, 0, MaxHitUser, 0, 0, 0);
			}
		}
	}
}
...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ( lpObj->Class == 466) // Evil Goblin
	{
		dur = 10.0f;
		x = lpObj->X;
		y = lpObj->Y;
		...
		LogAddTD("[XMAS Event] Drop Santa Town Invitation");
		return;
	}
	...
	{
		if (g_RaklionSelupanItemDrop)
		{
			int ItemDropRate = rand()%100;

			if( ItemDropRate <= g_RaklionSelupanItemDropRate)
			{
				int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
	...
	if ( lpObj->Class == 508 && IMPERIAL_MAP_RANGE(lpObj->MapNumber) && lpObj->MapNumber != 72 ) // Destler
	{
		type = ItemGetNumberMake(14, 103);
		ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, type, 0, 0, 0, 0, 0, 0, 0, 0);
		return;
	}

	else if ( lpObj->Class == 509 && IMPERIAL_MAP_RANGE(lpObj->MapNumber) && lpObj->MapNumber != 72 ) // Vermont
	{
	...
	else if ( lpObj->Class == 510 && IMPERIAL_MAP_RANGE(lpObj->MapNumber) && lpObj->MapNumber != 72 ) // Kato
	{
	...
	else if ( lpObj->Class == 511 && IMPERIAL_MAP_RANGE(lpObj->MapNumber) && lpObj->MapNumber != 72 ) // Galia
	{
	...
	else if ( lpObj->Class == 507 && IMPERIAL_MAP_RANGE(lpObj->MapNumber) && lpObj->MapNumber != 72 ) // Erkanne
	{
	...
	else if ( lpObj->Class == 506 && IMPERIAL_MAP_RANGE(lpObj->MapNumber) && lpObj->MapNumber != 72 ) // Raymond
	{
	...
	if(lpObj->Class == 77)
	{
		PhoenixItemBagOpen(lpObj);
		LogAddTD("[Phoenix] Item Drop Request from ItemBag");
	}

	if(lpObj->Class == 309)
	{
		HellMainItemBagOpen(lpObj);
		LogAddTD("[Hell Main] Item Drop Request from ItemBag");
	}

	if(lpObj->Class == 59)
	{
		ZaikanItemBagOpen(lpObj);
		LogAddTD("[Zaikan] Item Drop Request from ItemBag");
	}

	if ( lpObj->Class == 529 || lpObj->Class == 530 )
	{
		if ( g_DoppelGanger.CreateSilverChest(lpObj) == true )
		{
			return;
		}
	}
	...
	{
		BYTE btIndex = g_IllusionTemple.ReturnIndex(lpObj->MapNumber);
		BYTE btKillPoints = 2;
		g_IllusionTemple.UpdateKillCount(btIndex,btKillPoints,lpTargetObj->m_Index,IT_KILLPOINT_ADD);
	}*/
	...
	ExtDropPer = rand()%gExcDropPer;	// Excellent Drop Percent #warning
	int ItemDropPer = gItemDropPer + lpTargetObj->m_iVipDropAdd;
	...
	if ( ExtDropPer == TRUE )
	{
		...
		else
		{
			...
			for ( int i=0;i<MAX_TYPE_PLAYER;i++)	// #error Delete the -1
			{
	...
	if ( item_drop )
	{
		...
		if ( gItemSocket.IsSeason4Item(DropItem->m_Type) == true )
		{
			if ( lpObj->MapNumber != MAP_INDEX_RAKLION && lpObj->MapNumber != MAP_INDEX_RAKLIONBOSS )
			{
				item_drop = FALSE;
			}
		}
	...
	if ( item_drop)
	{
		...
		{
			option1rand=gSkillDropPer;
			option2rand=gLuckDropPer;
			option3rand=rand()%100;
			optionc=rand()%3;
			NOption = 0;
		}
...
void CQeustNpcTeleport::Run(int aIndex)
{
	...
	if ( this->TimeCount > gQuestNPCTeleportTime )
	{
		...
		LogAdd("NPC %d Marlon location change", tableindex);	// #warning Translation Deathway
	}
}
