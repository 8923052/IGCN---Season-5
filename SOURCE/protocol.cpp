...
// void CGCloseWindow(int aIndex) Weird form of compilation
#include "stdafx.h"
#include "protocol.h"
#include "user.h"
#include "giocp.h"
...
#pragma pack(1)
struct PBMSG_MOVEREQ
{
    PBMSG_HEAD2 h;  // 8E:02, not just 8E
    int iCheckVal;
    WORD wMoveId;
};
#pragma pack()
...
//	MsgOutput(aIndex,"0x%02X,0x%02X,0x%02X",aRecv[2],aRecv[3],aRecv[4]);
	if ( protoNum == 0xF1 || protoNum == 0xF3 )
	{
		switch ( protoNum )
		{
			...
			case 0xF3:
				...
				//	MsgOutput(aIndex,"%02X %02X",aRecv[3],aRecv[4]);
					switch ( lpDef->subcode )
					{
						...
						case 0x30:
							...
							break;
						case 0x52:
							MasterLevelSystem.CGReqAddMasterPoint(aIndex, (PMSG_MASTER_SKILL_RECV *)aRecv);
							break;
						case 0x15:
							{
				struct PMSG_CHARCHECK
								...
								pMsg.bIsFromOtherAccount = FALSE;	// if i put 0x0e == 1 it pop's the selection screen
		...
		switch ( protoNum )
		{
			...
			case 0x03:{
				...
				gObjCalCharacter(aIndex);
					...
					g_SQuestSys.SendListChar(aIndex);
					  }
				break;
			case 0x0E:
				CGLiveClient((PMSG_CLIENTTIME *)aRecv, aIndex);
			...
			case ATTACK_PROTOCOL:
				{
				//LogAddC(2,"0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X",aRecv[0], aRecv[1],aRecv[2],aRecv[3],aRecv[4],aRecv[5],aRecv[6],aRecv[7]);
			...
			case 0x19:
				if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
				{
									//LogAddC(2,"0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X 0x%X",aRecv[0], aRecv[1],aRecv[2],aRecv[3],aRecv[4],aRecv[5],aRecv[6],aRecv[7],aRecv[8]);
			...
			case 0xBD:
				...
				break;
			case 0xBF: // Double Goer
				{
					PMSG_DEFAULT2 * lpDef = (PMSG_DEFAULT2 *)aRecv;
					LogAddC(2,"[DoubleGoer] Size: 0x%X, Subcode 0x%X other 0x%X 0x%X, 0x%X, 0x%X",aLen,lpDef->subcode,aRecv[4],aRecv[5],aRecv[6],aRecv[7]);
					switch (lpDef->subcode)
					{
						case 0x00:
								CGRequestEnterIllusionTemple((PMSG_IT_MOVE_MAP*)aRecv,aIndex);
								break;
						case 0x02:
							g_IllusionTemple.UseSkill(gObj[aIndex].m_cIllusionTempleIndex, (PMSG_REQ_USEILLUSIONTEMPLESKILL *)aRecv, aIndex);
								break;
						case 0x0E: // Click ENTER
							g_DoppelGanger.EnterEvent(&gObj[aIndex]);
							break;

						/*case 0x17:
							BYTE Packet[7] = {0xC1,0x07,0x17,0x00,0x00,0x00,0x00};
							DataSend(aIndex,Packet,7);
							break;*/
						case 0x17:
							{
			...
			case 0x8e:
				{
					PBMSG_MOVEREQ *pMsg = (PBMSG_MOVEREQ*)aRecv;
					gObjMoveGate(aIndex,gMoveCommand.m_MoveCommandData[pMsg->wMoveId].GateNumber);
					LogAddTD("[0x8E] Move Number %d ",pMsg->wMoveId);
				}
				break;
			case 0x90:
			...
			case 0xF6:
				{
					if(aRecv[3] == 0x30)
					{	
						//Request Available Quest List
						g_SQuestSys.SendListNpc(aIndex);
					}

					if(aRecv[3] == 0x0A)
					{		
						g_SQuestSys.Accept(aIndex,aRecv);
							//Start Quest
							//g_MUQuest.QuestAccept(aIndex,aRecv);
					}
					if(aRecv[3] == 0x0B)
					{	
						g_SQuestSys.Continue(aIndex,aRecv);
							//Quest Continue or start
						//g_MUQuest.QuestOnGoing(aIndex,aRecv);	
					}
					if(aRecv[3] == 0x1B)
					{
						int QuestIndex = (aRecv[4] + aRecv[5] * 256);
						g_SQuestSys.SendRequirments(aIndex,aRecv,true,QuestIndex);
					}
					if(aRecv[3] == 0x0D)
					{
						g_SQuestSys.Reward(aIndex,aRecv);
					}
					if(aRecv[3] == 0x31)
					{
						...
						BuffEffectC.EnableBuff(lpObj->m_Index, 3, GetTickCount(), (lpObj->Level / 5 +50), (lpObj->Level / 3 +45), 0);
					}
					LogAddC(2,"NEW PROTOCOL F6: %x %x %x %x",aRecv[3],aRecv[4],aRecv[5],aRecv[6]);
				}
				break;
			case 0xF7:
				{
			...
			case 0xF8:
				{
					...
					{
						case 0x01:
							PvPGens.CGReqJoinGens((PMSG_REQ_GENS_JOIN *)aRecv, &gObj[aIndex]);
							break;
						case 0x03:
							PvPGens.CGReqQuitGens(&gObj[aIndex]);
							break;
						case 0x09:
							PvPGens.CGReqGensReward((PMSG_REQ_GENS_REWARD *)aRecv, &gObj[aIndex]);
							break;
						case 0x0B:
...
void GCSendQuestInfo(int aIndex, int QuestIndex)
{
	...
	if( lpObj->m_Quest[30] > 20)
	{
		lpObj->m_Quest[30] = 0;
	}
	if( lpObj->m_Quest[31] > 20 )
	{
		lpObj->m_Quest[31] = 0;
	}
	if( lpObj->m_Quest[32] > 20 )
	{
		lpObj->m_Quest[32] = 0;
	}
...
void WereWolfQuarrelCheck(int aIndex)
{
	...
	if (gObj[aIndex].m_Quest[1] == 0xF6 || gObj[aIndex].m_Quest[1] == 0xDA || gObj[aIndex].m_Quest[1] == 0xEA) //Anti-Hack
	{
		int X = gObj[aIndex].X;
		int Y = gObj[aIndex].Y;
		if(X >= 59 && Y >= 237 && X <= 64 && Y <= 241) //My Range Anti-Hack
		{
			gObj[aIndex].Money -= 3000000; //3Kk de quem clico no 
			GCMoneySend(gObj[aIndex].m_Index, gObj[aIndex].Money);
			if(gObj[aIndex].PartyNumber != -1)
			{
				for(int i=OBJ_STARTUSERINDEX;i<OBJMAX;i++)
				{
					if(gObj[i].PartyNumber == gObj[aIndex].PartyNumber)
					{
						gObjMoveGate(i, 256);
					}
				}
		...
		{
			LogAddTD("[Anti-Hack][WereWolf Quarrel] [%s][%s] User out of range",gObj[aIndex].AccountID, gObj[aIndex].Name);
		}
	}
	else
	{
		LogAddTD("[Anti-Hack][WereWolf Quarrel] [%s][%s] User try to Enter Barracks without Season3 Quest State",gObj[aIndex].AccountID, gObj[aIndex].Name);
	}
...
void GateKeeperCheck(int aIndex)
{
	...
	if (gObj[aIndex].m_Quest[1] == 0xDA) //Anti-Hack
	{
		...
		if(X >= 119 && Y >= 166 && X <= 121 && Y <= 169) //My Range Anti-Hack
		{
			...
			{
				gObjMoveGate(aIndex, 257); //Refuge of Balgass Gate
			}
		}
		else
		{
			LogAddTD("[Anti-Hack][GateKeeper] [%s][%s] User out of range",gObj[aIndex].AccountID, gObj[aIndex].Name);
		}
	}
	else
	{
		LogAddTD("[Anti-Hack][GateKeeper] [%s][%s] User try to Enter Refugee without Season3 Quest State",gObj[aIndex].AccountID, gObj[aIndex].Name);
	}
}
...
void CGClientCloseMsg(PMSG_CLIENTCLOSE * lpMsg, int aIndex)
{
	/*if(gObj[aIndex].MapNumber == 79)
	{
		MsgOutput(aIndex,"Please go back to lorencia before leaving game");
		return;
	}*/
	switch ( lpMsg->Flag )
	{
...
void CGClientMsg(PMSG_CLIENTMSG* lpMsg, int aIndex) // here client send his self that user using hack wait let me test sth
{
	if ( lpMsg->Flag )
	{
...
void GCMagicListOneSend(int aIndex, char Pos, WORD type, BYTE level, WORD skill, BYTE btListType)
{
	...
	pList.Pos = Pos;
	memcpy(&pList.MagicInfo[DBM_TYPE], &type, sizeof(type));
...
void GCMagicListMultiSend(LPOBJ lpObj, BYTE btListType)
{
	...
	for ( int n=0;n<MAX_MAGIC;n++)
	{
		...
		{
			pList.Pos = n;
			memcpy(&pList.MagicInfo[DBM_TYPE], &lpObj->Magic[n].m_Skill, sizeof(lpObj->Magic[n].m_Skill));
...
void GCSkillKeyRecv(PMSG_SKILLKEY * lpMsg, int aIndex)
{
	...
	DGOptionDataSend(aIndex, gObj[aIndex].Name,
		...
		lpMsg->QkeyDefine,  lpMsg->WkeyDefine, lpMsg->EkeyDefine, lpMsg->RkeyDefine, lpMsg->ChatWindow, lpMsg->QWERLevelDefine);
...
void GCSkillKeySend(int aIndex, LPBYTE keybuffer, BYTE GO, BYTE Qk, BYTE Wk, BYTE Ek, BYTE Rk, BYTE ChatWnd, int QWERLevel)
{
	PMSG_SKILLKEY pMsg;
	...
	pMsg.QWERLevelDefine = QWERLevel;
	DataSend(aIndex, (UCHAR*)&pMsg, pMsg.h.size);
...
void CGItemGetRequest(PMSG_ITEMGETREQUEST * lpMsg, int aIndex)
{
	...
	{
		if ( lpItem->m_QuestItem != false && !(lpItem->m_Type >= ITEMGET(14,65) && lpItem->m_Type <= ITEMGET(14,67)))
		{
		...
		if ( lpItem->m_Type == ITEMGET(13,66) )	// Santa Invation
		{
			int count = gObjGetItemCountInIventory(aIndex, lpItem->m_Type/MAX_SUBTYPE_ITEMS,
				lpItem->m_Type % MAX_SUBTYPE_ITEMS, lpItem->m_Level);
			...
			if ( count > 0 )
			{
				LogAdd("[ Santa Invation ] User Already Have Invation [%s][%s] ( Name:%s, Count:%d )",
					gObj[aIndex].AccountID, gObj[aIndex].Name, lpItem->GetName(), count);
				...
				DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);	
				GCServerMsgStringSend("You can have only 1 Santa Invation in time", aIndex, 1);
	...
	/*	if ( lpItem->m_Type == ITEMGET(14,64) )	// TorchOfIllusion
		{
			int count = gObjGetItemCountInIventory(aIndex, lpItem->m_Type/MAX_SUBTYPE_ITEMS,
				lpItem->m_Type % MAX_SUBTYPE_ITEMS, lpItem->m_Level);
			...
			if ( count > 0 )
			{
				LogAdd("[IllusionTemple] You only can get one torch [%s][%s] ( Name:%s, Count:%d )",
					gObj[aIndex].AccountID, gObj[aIndex].Name, lpItem->GetName(), count);
			...
			else if( g_IllusionTemple.GetCurrentState(gObj[aIndex].m_cIllusionTempleIndex) != IT_STATE_PLAYING)
			{
				LogAdd("[IllusionTemple] You only can get torch inside of event [%s][%s] ( Name:%s, Count:%d )",
					gObj[aIndex].AccountID, gObj[aIndex].Name, lpItem->GetName(), count);
				...
				DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);	
				GCServerMsgStringSend(lMsg.Get(MSGGET(15, 104)), aIndex, 1);
			}
			else
			{
				g_IllusionTemple.SetHero(aIndex);
			}
		...
		else
		{
			...
			if ( lpItem->m_Type == ITEMGET(14,110) ) // Double Goar
			{
					int pos = gObjOverlapItemUsingDur((lpItem)?((CItem *)&lpItem->m_Number):NULL, map_num, item_num, aIndex, 10, ITEMGET(14,110), 0);
					if ( MAIN_INVENTORY_RANGE(pos) )
					{
						pResult.result = -3;
						...
						if ( gObj[aIndex].pInventory[pos].m_Durability >= 5.0f )
							{
								...
								LogAddTD("[DoubleGoer] [%s][%s] Make Dimension Magyon (Left Marker %d)",
									gObj[aIndex].AccountID, gObj[aIndex].Name, NewDur);
			...
			{
					int pos = gObjOverlapItemUsingDur((lpItem)?((CItem *)&lpItem->m_Number):NULL, map_num, item_num, aIndex, 10, ITEMGET(14,101), 0);
					if ( MAIN_INVENTORY_RANGE(pos) )
					{
						pResult.result = -3;
			...
			if ( pResult.result != 0xFF )
			{
				...
				lpItem->StoreSockets(&lpItem->m_ItemSocket[0]);
				if ( MapC[map_num].ItemGive(aIndex, item_num, false) == TRUE )
				{
					...
					else
					{
						...
						{
							if ( g_BloodCastle.CheckQuestItemSerial(gObj[aIndex].m_cBloodCastleIndex, lpItem) )
							{
								g_BloodCastle.m_BridgeData[gObj[aIndex].m_cBloodCastleIndex].m_iBC_QUEST_ITEM_USER_INDEX = gObj[aIndex].m_Index;
								char szTempMsg[128];
								...
								switch ( lpItem->m_Level )
								{
									case 0:	// Staff of Archangel
										...
										g_BloodCastle.SendNoticeMessage(gObj[aIndex].m_cBloodCastleIndex, szTempMsg);
										break;
								...
								LogAddTD("[Blood Castle] (%d) (Account:%s, Name:%s) picked up Angel King's Weapon (%d)",
									gObj[aIndex].m_cBloodCastleIndex+1, gObj[aIndex].AccountID, gObj[aIndex].Name, lpItem->m_Level);
							}
						}
						if( IT_MAP_RANGE(map_num) )
						{
							if( g_IllusionTemple.CheckSacredItemSerial(map_num-MAP_INDEX_ILLUSION_TEMPLE1, lpItem))
							{
								g_IllusionTemple.SetHero(map_num-MAP_INDEX_ILLUSION_TEMPLE1, aIndex);
								LogAddTD("[Illusion Temple] (%d) (Account:%s, Name:%s) is a current hero", map_num-MAP_INDEX_ILLUSION_TEMPLE1+1,
									gObj[aIndex].AccountID, gObj[aIndex].Name);
							}
						}
					}
...
BOOL CGItemDropRequest(PMSG_ITEMTHROW * lpMsg, int aIndex, BOOL drop_type)
{
	...
	if ( (lpObj->pInventory[lpMsg->Ipos].m_Type >= ITEMGET(13,0) && lpObj->pInventory[lpMsg->Ipos].m_Type <= ITEMGET(13,3))
		...
		/*|| lpObj->pInventory[lpMsg->Ipos].IsSocketItem() != FALSE*/)
	{
	...
	if ( pResult.Result == 1 )
	{
		...
		for(int i=0;i<5;i++)
		{
			SocketOpt[i] = lpObj->pInventory[lpMsg->Ipos].m_ItemSocket[i]; 
		}
		/*BYTE Socket2 = lpObj->pInventory[lpMsg->Ipos].m_ItemSocket[1];
		BYTE Socket3 = lpObj->pInventory[lpMsg->Ipos].m_ItemSocket[2];
		BYTE Socket4 = lpObj->pInventory[lpMsg->Ipos].m_ItemSocket[3];
		BYTE Socket5 = lpObj->pInventory[lpMsg->Ipos].m_ItemSocket[4];*/
		...
		if ( bIsItem == TRUE )
		{
			...
			{
				gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
				GameMasterBoxItemBagOpen(lpObj);
			...
			{
				gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
				GreenChaosBoxItemBagOpen(lpObj);
				LogAddTD("[%s][%s][%d]%d/%d Used green chaos Box Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
					lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
					serial, szItemName, type, level, Option1, Option2, Option3);
			}
			...
			{
				gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
				RedChaosBoxItemBagOpen(lpObj);
				LogAddTD("[%s][%s][%d]%d/%d Used red chaos Box Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
					lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
					serial, szItemName, type, level, Option1, Option2, Option3);
			}
			...
			{
				gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
				PurpleChaosBoxItemBagOpen(lpObj);
				LogAddTD("[%s][%s][%d]%d/%d Used Purple chaos Box Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
					lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
					serial, szItemName, type, level, Option1, Option2, Option3);
			}
			...
			else
			{
				...
				{
					if(type == gObjItemBags[b].ItemID)
					{
						gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
						exitembags[b]->DynamicItemDrop(aIndex);
						LogAddTD("[Custom ExBag System] %d Drop item",aIndex);
					}
				}
				...
				if ( MapC[map].ItemDrop( type, level, dur, lpMsg->px, lpMsg->py, Option1, Option2, Option3,
					NOption, SOption, serial2, OwnerIndex, PetLevel, PetExp, btItemEffectEx,&SocketOpt[0]) == TRUE )
				{
					gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
					pResult.Result = true;
					...
					type == ITEMGET(5,23) ))
					{
						if(ItemAttribute[type].SkillType != 0)
						{
							gObjMagicDel(lpObj, ItemAttribute[type].SkillType, 0);
...
void CGInventoryItemMove(PMSG_INVENTORYITEMMOVE * lpMsg, int aIndex)
{
	...
	if ( (lpMsg->sFlag == 0 && lpMsg->tFlag == 0) ||
		 ...
		 (lpMsg->sFlag >= 0x0B && lpMsg->sFlag <= 0x0E) ||
		 (lpMsg->tFlag >= 0x0B && lpMsg->tFlag <= 0x0E ) )
	{
		BOOL DurSSend;
		BOOL DurTSend;
		...
		if ( lpMsg->tFlag == 2 )
		{
			...
			{
				g_PropItems.SendPropertyInfo(&gObj[aIndex], gObj[aIndex].pWarehouse[target].m_Type, gObj[aIndex].pWarehouse[target].m_Number, target);
			}
		}
		else
		{
			...
			{
				g_PropItems.SendPropertyInfo(&gObj[aIndex], gObj[aIndex].pInventory[target].m_Type, gObj[aIndex].pInventory[target].m_Number, target);
			}
		}
...
void GCUserWarehouseSend(LPOBJ lpObj)
{
	...
	DataSend(lpObj->m_Index, SendByte, lOfs);//hmm
...
void CGBuyRequestRecv(PMSG_BUYREQUEST * lpMsg, int aIndex)
{
	...
	if ( ShopC[tShop].m_item[lpMsg->Pos].IsItem() == TRUE )
	{
		...
		else
		{
			...
			if ( bNoItem != FALSE )
			{
				if(lpObj->TargetShopNumber == 20)
				{
					if(lpMsg->Pos == 0)
					{
						ShopC[tShop].m_item[lpMsg->Pos].m_Type = 0*512+rand()%31;
						ShopC[tShop].m_item[lpMsg->Pos].m_Level = rand()%13;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option1 = rand()%1;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option2 = rand()%1;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option3 = rand()%7;
						ShopC[tShop].m_item[lpMsg->Pos].m_Part = 0;
				//		ShopC[tShop].m_item[lpMsg->Pos].m_ItemOptionEx  = rand()%8;
						
						pResult.Result = gObjShopBuyInventoryInsertItem(aIndex, ShopC[tShop].m_item[lpMsg->Pos]);
					}
					if(lpMsg->Pos == 2)
					{
						ShopC[tShop].m_item[lpMsg->Pos].m_Type = 5*512+rand()%34;
						ShopC[tShop].m_item[lpMsg->Pos].m_Level = rand()%13;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option1 = rand()%1;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option2 = rand()%1;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option3 = rand()%7;
					...
					if(lpMsg->Pos == 4)
					{
						ShopC[tShop].m_item[lpMsg->Pos].m_Type = 4*512+rand()%24;
						ShopC[tShop].m_item[lpMsg->Pos].m_Level = rand()%13;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option1 = rand()%1;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option2 = rand()%1;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option3 = rand()%7;
					...
					if(lpMsg->Pos == 6)
					{
						ShopC[tShop].m_item[lpMsg->Pos].m_Type = 5*512+rand()%24;
						ShopC[tShop].m_item[lpMsg->Pos].m_Level = rand()%13;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option1 = rand()%1;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option2 = rand()%1;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option3 = rand()%7;
					...
					if(lpMsg->Pos == 24)
					{
						ShopC[tShop].m_item[lpMsg->Pos].m_Type = 2*512+rand()%18;
						ShopC[tShop].m_item[lpMsg->Pos].m_Level = rand()%13;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option1 = rand()%1;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option2 = rand()%1;
						ShopC[tShop].m_item[lpMsg->Pos].m_Option3 = rand()%7;
...
void ItemDurRepaire(LPOBJ lpObj, CItem * DurItem, int pos, int RequestPos)
{
	...
	if ( itemtype >= ITEMGET(14,0) || ( itemtype >= ITEMGET(13,0) &&itemtype  < ITEMGET(13,4) ) ||itemtype == ITEMGET(13,10) || (itemtype >= ITEMGET(12,7) &&itemtype <= ITEMGET(12,35) ) || (itemtype >= ITEMGET(12,44) && itemtype < ITEMGET(13,0) )||itemtype ==ITEMGET(4,7) ||itemtype ==ITEMGET(4,15)  )
	{
		pResult.Money = 0;
		DataSend(lpObj->m_Index, (LPBYTE)&pResult, pResult.h.size);
	...
	else
	{
		...
		DurItem->Convert(DurItem->m_Type, DurItem->m_Option1, DurItem->m_Option2, DurItem->m_Option3,
			DurItem->m_NewOption, DurItem->m_SetOption,DurItem->m_ItemOptionEx, CURRENT_DB_VERSION, &DurItem->m_ItemSocket[0]);
...
void CGTradeRequestSend(PMSG_TRADE_REQUEST * lpMsg, int aIndex)
{
	...
	{
		if ( g_BloodCastle.GetCurrentState(gObj[aIndex].m_cBloodCastleIndex) != 1 || g_BloodCastle.CheckCanEnter(gObj[aIndex].m_cBloodCastleIndex) == false )
		{
	...
	{
		GCServerMsgStringSend("You cannot trade now", aIndex, 1);
		return;
	}
...
void CGPShopReqSetItemPrice(PMSG_REQ_PSHOP_SETITEMPRICE * lpMsg, int aIndex)
{
	...
	{
		LogAddTD("[PShop] [%s][%s] ERROR : Item Price Change - Item Block",gObj[aIndex].AccountID, gObj[aIndex].Name);	// Deathway fix Here - Last parameter can cause crash
...
void CGPartyRequestResultRecv(PMSG_PARTYREQUESTRESULT * lpMsg, int aIndex)
{
	...
	{
		if(g_IllusionTemple.m_IllusionTempleData[g_IllusionTemple.ReturnIndex(gObj[aIndex].MapNumber)].m_bCanParty == false)
		{
			GCServerMsgStringSend(lMsg.Get(MSGGET(15,103)), aIndex, 1);
			result = false;
			::GCResultSend(number, 0x41, 0x00);
		}
	}*/
...
void CGChaosBoxItemMixButtonClick(PMSG_CHAOSMIX* aRecv, int aIndex)
{
	...
	//		break;
		...
		case SEED_TYPE_SEEDEXTRACT:
			gItemSocket.SeedExtract(lpObj->m_Index);
			break;
		case SEED_TYPE_SEEDSPHERE:
			gItemSocket.SeedSphere(lpObj->m_Index);
			break;
		case SEED_TYPE_SEEDCALC:
			gItemSocket.SeedCalc(lpObj->m_Index, aRecv->Info);
			break;
		case SEED_TYPE_NEWMIX2:
			gItemSocket.NewMix4(lpObj->m_Index, aRecv->Info);
			break;
	...
	if ( BC_MAP_RANGE(lpObj->MapNumber) && lpObj->Type == OBJ_USER )
	{
		switch ( g_BloodCastle.GetCurrentState(lpObj->MapNumber-11) )
		{
			...
			case 0x02:
				...
				if ( lpObj->TY > 76 &&  g_BloodCastle.m_BridgeData[lpObj->m_cBloodCastleIndex].m_bCASTLE_DOOR_LIVE )
				{
					lpObj->m_Rest = 1;
					lpObj->PathCur = 0;
					lpObj->PathCount = 0;
...
void GCDamageSend(int aIndex, int TargetIndex, int AttackDamage, int MSBFlag, int MSBDamage, int iShieldDamage)
{
	...
	{
		cManager.GMDataSend((LPBYTE)&pResult, pResult.h.size);
	}
...
struct PMSG_ML_EXPSEND
{

};

void GCMasterExpSend(int aIndex, __int64 exp)
{
	BYTE szMLExp[0x0C] = {0xC3,0x0C ,0x9C ,0xFF ,0xFF , 0,0,0,0,0,0,0 } ;

	//szMLExp[5] = 0 ;
	//43845615 29D07EF
	//C3 0C 9C FF FF 00 9D 02 EF 07 00 00
	//i64Exp = 43845615 ;  // 
	*(WORD *)&szMLExp[6]  = (WORD)(exp>>16) ;
	*(WORD *)&szMLExp[8]  = (WORD)(exp) ;
	//szMLExp[0x0A] = 0  ;
	//szMLExp[0x0B] = 0  ;

	DataSend ( aIndex, szMLExp ,sizeof(szMLExp)) ; 
}
...
void GCUseEffectItem(LPOBJ lpObj, int iEffectUseOption, int iOptionType, int iEffectType, int iLeftTime, BYTE BuffNumber)
{
...
void GCStateInfoSend(LPOBJ lpObj, BYTE state, int Effect)
{
	PMSG_VIEWSKILLSTATE pMsg;

	PHeadSetB((LPBYTE)&pMsg, 0x07, sizeof(pMsg));
	pMsg.State = state;
	pMsg.SkillIndex = static_cast<BYTE>(Effect);
	pMsg.NumberH = SET_NUMBERH(lpObj->m_Index);
	pMsg.NumberL = SET_NUMBERL(lpObj->m_Index);
...
void CGTeleportRecv(PMSG_TELEPORT* lpMsg, int aIndex)
{
	...
	if ( gObj[aIndex].m_iSkillStunTime > 0 )
		return;
	if ( gObj[aIndex].m_SummonerSkillSleepTime > 0 )
		return;
...
/*void CGTeleportRecv(PMSG_TELEPORT* lpMsg, int aIndex)
{
	...
	if ( lpMsg->MoveNumber == 0 )
	{
		...
		if ( gObjCheckTeleportArea(aIndex, x, y) == FALSE )
		{
			...
			pTeleportResult.h.size = sizeof(pTeleportResult);
			pTeleportResult.h.headcode = 0x1C;
			pTeleportResult.MoveNumberH = 0;
			pTeleportResult.MoveNumberL = 0;
			pTeleportResult.MapNumber = gObj[aIndex].MapNumber;
			pTeleportResult.MapX = gObj[aIndex].X;
			pTeleportResult.MapY = gObj[aIndex].Y;
			pTeleportResult.Dir = gObj[aIndex].Dir;
//			pTeleportResult.UNk0 = 0x00;
...
/*	PMSG_TELEPORT_RESULT pMsg;
	...
	pMsg.MoveNumberH = SET_NUMBERH(MoveNumber);
	pMsg.MoveNumberL = SET_NUMBERL(MoveNumber);
	pMsg.MapNumber = MapNumber;
	pMsg.MapX = MapX;
	pMsg.MapY = MapY;
	pMsg.Dir = Dir;
...
void CGBeattackRecv(unsigned char* lpRecv, int aIndex, int magic_send)
{
	...
	int tNumber;
	PMSG_BEATTACK * lpMsg;
	int magicnumber = MAKE_NUMBERW(lpCount->MagicNumberH,lpCount->MagicNumberL);
	if( magicnumber == 0)
	{
		magicnumber = 235;
	}
	...
	if ( gObj[aIndex].Type == OBJ_USER )
	{
		lpMagic = gObjGetMagicSearch(&gObj[aIndex],magicnumber );//MAKE_NUMBERW(lpCount->MagicNumberH,lpCount->MagicNumberL));
	}
	else
	{
		lpMagic = gObjGetMagic(&gObj[aIndex], MAKE_NUMBERW(lpCount->MagicNumberH,lpCount->MagicNumberL));
	}
	...
	for (int n=0;n<lpCount->Count;n++)
	{
		...
		LPOBJ lpObj = &gObj[aIndex];
		LPOBJ lpTargetObjT = &gObj[tNumber];
...
UNK3 = UNK4*/ 
	...
	if ( !gObjUseSkill.EnableSkill(lpMagic->m_Skill) && lpMagic->m_Skill < 300 )
	{
		return;
	}
...
void CGUseItemRecv(PMSG_USEITEM* lpMsg, int aIndex)
{
	...
	if ( citem->IsItem() )
	{
		...
		if ( citem->m_Type == ITEMGET(13,66))
		{
			if(citem->m_Durability > 0)
			{
				citem->m_Durability -= 1;
			//	gObjMoveGate(aIndex,294);
				GCItemDurSend(aIndex,pos,citem->m_Durability,0);
				gObjTeleport(aIndex,62,205,25);
				LogAddTD("[%s][%s] Teleport to Santa Town",gObj[aIndex].AccountID,gObj[aIndex].Name);
			}
			else 
				{
					GCInventoryItemDeleteSend(aIndex,pos,1);
					gObjDeleteItemsCount(aIndex,citem->m_Type,0,1);
					MsgOutput(aIndex,"Your santa invation is expired");
				}
			return;
		}
		...
		else if ( citem->m_Type == ITEMGET(14,145) ) // Jewel Of Luck
		{
			if ( gObjItemLuckUp(&gObj[aIndex], lpMsg->inventoryPos, lpMsg->invenrotyTarget) == TRUE )
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				gObj[aIndex].pInventory[pos].Clear();
				GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
				  ...
				  (citem->m_Type >= ITEMGET(12,44) && citem->m_Type <= ITEMGET(12,48)) )	// S4 Orbs
		{
			...
			else
			{
				...
				if(citem->m_Type >= ITEMGET(15,19) && citem->m_Type <= ITEMGET(15,29) || citem->m_Type == ITEMGET(12,48))
				{
					switch(citem->m_Type)
					{
					case ITEMGET(15,19):
						addskill = gObjMagicAdd(&gObj[aIndex],215,0);
						skillnumber = 215;
						break;
					case ITEMGET(15,20):
						addskill = gObjMagicAdd(&gObj[aIndex],214,0);
						skillnumber = 214;
						break;
					case ITEMGET(15,21):
						addskill = gObjMagicAdd(&gObj[aIndex],230,0);
						skillnumber = 230;
						break;
					case ITEMGET(15,22):
						addskill = gObjMagicAdd(&gObj[aIndex],217,0);
						skillnumber = 217;
						break;
					case ITEMGET(15,23):
						addskill = gObjMagicAdd(&gObj[aIndex],218,0);
						skillnumber = 218;
						break;
					case ITEMGET(15,24):
						addskill = gObjMagicAdd(&gObj[aIndex],219,0);
						skillnumber = 219;
						break;
					case ITEMGET(15,25):
						addskill = gObjMagicAdd(&gObj[aIndex],220,0);
						skillnumber = 220;
					case ITEMGET(15,26):
						addskill = gObjMagicAdd(&gObj[aIndex],221,0);
						skillnumber = 221;
						break;
					case ITEMGET(15,27):
						addskill = gObjMagicAdd(&gObj[aIndex],222,0);
						skillnumber = 222;
						break;
					case ITEMGET(15,28):
						addskill = gObjMagicAdd(&gObj[aIndex],233,0);
						skillnumber = 233;
						break;
					case ITEMGET(15,29):
						addskill = gObjMagicAdd(&gObj[aIndex],237,0);
						skillnumber = 237;
						break;
					case ITEMGET(12,48):
						addskill = gObjMagicAdd(&gObj[aIndex],238,0);
						skillnumber = 238;
				...
				{
					addskill = gObjMagicAdd(&gObj[aIndex], citem->m_Type>>(9), citem->m_Type %  MAX_SUBTYPE_ITEMS, citem->m_Level, skillnumber);
				}
				if ( addskill >= 0 )
				{
					gObjInventoryItemSet(aIndex, pos, -1);
		...
		else if ( citem->m_Type == ITEMGET(14,10) ) // Town Portal Scroll
		{
			...
			{
				if ( g_BloodCastle.GetCurrentState(gObj[aIndex].m_cBloodCastleIndex) == 2 )
				{
		...
		else if ( citem->m_Type == ITEMGET(14,63) ) // 
		{
			int x = gObj[aIndex].X;
	        int y = gObj[aIndex].Y;
			...
			for ( int i=0;i<15;i++)
			{
				pMsg.X = x+(rand() % 2)*1 - 2;
				pMsg.Y = y+(rand() % 2)*1 - 2;
				MsgSendV2(&gObj[aIndex],(UCHAR*)&pMsg, sizeof(pMsg));
				::DataSend(aIndex ,(UCHAR*)&pMsg, sizeof(pMsg));
			}
		...
		case 5:
			{
				...
				if ( g_IllusionTemple.GetCurrentState(lpMsg->btItemLevel-1) == 1 )
				{
					if ( g_IllusionTemple.CheckCanEnter(lpMsg->btItemLevel-1) != false )
					{
						pResult.RemainTime = 0;
					}
					else
					{
						pResult.RemainTime = g_IllusionTemple.GetRemainTime(lpMsg->btItemLevel-1);
					}
				}
				else
				{
...
void CGRequestEnterChaosCastle(PMSG_REQ_MOVECHAOSCASTLE* lpMsg, int iIndex)
{
	...
	if ( g_DuelManager.DuelCheck(&gObj[iIndex]) == TRUE )
	{
		PMSG_NOTICE pNotice;
...
void CGRequestPetItemInfo(PMSG_REQUEST_PET_ITEMINFO * lpMsg, int aIndex)
{
	...
	if ( lpMsg->InvenType == 0 )	// Inventory
	{
		...
		if ( lpObj->pInventory[lpMsg->nPos].IsItem() )
		{
			...
			pMsg.Dur = static_cast<unsigned char>(lpObj->pInventory[lpMsg->nPos].m_Durability);
...
void GCAnsCsAccessCrownState(int iIndex, BYTE btCrownState)
{
	...
	if(btCrownState == FALSE)
	{
		...
		LogAddTD("[CastleSiege] [Reg. Attempt] Accumulated Crown AccessTime : %d [%s](%s)(%s)",gObj[iIndex].m_iAccumulatedCrownAccessTime,
				gObj[iIndex].GuildName,gObj[iIndex].AccountID,gObj[iIndex].Name);
	}

	if(btCrownState == TRUE)
	{
		LogAddTD("[CastleSiege] [Reg. Success] Accumulated Crown AccessTime : %d [%s](%s)(%s)",gObj[iIndex].m_iAccumulatedCrownAccessTime,
				gObj[iIndex].GuildName,gObj[iIndex].AccountID,gObj[iIndex].Name);
...
struct PMSG_RESULT_MOVEILLUSIONTEMPLE
{
	PBMSG_HEAD2 h;
	BYTE Result;	
};

void CGRequestEnterIllusionTemple(PMSG_IT_MOVE_MAP* lpMsg,int aIndex)
{
	PMSG_ILLUSION_ENTER_RESULT pResult;
	...
	if(gObj[aIndex].m_IfState.use == 1 && gObj[aIndex].m_IfState.type != 12)
	{
		return;
	}
	if(BC_MAP_RANGE(gObj[aIndex].MapNumber) || DS_MAP_RANGE(gObj[aIndex].MapNumber) 
		|| CC_MAP_RANGE(gObj[aIndex].MapNumber) || IT_MAP_RANGE(gObj[aIndex].MapNumber) 
		|| DG_MAP_RANGE(gObj[aIndex].MapNumber) || IMPERIAL_MAP_RANGE(gObj[aIndex].MapNumber))
	{
		return;
	}
	if(gObj[aIndex].m_PK_Level >= 4)
	{
		GCServerMsgStringSend("Killers cannot enter to Illusion Temple", aIndex, 1);
		return;
	}
	
	if(g_DuelManager.DuelCheck(&gObj[aIndex]))
	{
		return;
	}
	if(gObj[aIndex].lpGuild)
	{
		if(gObj[aIndex].lpGuild->WarState != 0)
		{
			return;
		}
	}
	BYTE IllusionTempleLevel = lpMsg->btIllusionIndex-1;
	BYTE InvitationPos = lpMsg->btIllusionTicketPos;
	BYTE InvitationLevel;
	int InvitationSerial;
	if(IllusionTempleLevel < 0)
	{
		return;
	}
	if(InvitationPos < 0)
	...
	if(g_IllusionTemple.GetUserLevelRoom(aIndex) != IllusionTempleLevel)
	{
		pResult.Result = 1;
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		GCServerMsgStringSend("Level mismatch", aIndex, 1);
		return;
	}
	if(gObj[aIndex].pInventory[InvitationPos].IsItem() == true)
	{
		if(gObj[aIndex].pInventory[InvitationPos].m_Type == ITEMGET(13,51)) // Scroll of Blood
		{
			InvitationLevel = gObj[aIndex].pInventory[InvitationPos].m_Level;
			InvitationSerial = gObj[aIndex].pInventory[InvitationPos].m_serial;
		}
		else
		{
			pResult.Result = 1;
	...
	if(InvitationLevel-1 != IllusionTempleLevel)
	{
		pResult.Result = 1;
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
	}
	if(!g_IllusionTemple.CheckCanEnter(aIndex, IllusionTempleLevel))
	{
		pResult.Result = 2;
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		GCServerMsgStringSend("Temple is closed!", aIndex, 1);
		return;
	}
	pResult.Result = 0;
	...
	DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
	gObj[aIndex].m_cIllusionTempleIndex = IllusionTempleLevel;
	gObj[aIndex].m_IfState.use = 0;
	gObj[aIndex].m_IfState.type = 0;
	gObj[aIndex].m_IfState.state = 0;
	if(gObj[aIndex].m_bPShopOpen == true)
	...
	if(gObj[aIndex].PartyNumber != -1)
	{
		PMSG_PARTYDELUSER pParty;
		int pIndex = gParty.GetIndex(gObj[aIndex].PartyNumber, aIndex, gObj[aIndex].DBNumber);
		if(pIndex >= 0)
		{
			pParty.Number = pIndex;
			CGPartyDelUser(&pParty, aIndex);
		}
	}
	if(gObj[aIndex].Class == CLASS_ELF && gObj[aIndex].m_RecallMon >= 0)
	{
		GCRecallMonLife(aIndex, 60, 0);
		gObjMonsterCallKill(aIndex);
	}
	switch(IllusionTempleLevel)
	{
		case 0:
			{
	...
	g_IllusionTemple.EnterToWaitingRoom(aIndex, IllusionTempleLevel);
	LogAddTD("[Illusion Temple] (%d) (Account:%s, Name:%s) Entered Illusion Temple (Scroll of Blood Serial:%d)",
		IllusionTempleLevel+1, gObj[aIndex].AccountID, gObj[aIndex].Name, InvitationSerial);
}
void GCMasterLevelUpMsgSend(int aIndex, int iSendEffect)
{
	if ( !OBJMAX_RANGE(aIndex) )
		return;
	...
	pMsg.MasterLevel = gObj[aIndex].m_uMasterLevel;
...
void CGReqIllusionTempleEnter(PMSG_REQ_MOVEILLUSIONTEMPLE * aRecv, int aIndex)
{
	PMSG_ILLUSION_ENTER_RESULT pResult;
	PHeadSubSetB((LPBYTE)&pResult, 0xBF, 0x00, sizeof(pResult));
	if(OBJMAX_RANGE(aIndex) == false)
	...
	if(gObj[aIndex].m_PK_Level >= 4)
	{
		GCServerMsgStringSend("PK is not allowed to enter event", aIndex, 1);
		return;
	}
	...
	BYTE IllusionTempleLevel = aRecv->ID-1;
	BYTE InvitationPos = aRecv->iItemPos;
	BYTE InvitationLevel;
	int InvitationSerial;
	if(IllusionTempleLevel < 0)
	{
