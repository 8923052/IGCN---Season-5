...
struct PMSG_ITEMVIEWPORTCREATE {
  ...
  /*<thisrel this+0x4>*/ /*|0x7|*/ unsigned char ItemInfo[12];
...
void gObjInit()
{
	...
	for ( int n = 0; n< OBJMAX ; n++)
	{
		...
		gObj[n].lpEffect = new CEffect[MAX_PLAYER_BUFF];
...
//	lpObj->NPggCSAuth.Init();
	...
	for ( i=0;i<MAX_PLAYER_BUFF ; i++)
	{
		lpObj->lpEffect[i].Clear();
	}
	...
	lpObj->m_uMasterLevel = 1;
	lpObj->m_uMasterPoints = 0;
	lpObj->m_uMasterExp = 0;
	lpObj->m_uMasterNextExp = 0;
	...
	lpObj->m_cIllusionTempleSubIndex = -1;
	lpObj->m_btSkillRestraintOrder = FALSE;
	lpObj->m_btSkillRestraintOrderTime = 0;
	lpObj->m_btSkillShieldOrder = FALSE;
	lpObj->m_btSkillShieldOrderTime = 0;
	lpObj->m_btWareHouseId = 0;
	lpObj->m_iIllusionFreezeTime = 0;
	lpObj->m_iIllusionShieldTime = 0;
	lpObj->m_iIllusionTempleExp = 0;
	lpObj->m_cIllusionTempleIndex = 0;
	lpObj->m_cIllusionTempleSubIndex = 0;
	lpObj->m_cIllusionTempleTeam = 0;
	lpObj->m_iResets = 0;
	gItemSocket.InitEffect(&lpObj->ItemSocketEffect);
	memset(&lpObj->SupportQuestInfo,0,sizeof(lpObj->SupportQuestInfo));
	...
	for(n = 0; n < INVENTORY_SIZE;n++)
	{
		...
		if(itype != -1)
		{
			...
			/*if(ItemDbByte >= 10)
			{
				//if(_type == ITEMGET(13,3))
				//{
					item.m_Option3 |= DBI_GET_DINO_OPTION(lpMsg->dbInventory[n*ItemDbByte+DBI_NOPTION_DATA]) >> 4;
				//}
				//else
				//{
			/*	if	((lpMsg->dbInventory[n*ItemDbByte+DBI_NOPTION_DATA] & 0x40)== 0x40)
			...
			{
				item.Convert(itype,item.m_Option1,item.m_Option2,item.m_Option3,DBI_GET_NOPTION(lpMsg->dbInventory[n*ItemDbByte+DBI_NOPTION_DATA]),lpMsg->dbInventory[n*ItemDbByte+DBI_SOPTION_DATA],item.m_ItemOptionEx,1,0);
			}
			else
			{
				item.Convert(itype,item.m_Option1,item.m_Option2,item.m_Option3,DBI_GET_NOPTION(lpMsg->dbInventory[n*ItemDbByte+DBI_NOPTION_DATA]),lpMsg->dbInventory[n*ItemDbByte+DBI_SOPTION_DATA],item.m_ItemOptionEx,3,&item.m_ItemSocket[0]);
			}
...
#if(GS_CASTLE)//Revisar a quantidade de itens que cada um deve ter...
...
BOOL gObjSetCharacter(LPBYTE lpdata, int aIndex)
{
	...
	lpObj->ChangeUP2 = (lpMsg->Class & 0x07 & 0x02) >> 1; // Master Class Update
	...
	lpObj->m_cIllusionTempleIndex = 0;
	lpObj->m_cIllusionTempleSubIndex = 0;
	if(lpObj->MapNumber == MAP_INDEX_SANTATOWN || lpObj->MapNumber == MAP_INDEX_DUELROOM)
	{
		lpObj->MapNumber = MAP_INDEX_DEVIAS;
		MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber,lpObj->X,lpObj->Y);
	}
	...
	if ( lpObj->MapNumber == MAP_INDEX_RAKLIONBOSS )
	{
		int iRaklionState = g_Raklion.GetRaklionState();
		
		if ( iRaklionState >= 7 && iRaklionState <= 10 )
		{
			gt = 286;
		}
	}
	...
	{
		RAKLION_UTIL.NotifyRaklionCurrentState(lpObj->m_Index, g_Raklion.GetRaklionState(), g_Raklion.GetRaklionDetailState());
	}
...
//	lpObj->MaxMana /= 10.0f;
	...
	lpObj->m_uMasterLevel = lpMsg->MLevel;
	lpObj->m_uMasterExp = lpMsg->MLExp;
	lpObj->m_uMasterPoints = lpMsg->MLPoint;
	lpObj->m_iResets = lpMsg->resets;
	lpObj->m_uMasterNextExp = MasterLevelSystem.NextExpCal(lpObj);
	if ( lpObj->Mana < 1.0f )
	{
		lpObj->Mana = 1.0f;
	}
		...
		if ( lpObj->Class == 0 || lpObj->Class == 2 || lpObj->Class == 1 || lpObj->Class == 4 || lpObj->Class == 3 || lpObj->Class == 5 )
	{
		...
		{
			memcpy(&itype, &lpMsg->dbMagicList[n*3], 2);
			if ( itype != (WORD)-1 )
			{
				/*if(itype > 300)
				{
					if(MasterLevelSystem.IsMasterSkill(itype))
					{
						gObjMagicAdd(lpObj, itype, lpMsg->dbMagicList[n*3+DBM_LEVEL]&0xF);
						CMagicInf * Magic = gObjGetMagicSearch(lpObj, itype);
						MasterLevelSystem.SetEffect(lpObj->m_Index, Magic);
						continue;
					}
				}*/
				if ( itype == 17 )
				...
				if ( itype >= 300)// && itype <= 384 )
				{
					gObjMagicAdd(lpObj, itype, lpMsg->dbMagicList[n*3+DBM_LEVEL]&0xF);
					CMagicInf * Magic = gObjGetMagicSearch(lpObj, itype);
					MasterLevelSystem.SetEffect(lpObj->m_Index, Magic);
				}
				else if ( itype != 18 && itype != 19 && itype != 20 && itype != 21 && itype != 22 && itype != 23 && itype != 24 && itype != 25 && itype != 49 && itype != 56 && itype != 62 && itype != 76 && itype != 54 && itype != 223 && itype != 224 && itype != 225 )
				{
		...
		gObjMagicDel(lpObj,210,0);
		gObjMagicDel(lpObj,211,0);
		gObjMagicDel(lpObj,212,0);
		gObjMagicDel(lpObj,213,0);
	...
	{
		lpObj->Authority = 2;
		BuffEffectC.EnableBuff(lpObj->m_Index, 28, -1, 0, 0, 0);
...
BOOL gObjSetMonster(int aIndex, int MonsterClass)
{
	...
	if ( MonsterClass >= 204 && MonsterClass <= 259
		...
		|| MonsterClass == 367
		|| MonsterClass == 375 || MonsterClass == 407 || MonsterClass == 408 || MonsterClass == 465 
		|| MonsterClass == 467 || MonsterClass == 468 || MonsterClass == 469 || MonsterClass == 470 || MonsterClass == 379 ||
		...
		MonsterClass == 543 || MonsterClass == 544 || MonsterClass == 385 || MonsterClass == 380 || MonsterClass == 383 || MonsterClass == 384 || MonsterClass == 541 || MonsterClass == 542 || MonsterClass == 546 || MonsterClass == 545 ||  MonsterClass == 547)
	{
		lpObj->Type = OBJ_NPC;
	}
	else
...
//		MsgBox(lMsg.Get(MSGGET(1, 253) ), lpObj->Class, __FILE__, __LINE__);
	...
	if ( lpObj->m_AttackType != 0 )
	{
		...
		if ( lpObj->Class == 535 )
		{
			gObjMonsterMagicAdd(lpObj, AT_SKILL_WHEEL, 1);
		}

		if ( lpObj->Class == 536 )
		{
			gObjMonsterMagicAdd(lpObj, AT_SKILL_EXPICE, 1);
		}

		if ( lpObj->Class == 537 )
		{
			gObjMonsterMagicAdd(lpObj, AT_SKILL_SWORD6, 1);
		}
		
		if ( lpObj->Class == 538 )
		{
			gObjMonsterMagicAdd(lpObj, AT_SKILL_DARKHORSE_ATTACK, 1);
		}

		if ( lpObj->Class == 539 )
		{
			gObjMonsterMagicAdd(lpObj, AT_SKILL_RED_STORM, 1);
		}
...
BOOL gObjGameClose(int aIndex)
{
	...
	if ( lpObj->Type != OBJ_USER )
	{
		LogAddTD("%s %s not user type", lpObj->AccountID, lpObj->Name);
		return FALSE;
	}
	...
	{
		g_DuelManager.RoomReset(lpObj->m_iDuelRoom);
	}
	...
	/*if ( lpObj->m_iGensInfoLoad == true )
	{
		GDReqGensSave(lpObj->m_Index);
	}

	if( lpObj->m_iInGameShopInfoLoad == true )
	{
	...
	if ( IT_MAP_RANGE(lpObj->MapNumber) && lpObj->m_cIllusionTempleIndex >= 0 )
	{
		if(g_IllusionTemple.GetHeroIndex(lpObj->m_cIllusionTempleIndex) == lpObj->m_Index)
		{
			g_IllusionTemple.DropSacredItemFromHero(lpObj->m_cIllusionTempleIndex);
		}
	}
	...
	gObjViewportClose(lpObj);
	LogAddTD("%d Game Close %s %s", lpObj->m_Index, lpObj->AccountID, lpObj->Name);
	...
	switch ( aMsgCode )
	{
		...
		case 59:
			if ( lpObj->Live )
			{
				...
				{
					//lpObj->m_OldX
					gObjUseSkill.NewSkillProc(lpObj->m_Index, lpMagic, lpObj->m_iSkillPollusionX, lpObj->m_iSkillPollusionY, aIndex, SubCode+1);
				}
			}
			break;
...
BOOL gObjBackSpring(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ( lpObj->Class == 277 || lpObj->Class == 283 || lpObj->Class == 288 || lpObj->Class == 278
		|| lpObj->Class == 215 || lpObj->Class == 216 || lpObj->Class == 217 || lpObj->Class == 218 ||
		lpObj->Class == 219 )
	{
		return FALSE;
	}
	...
	{
		if(g_CrywolfNPC_Altar.GetAltarUserIndex(i) == lpObj->m_Index)
		{
			return FALSE;
		}
	}
...
bool gObjMasterLevelUp(LPOBJ lpObj, int addexp, int iMonsterIndex)
{
	LPOBJ lpTargetObj = &gObj[lpObj->TargetNumber];

	LogAddTD("[Master Level] Experience gain: [%s][%s] Map[%d]-(%d,%d) Monster Type: %d",lpObj->AccountID,lpObj->Name,lpObj->MapNumber,lpObj->X,lpObj->Y,lpTargetObj->Class);

	if(lpObj->ChangeUP != 2)
		return false;
	
	if(lpObj->m_uMasterLevel >= 200)
	{
		lpObj->m_uMasterLevel = 200;
		MsgOutput(lpObj->m_Index,"You have reached Max Master Level");
		return false;
	}
	...
	MsgOutput(lpObj->m_Index,"[ML LevelUp] You mastered Level: %d",lpObj->m_uMasterLevel);
	lpObj->m_uMasterPoints += 1;

	//GCMasterExpSend(lpObj->m_Index,exp);
//	GCMasterLevelUpSend(lpObj,1);
	gObjNextMasterExpCal(lpObj);
	return true;
}
...
{
	if(MasterLevelSystem.CanGainMasterExp(lpObj))
	{
		return MasterLevelSystem.LevelUp(lpObj, addexp, iMonsterType, iEventType);
	}
	...
	if ( lpObj->Level >= MAX_CHAR_LEVEL && lpObj->Experience >= gLevelExperience[-1+lpObj->Level] )
	{
		...
		//GCServerMsgStringSend("You have reached max level", lpObj->m_Index, 1);
		return false;
	}
	...
	if ( lpObj->Class == 4 )
	{
		lpObj->LevelUpPoint += gLevelUpPointMGDL;
	}
	...
	{
		lpObj->LevelUpPoint += gLevelUpPointNormal;
	}
...
//	LogAddTD(Lang.GetText(0, 132), lpObj->AccountID, lpObj->Name, lpObj->Level);
...
BYTE gObjWarehouseInsertItem(LPOBJ lpObj, int type, int index, int level, int pos)
{
	...
	item.m_Level =level;
	item.Convert(item_type, 0 , 0, 0, 0, 0,0, CURRENT_DB_VERSION,0);
...
void gObjNextMasterExpCal(LPOBJ lpObj)
{
	lpObj->m_uMasterNextExp = gMasterLevelExperience[lpObj->m_uMasterLevel];
}
...
BOOL gObjAttackQ(LPOBJ lpObj)
{
	...
	if ( BC_MAP_RANGE(lpObj->MapNumber) )
	{
		if ( lpObj->Type >= OBJ_MONSTER )
		{
			...
			if ( lpObj->Class == 131 && g_BloodCastle.CheckMonsterKillSuccess(lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1) == false && lpObj->MapNumber != MAP_INDEX_BLOODCASTLE8)
			{
				return FALSE;
			}
			if ( lpObj->Class == 131 && g_BloodCastle.CheckMonsterKillSuccess(7) == false && lpObj->MapNumber == MAP_INDEX_BLOODCASTLE8)
			{
				return FALSE;
			}
			if ( (((lpObj->Class - 132) < 0)?FALSE:((lpObj->Class-132) > 2)?FALSE:TRUE) !=FALSE && g_BloodCastle.CheckBossKillSuccess(lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1) == false && lpObj->MapNumber != MAP_INDEX_BLOODCASTLE8 )
			{
				return FALSE;
			}
			if ( (((lpObj->Class - 132) < 0)?FALSE:((lpObj->Class-132) > 2)?FALSE:TRUE) !=FALSE && g_BloodCastle.CheckBossKillSuccess(7) == false  && lpObj->MapNumber == MAP_INDEX_BLOODCASTLE8)
			{
				return FALSE;
			}
		}
...
void gObjPlayerKiller(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ( lpObj->MapNumber == gPKFreeMap)
	{
		MsgOutput(lpObj->m_Index,"PK here is Free, enjoy");
		return;
	}
	...
	{
		if ( g_IllusionTemple.GetCurrentState(lpTargetObj->m_cIllusionTempleIndex) )
		{
			return;
		}
	}*/
...
void gObjUserDie(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	{
		if(g_IllusionTemple.GetHeroIndex(lpObj->MapNumber-MAP_INDEX_ILLUSION_TEMPLE1) == lpObj->m_Index)
		{
			g_IllusionTemple.DropSacredItemFromHero(lpObj->MapNumber-MAP_INDEX_ILLUSION_TEMPLE1);
			LogAddTD("[Illusion Temple] (%d) Hero is dead -> drop Sacred Item", lpObj->MapNumber-MAP_INDEX_ILLUSION_TEMPLE1+1);
		}
		g_IllusionTemple.SetKillPoint(lpObj->MapNumber-MAP_INDEX_ILLUSION_TEMPLE1, false, lpObj->m_Index, 10);
		return;		return;
	}
	...
	if(lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER ) 
	{
		...
		if ( PvPGens.GensCheck(lpObj, lpTargetObj) )
		{
			PvPGens.KillUserProc(lpObj, lpTargetObj);
		}
...
void gObjPkDownTimeCheck(LPOBJ lpObj, int TargetLevel)
{
	...
	if(lpObj->m_PK_Level < 3)
	{
		if(lpObj->m_PK_Time > 3600)
		{
			...
			if(lpObj->m_PK_Count > 10000)
			{
				lpObj->m_PK_Count = 10000;
			}
...
BOOL gObjDemonSprite(LPOBJ lpObj)
{
	...
	if ( Satan->m_Type == ITEMGET(13,64) && Satan->m_Durability > 0.0f )
	{
		return TRUE;
	}
...
BOOL gObjWingSprite(LPOBJ lpObj)
{
	...
	if ( (Wing->m_Type >= ITEMGET(12,0) && Wing->m_Type <= ITEMGET(12,6)) || Wing->m_Type == ITEMGET(13,30) || (Wing->m_Type >= ITEMGET(12,36) && Wing->m_Type <= ITEMGET(12,40)))
	{
...
void gObjSpriteDamage(LPOBJ lpObj, int damage)
{
	...
	{
		fdamage = (damage*4)/10.0f;
		fdamage /= fN;
		sprite->m_Durability -= fdamage;
		send_dur = 1;
	}
//	else if( sprite->m_Type == ITEMGET(5,21))
	...
	if ( sprite->m_Type == ITEMGET(13,65) )
	{
		fdamage = (damage*2)/10.0f;
		fdamage /= fN;
		sprite->m_Durability -= fdamage;
	...
	if ( sprite->m_Type == ITEMGET(13,67) )
	{
		fdamage = (damage*2)/10.0f;
		fdamage /= fN;
		sprite->m_Durability -= fdamage;
	...
	if ( sprite->m_Type == ITEMGET(13,80) )
	{
		fdamage = (damage*2)/10.0f;
		fdamage /= fN;
		sprite->m_Durability -= fdamage;
	...
	if ( sprite->m_Type == ITEMGET(13,123) )
	{
		fdamage = (damage*2)/10.0f;
		fdamage /= fN;
		sprite->m_Durability -= fdamage;
	...
	else if ( sprite->m_Type == ITEMGET(13,4) )
	{
		...
		if ( sprite->m_DurabilitySmall > 1500 )
		{
			...
			if ( sprite->m_Durability < 1.0f )
			{
				...
				LogAddTD("[%s][%s] ´??©ÈÙ????»? Item is Broken because durability is exhausted [%d]",
						lpObj->AccountID,lpObj->Name,lpObj->pInventory[8].m_Number);
			}
...
/*int gObjMonsterExpSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg, bool& bSendExp)
{
	...
	double mymaxexp;
	if(lpObj->Level == 400 && lpObj->ChangeUP > 1)
	{
		mymaxexp = gMasterLevelExperience[lpObj->m_uMasterLevel];
	}
	else
	{
	...
	exp *= gAddExperience + lpObj->m_iVipExpAdd;
	...
	{
		lpObj->m_uMasterExp+=exp;
		gSkillTree.gObjMasterLevelUp(lpObj,exp);
	}
...
int gObjMonsterExpSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg, bool& bSendExp)
{
	...
	if(MasterLevelSystem.CanGainMasterExp(lpObj) && lpTargetObj->Class < 95)
	{
//		return 0;
	}
	...
	if(BC_MAP_RANGE(lpObj->MapNumber))
	{
	//	exp *= g_BloodCastle.m_BridgeData[lpObj->m_cBloodCastleIndex].m_iCastleExperience;
	}
	if(CC_MAP_RANGE(lpObj->MapNumber))
	{
	...
	{
		exp *= gAddExperience;
		if(lpObj->MapNumber == MAP_INDEX_VULCANROOM && lpObj->m_GladiatorGloryBuffTime != 0)
		{
			exp += ((exp * 20)/100.0f);
		}
	...
	{
		exp *= gAddMasterExperience;
		if(lpObj->pInventory[8].m_Type == ITEMGET(13,123))
		{
			exp += ((exp * 30)/100.0f);
		}
		if(exp+lpObj->m_uMasterExp > lpObj->m_uMasterNextExp)
		{
			exp = lpObj->m_uMasterNextExp;
		}
	}

	if ( lpObj->m_wExprienceRate > 0 && !MasterLevelSystem.CanGainMasterExp(lpObj) )
	{
		exp =  (float)exp * ((float)lpObj->m_wExprienceRate  / 100.0f);
	}
	...
	if(exp > 0)
	{
		if(lpObj->Type == OBJ_USER)
		{
			...
			else if(MasterLevelSystem.CanGainMasterExp(lpObj))
			{
				lpObj->m_uMasterExp += exp;
			}
...
void gObjExpParty(LPOBJ lpObj , LPOBJ lpTargetObj, int AttackDamage, int MSBFlag )
{
	...
	if(lpTargetObj->Type == OBJ_MONSTER) //Maybe here party zen division start??
	{
		lpTargetObj->Money = totalexp*gAddExperience; // I wonder how to fix this O_o
	}
	...
	for(int n = 0; n < MAX_USER_IN_PARTY; n++)
	{
		...
		if(number >= 0)
		{
			...
			if(lpTargetObj->MapNumber == lpPartyObj->MapNumber && dis[n] < 10)
			{
				...
				if(MasterLevelSystem.CanGainMasterExp(lpPartyObj) && lpTargetObj->Class < 95)
				{
					continue;
				}
				...
				{
					//exp *= g_BloodCastle.m_BridgeData[lpPartyObj->m_cBloodCastleIndex].m_iCastleExperience;
				}
				...
				if(!MasterLevelSystem.CanGainMasterExp(lpObj))
				{
					...
					if(lpObj->MapNumber == MAP_INDEX_VULCANROOM /*&& lpObj->m_EffectInfo.GladiatorGloryTime != 0*/)
					{
						exp += ((exp * 20)/100.0f);
					}
				}
				...
				if(lpPartyObj->m_wExprienceRate > 0 && !MasterLevelSystem.CanGainMasterExp(lpPartyObj))
				{
				...
				if(exp > 0)
				{
					if(lpPartyObj->Type == OBJ_USER)
					{
						...
						else if(MasterLevelSystem.CanGainMasterExp(lpPartyObj))
						{
							lpPartyObj->m_uMasterExp += exp;
						}
...
void gObjLifeCheck(LPOBJ lpTargetObj, LPOBJ lpObj, int AttackDamage, int DamageSendType, int MSBFlag, int MSBDamage, BYTE Skill, int iShieldDamage)
{
	...
	if(lpTargetObj->Life <= 0 && lpTargetObj->Live != 0)
	{
		...
		if(IT_MAP_RANGE(lpTargetObj->MapNumber) && lpObj->m_cIllusionTempleIndex != -1)
		{
			if(lpTargetObj->Type == OBJ_USER && lpTargetObj->m_cIllusionTempleIndex == lpObj->m_cIllusionTempleIndex)
			{
				g_IllusionTemple.AddKillPoint(lpObj->m_cIllusionTempleIndex, lpObj->m_Index, 5);
			}
			else if(lpTargetObj->Type == OBJ_MONSTER)
			{
				g_IllusionTemple.AddKillPoint(lpObj->m_cIllusionTempleIndex, lpObj->m_Index, 2);
			}
		}
		...
		if(lpTargetObj->Type == OBJ_MONSTER)
		{
			...
			if( lpTargetObj->m_Attribute == 0x3E )
			{
				...
				if( iTargetClass != 295)
				{
					...
					else
					{
						...
						{
							wsprintf(szBuff,"%s has killed %s", lpTargetObj->Name, lpObj->Name);
						...
						{
							wsprintf(szBuff, "%s has killed Boss", lpTargetObj->Name);
				...
				else
				{
					if( gObjIsConnected(lpObj->m_Index) )
					{	
						wsprintf(szBuff, "%s has killed boss", lpObj->Name);
					...
					{
						wsprintf(szBuff, "Boss has been killed");
			...
			gCXMasEvent.MonsterDie(lpTargetObj->m_Index);
			CQuestSystem.QuestManage(lpObj,lpTargetObj);
			g_SQuestSys.MonsterMng(lpObj->m_Index,lpTargetObj->m_Index);
			...
			if(BC_MAP_RANGE(lpTargetObj->MapNumber))
			{
				...
				if(BC_STATUE_RANGE(lpTargetObj->Class - 132))
				{
					...
					if(TopHitUser != -1)
					{
						...
						{
							g_BloodCastle.SendNoticeMessage(7,szMsg);
							LogAddTD("[Blood Castle] (%d) a stone statue Of Saint Terminated -> %d [%s][%s]",8,g_BloodCastle.m_BridgeData[7].m_iBC_BOSS_MONSTER_KILL_COUNT,gObj[TopHitUser].AccountID,gObj[TopHitUser].Name);
						}
					}
					else
					{
				...
				// no non o u will make errors n o yes this must stay as it is ok :D
		...
		else if(lpTargetObj->Type == OBJ_USER)
		{
			...
			if(DG_MAP_RANGE(lpTargetObj->MapNumber))
			{
				...
				lpTargetObj->m_btDoubleGoerRoom = -1;
			...
			int iDuel = g_DuelManager.DuelCheck(lpObj, lpTargetObj);
			bool Gens = PvPGens.GensCheck(lpCallObj,lpTargetObj);
			bool Illusion = g_IllusionTemple.AttackCheck(lpCallObj, lpTargetObj);

			if(iGuildWar == 0 && iDuel == 0 && Gens == false && Illusion == false)
			{
		...
		{
			if(lpTargetObj->Type == OBJ_MONSTER && RAKLION_EGG_CLASS_RANGE(lpTargetObj->Class))
			{
				g_Raklion.DecEggCount();
				RAKLION_UTIL.NotifyRaklionUserEggCount(g_Raklion.GetCurrentEggCount());
			}
		}
		if(lpTargetObj->m_RecallMon >= 0)
		{
...
/*		if(lpObj->MapNumber == 6)
		{
			gObjTeleport(lpObj->m_Index,0,128,128);
			MsgOutput(lpObj->m_Index,"[Death] Died in Arena -> Regen lorencia");
		}..*/
	}
	else
	{
		...
		for(w = 0; w < 8; w++)
		{
			if(*(BYTE*)(gObj[aIndex].pInventoryMap+h*8+w) == 255)
			{
				...
				if(blank != 255)
				{
					...
					copyitem.Convert(item->m_Type,item->m_Option1,item->m_Option2,item->m_Option3,item->m_NewOption,item->m_SetOption,item->m_ItemOptionEx,CURRENT_DB_VERSION, &item->m_ItemSocket[0]);
...
BYTE gObjMonsterInventoryInsertItem(LPOBJ lpObj, int type, int index, int level, int op1, int op2, int op3)
{
	...
	item.Convert(item_type,op1,op2,op3,0,0,0,CURRENT_DB_VERSION,0);
...
BYTE gObjInventoryInsertItemPos(int aIndex, CItem item, int pos, BOOL RequestCheck)
{
	...
	if(pos < 12)
	{
		...
		{
			if(gObjResetItem[i].itemIndex == item.m_Type)
			{
				if(lpObj->m_iResets >= gObjResetItem[i].iResets)
				{
					hasresets = 1;
					break;
				}
				...
				{
					hasresets = -1;
					MsgOutput(aIndex,"You must have %d resets to wear this item",gObjResetItem[i].iResets);
					return -1;
				}
			}
		}
...
BYTE gObjInventoryMoveItem(int aIndex, unsigned char source, unsigned char target, int& durSsend, int& durTsend, unsigned char sFlag, unsigned char tFlag, unsigned char* siteminfo)
{
				...
				else
				{
					if(sitem->m_Level < 4 && sitem->m_Option3*4 < 4)
					{
						...
						else if(sitem->m_Type == ITEMGET(12,26))
						{
							...
							sitem->m_Type >= ITEMGET(12,60) && sitem->m_Type <= ITEMGET(12,129)
							|| gItemSocket.IsSeason4Item(sitem->m_Type) == 1)
						{
	...
	/*.	case 11:
		...
		if(titem->IsItem() == 1)
		{
			...
			if(sitem->m_Type == ITEMGET(14,110))
			{
				if(sitem->m_Level == titem->m_Level)
				{
					max_count = 5;
			...
			if(sitem->m_Type == ITEMGET(14,101))
			{
				if(sitem->m_Level == titem->m_Level)
				{
					max_count = 5;
		...
		case 4:
			if(target < INVETORY_WEAR_SIZE)
			{
				...
				for(int i=0;i<1000;i++)
				{
					if(gObjResetItem[i].itemIndex == sitem->m_Type)
					{
						if(lpObj->m_iResets >= gObjResetItem[i].iResets)
						{
							hasresets = 1;
				...
				if(sFlag == 0)
				{
					if(source < INVETORY_WEAR_SIZE)
					{
						if(source == 10 || source == 11)
						{
							...
							if(lpObj->pWarehouse[target].m_Type == ITEMGET(13,39))
							{
								gObjUseSkill.SkillChangeUse(aIndex);
							}
	...
	/*	case 11:
		case 12:
			gObjChaosBoxInsertItemPos(aIndex,lpObj->pChaosBox[source],target,source);
			gObjMakePreviewCharSet(aIndex);
			GCEquipmentChange(aIndex,source);
			gObjChaosItemSet(aIndex,target,1);
			break;*/
		...
		if(sFlag == 0 && target == 1)
		{
			...
			{
				if(
					lpObj->pInventory[1].m_Type == ITEMGET(5,21) ||
		...
		{
			if( (
				lpObj->pInventory[target].m_Type == ITEMGET(5,21) ||
		...
		if(sFlag == 0 && source < INVETORY_WEAR_SIZE)
		{
			if(lpObj->pInventory[source].IsItem() == 1)
			{
				...
				if(lpObj->pInventory[source].m_Type == ITEMGET(13,42) && lpObj->Authority == 0x20)
				{
					gObjUseSkill.SkillChangeUse(aIndex);
					LogAdd(lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[source].m_Level);
				}
...
void gObjMakePreviewCharSet(int aIndex)
{
	...
	lpObj->CharSet[CS_CLASS] |= CS_SET_CHANGEUP2(lpObj->ChangeUP2); // Decompilado GS-N 1.00.66
...
#define SLOTNEGATIVE -1
	...
	// Set Part onf Wings
	if ( lpObj->pInventory[7].m_Type != ITEMGET(0,0) ) // estranho... enfiar qualquer coisa alem de kriss na asa?
	{
	...
	//if ( 
	//	(lpObj->pInventory[7].m_Type >= ITEMGET(12,3) && lpObj->pInventory[7].m_Type <= ITEMGET(12,6)) ||
	//	(lpObj->pInventory[7].m_Type == ITEMGET(13,30)) ||
	//	(lpObj->pInventory[7].m_Type >= ITEMGET(12,36) && lpObj->pInventory[7].m_Type <= ITEMGET(12,43)) // novas
	//)
	//{
	//	index |= CS_SET_WING1(lpObj->pInventory[7].m_Type);
	//}
	...
	) // aqui logicamente ta certo, porem no ida ta mostrando como == o.0 entao ak ta errado+ olha usa o convert do deathway caso precise :]
	{
		...
		{
			lpObj->CharSet[9] |= 5; // se ligue isso eu decompilei do gs .66 ta 100% certo, só pode ser aqui o erro ^^, pq se mostra errado, é protocolo, e isso é a unica coisa q faz o protocolo de viewport dos items, ta dentro desse if o erro, ctz
		}
...
*/
	...
	//if ( sub_402752() == 1 ) // Ring Of Wizard
	//{ // GS-N 1.00.66
	//	if ( lpObj->pInventory[8].m_Type == ITEMGET(13,20) )
	//	{
	//		if( lpObj->pInventory[8].m_Level == 0x03 )
	//		{
	//			rindex = lpObj->pInventory[8];
	...
	// CharSet[0] = 1294
		lpObj->CharSet[5] |= 0xC;
		//      *(_BYTE *)(v14 + 1358) |= 4 * ((*(_WORD *)(*(_DWORD *)(v14 + 3708) + 1322) - 35) & 7);
		lpObj->CharSet[16] |= 4 * ((lpObj->pInventory[7].m_Type - 35) & 0x07 ); // Demais asas lv3
	...
	switch(lpObj->pInventory[7].m_Type)
	{
		case 0x1882:
			...
			//	lpObj->CharSet[5] += 0x04;
				lpObj->CharSet[17] |= 0x20;
			}
			break;
		case 0x1883:
			{
				lpObj->CharSet[17] |= 0x40;
			}
			break;
		case 0x1884:
			{
			//	lpObj->CharSet[5] +=0x0C;
				lpObj->CharSet[17] |= 0x60;
		...
		//		lpObj->CharSet[5] +=0x08;
				lpObj->CharSet[17] |= 0x80;
			}
			break;
		case 0x1886:
			{
			//	lpObj->CharSet[5] += 0x04;
				lpObj->CharSet[17] |= 0xA0;
			}
			break;
	}
...
//#error message("put else if gObjViewportPaint")
	...
	wsprintf(szTemp, "Mobs:%d/11500  Players : %d/%d  Player(%d):%d Vp:%d(%d/%d) : items:%d",
...
struct PMSG_CHARREGEN {
  ...
  /*<thisrel this+0x10>*/ /*|0x4|*/ double Exp;
			...
			if(lpObj->DieRegen == 2)
			{
				...
				if(BC_MAP_RANGE(lpObj->MapNumber) && lpObj->Type >= OBJ_MONSTER)
				{
					if(lpObj->Class == 131)
					{
						...
						{
							g_BloodCastle.m_BridgeData[7].m_iBC_BOSS_MONSTER_MAX_COUNT = g_BloodCastle.GetCurrentLiveUserCount(7) * 2;
							g_BloodCastle.m_BridgeData[7].m_iBC_BOSS_MONSTER_KILL_COUNT = 0;
	
							if(g_BloodCastle.m_BridgeData[7].m_iBC_BOSS_MONSTER_MAX_COUNT > 10)
							{
								g_BloodCastle.m_BridgeData[7].m_iBC_BOSS_MONSTER_MAX_COUNT = 10;
							}
						}
					...
					//	ItemSerialCreateSend(lpObj->m_Index,lpObj->MapNumber + 236,lpObj->X,lpObj->Y,type,level,0,0,0,0,TopHitUser,0,0);
						...
						{
							g_BloodCastle.m_BridgeData[7].m_btBC_QUEST_ITEM_NUMBER = level;
							ItemSerialCreateSend(lpObj->m_Index,lpObj->MapNumber,lpObj->X,lpObj->Y,type,level,0,0,0,0,TopHitUser,0,0);	
							LogAddTD("[Blood Castle] (%d) Ultimate Weapon Dropped -> (%s)",
								8, (level !=0)?((level==1)?"Sword":"Crossbow"):"Staff");
						}
					}
					...
					if(lpObj->Type == OBJ_MONSTER)
					{
						if(lpObj->Class != 131 && BC_STATUE_RANGE(lpObj->Class - 132)== 0)
						{
							...
							{
								g_BloodCastle.ChangeMonsterState(7,lpObj->m_Index);
							}
						}
					}
				}
					...
					/*	else if(lpObj->MapNumber == MAP_INDEX_RAKLION)
						{
						  ...
						  else if(lpObj->MapNumber == MAP_INDEX_VULCANROOM)
						  {
						   int mgt = 294;
						   short x,y,level;
						   BYTE map = lpObj->MapNumber,dir;
						  ...
						  else if(lpObj->MapNumber == MAP_INDEX_DUELROOM)
						  {
							  switch(lpObj->m_iDuelRoom)
							  {
							  case 0:
								  {
									  lpObj->MapNumber = MAP_INDEX_DUELROOM;
									  lpObj->X = 89+rand()%20;
									  lpObj->Y = 62+rand()%19;
								  }
								  break;
						...
						{
							int mgt;
							if(lpObj->m_cIllusionTempleTeam == 0)
							{
								mgt = 148+lpObj->m_cIllusionTempleIndex;
							}
							else
							{
								mgt = 154+lpObj->m_cIllusionTempleIndex;
							}
							short x,y,level;
							BYTE map = lpObj->MapNumber,dir;
							int result = gGateC.GetGate(mgt,(short &)x,(short &)y, (BYTE &)map,(BYTE &)dir, (short &)level);
							...
							if(lpObj->m_cIllusionTempleIndex != -1 && lpObj->m_cIllusionTempleTeam >= 0)
							{
								BuffEffectC.EnableBuff(lpObj->m_Index, 34, 15, 0, 0, 0);
							}
						}
						...
						if(g_DuelManager.IsOnDuel(lpObj->m_Index))
						{
							if(g_DuelManager.m_Rooms[lpObj->m_iDuelRoom].lpObj02->m_Index != lpObj->m_Index)
							{
								gObjClearViewport(g_DuelManager.m_Rooms[lpObj->m_iDuelRoom].lpObj02);
								gObjViewportListProtocolCreate(g_DuelManager.m_Rooms[lpObj->m_iDuelRoom].lpObj02);
							}
							else
							{
								gObjClearViewport(g_DuelManager.m_Rooms[lpObj->m_iDuelRoom].lpObj01);
								gObjViewportListProtocolCreate(g_DuelManager.m_Rooms[lpObj->m_iDuelRoom].lpObj01);
							}
						}
						GCMoneySend(lpObj->m_Index,lpObj->Money);
...
void gObjSecondProc()
{
	...
	for(n = 0; n < OBJMAX;n++)
	{
		...
		if(lpObj->Connected >= PLAYER_LOGGED &&
			lpObj->Type == OBJ_USER)
		{
			if(GetTickCount() - lpObj->m_ServerTimeTick > 300000)
			{
				...
				LogAddTD("[Time Sync] (%s) Sync Server Time", lpObj->AccountID);

				lpObj->m_ServerTimeTick = GetTickCount();
			}
		}
...
void gObjViewportListProtocolCreate(LPOBJ lpObj)
{
	...
	if(lpObj->Type == OBJ_USER)
	{
		if(lpObj->m_Change >= 0)
		{
			...
			memcpy(&pViewportCreateChange.CharSet[0], &lpObj->CharSet[0], 18); 
		...
		if(lpObj->GensNumber != 0)
		{
			...
			PMSG_VIEWPORT_GENS pGens = {0};

			pGens.GensID = lpObj->GensNumber;
			pGens.Level = lpObj->GensPosition;
			pGens.Ranking = 0;
			pGens.IndexH = SET_NUMBERH(lpObj->m_Index);
			pGens.IndexL = SET_NUMBERL(lpObj->m_Index);

			memcpy(&GensInfoBuf[GensInfoOfs],&pGens,sizeof(pGens));

			GensInfoOfs += sizeof(pGens);

			PWMSG_VIEWPORT_GENS_COUNT pwCount;

			pwCount.h.c = 0xC2;
			pwCount.h.headcode = 0xF8;
			pwCount.h.subcode = 0x05;
			pwCount.h.sizeH = SET_NUMBERH(GensInfoOfs);
			pwCount.h.sizeL = SET_NUMBERL(GensInfoOfs);
			pwCount.count = 1;

			memcpy(GensInfoBuf,&pwCount,sizeof(pwCount));

			DataSend(lpObj->m_Index,GensInfoBuf,GensInfoOfs);
		}
...
void gObjViewportListProtocol(short aIndex)
{
	...
	BYTE btViewSkillBuffer[MAX_VIEWSKILL] = {0}; // teste
					...
					if(tObjNum >= 0)
					{
						...
						case 1:
							...
							if(lpTargetObj->m_Change >= 0)
							{
								...
								// aqui ele poe as modificações coloca isso lah em baixo
...
//								pViewportCreate.ViewSkillState = 0;
							...
							if(lpObj->Type == OBJ_USER)
							{
								...
								if(lpTargetObj->GensNumber != 0)
								{
									PMSG_VIEWPORT_GENS pGens = {0};

									pGens.GensID = lpTargetObj->GensNumber;
									pGens.Level = lpTargetObj->GensPosition;
									pGens.Ranking = 0;
									pGens.IndexH = SET_NUMBERH(lpTargetObj->m_Index);
									pGens.IndexL = SET_NUMBERL(lpTargetObj->m_Index);
		...
		if(lpObj->Type == OBJ_USER)
		{
			...
			if(GensInfoCount != 0)
			{
				PWMSG_VIEWPORT_GENS_COUNT pCount;
				pCount.h.headcode = 0xF8;
				pCount.h.subcode = 0x05;
				pCount.h.c = 0xC2;
				pCount.h.sizeH = SET_NUMBERH(GensInfoOfs);
				pCount.h.sizeL = SET_NUMBERL(GensInfoOfs);
				pCount.count = GensInfoCount;

				memcpy(GensInfoBuf,&pCount,sizeof(pCount));
				DataSend(aIndex,(unsigned char *)&GensInfoBuf,GensInfoOfs);
			}
		}
	}
}
...
void gObjSkillUseProc(LPOBJ lpObj) // FUNCTION DEPRECATED
{
	if(lpObj->m_SkillAttackTime > 0)
	{
		...
		if(lpObj->m_SkillAttackTime == 0)
		{
			BuffEffectC.DelEffect(lpObj, 1);

			GCMagicCancelSend(lpObj,28);
		}
	}

	if(lpObj->m_SkillFlameStrikeTime > 0)
	{
		lpObj->m_SkillFlameStrikeTime--;

		if(lpObj->m_SkillFlameStrikeTime == 0)
		{
			BuffEffectC.DelEffect(lpObj, 83);
		}
	}

	if(lpObj->m_SkillStormTime > 0)
	{
		lpObj->m_SkillStormTime--;

		if(lpObj->m_SkillStormTime == 0)
		{
			BuffEffectC.DelEffect(lpObj, 84);
		}
	}
	...
	if(lpObj->m_SkillHardenTime > 0)
	{
		...
		{
			lpObj->m_SkillHarden = 0;
			BuffEffectC.DelEffect(lpObj, 57);
	...
	if(lpObj->m_SkillMagumReduceDefenseTime > 0)
	{
		...
		{
			//lpObj->m_SkillMagumReduceDefense = 0;
			BuffEffectC.DelEffect(lpObj, 58);
	...
	if(lpObj->Class == 77)
	{
		...
		if(lpObj->m_SkyBossMonSheildTime > 5)
		{
			...
			{
				lpObj->m_SkyBossMonSheild = 0;
				BuffEffectC.DelEffect(lpObj, 64);
			...
			else
			{
				...
				BuffEffectC.EnableBuff(lpObj->m_Index, 64, -1, 0, 0, 0);
	...
	if(lpObj->SkillRecallParty_Time != 0 && lpObj->Type == OBJ_USER)
	{
		...
		if(lpObj->SkillRecallParty_Time <= 0)
		{
			...
			{
				char szMsg[256];
				wsprintf(szMsg,"Summoning Canceled");
	...
	if(lpObj->SkillAddCriticalDamageTime != 0 && lpObj->Type == OBJ_USER)
	{
		...
		if(lpObj->SkillAddCriticalDamageTime <= 0)
		{
			...
			MsgOutput(lpObj->m_Index,"The critical damage spell has diminshed");

			BuffEffectC.DelEffect(lpObj, 61);
		}
	}
	...
	if(lpObj->m_iSkillStunTime != 0 && lpObj->Type == OBJ_USER)
	{
		...
		{
			lpObj->m_iSkillStunTime = 0;
			MsgOutput(lpObj->m_Index,"Stun effect has been diminshed");
			BuffEffectC.DelEffect(lpObj, 61);
		}
	}
	if(lpObj->m_iIllusionFreezeTime != 0 && lpObj->Type == OBJ_USER)
	{
		lpObj->m_iIllusionFreezeTime--;
		if(lpObj->m_iIllusionFreezeTime <= 0)
		{
			lpObj->m_iIllusionFreezeTime = 0;
			BuffEffectC.DelEffect(lpObj, 34);
	...
	if(lpObj->m_iSkillBrandOfSkillTime != 0 && lpObj->Type == OBJ_USER)
	{
		...
		{
			lpObj->m_iSkillBrandOfSkillTime = 0;
			MsgOutput(lpObj->m_Index,"The Brand of Skill effect has been diminshed");

			BuffEffectC.DelEffect(lpObj, 19);
		}
	}
	...
	if(lpObj->m_iSkillInvisibleTime != 0 && lpObj->Type == OBJ_USER)
	{
		...
		{
			lpObj->m_iSkillInvisibleTime = 0;
			MsgOutput(lpObj->m_Index,"The Invisible effect has been diminshed");

			BuffEffectC.DelEffect(lpObj, 18);
		}
	}
	...
	if(lpObj->m_iSkillManaSwellTime != 0 && lpObj->Type == OBJ_USER)
	{
		...
		if(lpObj->m_iSkillManaSwellTime <= 0)
		{
			...
			lpObj->m_iSkillManaSwellTime = 0;
			lpObj->m_iSkillManaSwell = 0;
			MsgOutput(lpObj->m_Index,"Mana swell effect has been diminshed");

			BuffEffectC.DelEffect(lpObj, 9);
		}
	}
	...
	if(lpObj->m_iPotionBlessTime != 0 && lpObj->Type == OBJ_USER)
	{
		...
		{
			lpObj->m_iPotionBlessTime = 0;
			MsgOutput(lpObj->m_Index,"Potion of Bless effect has been diminshed");

			BuffEffectC.DelEffect(lpObj, 11);
		}
	}
	...
	if(lpObj->m_iPotionSoulTime != 0 && lpObj->Type == OBJ_USER)
	{
		...
		{
			lpObj->m_iPotionSoulTime = 0;
			MsgOutput(lpObj->m_Index,"Potion of soul effect has been diminshed");
	...
	if(lpObj->m_iMuseElfInfinityArrowSkillTime != 0 && lpObj->Type == OBJ_USER)
	{
		...
		{
			lpObj->m_iMuseElfInfinityArrowSkillTime = 0;
			BuffEffectC.DelEffect(lpObj, 6);
		}
	}
	if(lpObj->m_iSkillParalyzeTime != 0)
	{
		lpObj->m_iSkillParalyzeTime--;
		if(lpObj->m_iSkillParalyzeTime <= 0)
		{
			lpObj->m_iSkillParalyzeTime = 0;
			lpObj->lpAttackObj = 0;
			BuffEffectC.DelEffect(lpObj, 86);
		}
	}
	if(lpObj->m_iSkillRageulTime != 0)
	{
		lpObj->m_iSkillRageulTime--;
		if(lpObj->m_iSkillRageulTime <= 0)
		{
			lpObj->m_iSkillRageulTime = 0;
			lpObj->lpAttackObj = 0;
			BuffEffectC.DelEffect(lpObj, 56);
		}
	}
	if(lpObj->m_iSkillShockTime != 0)
	{
		lpObj->m_iSkillShockTime--;
		if(lpObj->m_iSkillShockTime <= 0)
		{
			lpObj->m_iSkillShockTime = 0;
			BuffEffectC.DelEffect(lpObj, 85);
		}
	}
			
	if(lpObj->m_iSkillSleepTime != 0)
	{
		lpObj->m_iSkillSleepTime--;
		if(lpObj->m_iSkillSleepTime <= 0)
		{
			lpObj->m_iSkillSleepTime = 0;
			BuffEffectC.DelEffect(lpObj, 72);
		}
	}
	if(lpObj->m_iSkillDamageDecTime != 0)
	{
		lpObj->m_iSkillDamageDecTime--;
		if(lpObj->m_iSkillDamageDecTime <= 0)
		{
			lpObj->m_iSkillDamageDecTime = 0;
			lpObj->m_iSkillDamageDecValue = 0;
			BuffEffectC.DelEffect(lpObj, 76);
		}
	}
	if(lpObj->m_iSkillDefenseDecTime != 0)
	{
		lpObj->m_iSkillDefenseDecTime--;
		if(lpObj->m_iSkillDefenseDecTime <= 0)
		{
			lpObj->m_iSkillDefenseDecTime = 0;
			lpObj->m_iSkillDefenseDecValue = 0;
			BuffEffectC.DelEffect(lpObj, 77);
		}
	}
	if(lpObj->m_iIllusionShieldTime != 0)
	{
		lpObj->m_iIllusionShieldTime--;
		if(lpObj->m_iIllusionShieldTime <= 0)
		{
			lpObj->m_iIllusionShieldTime = 0;
			BuffEffectC.DelEffect(lpObj, 34);
	...
	if(lpObj->m_PoisonBeattackCount > 0)
	{
		...
		if(lpObj->m_PoisonBeattackCount % 3 == 0)
		{
			...
			if(lpObj->m_PoisonBeattackCount <= 0)
			{
				...
				lpObj->m_ViewSkillState[eVS_POISON] = 0;
				lpObj->m_ViewState = 0;
//..				GCStateInfoSend(lpObj,0,eVS_POISON,0,0);
				BuffEffectC.DelEffect(lpObj,eVS_POISON);
				if(lpObj->m_PoisonType == 38)
				{
					BuffEffectC.DelEffect(lpObj,eVS_POISON);
				}
	...
	else if(lpObj->m_iSkillSahamuttTime > 0)
	{

		lpObj->m_iSkillSahamuttTime--;

		if(lpObj->m_iSkillSahamuttTime % 3 == 0)
		{
			if(lpObj->Life > 0.0f)
			{
				if(lpObj->m_PoisonType == 38)
			...
			{
				lpObj->lpAttackObj = 0;
				lpObj->m_ViewSkillState[75] = 0;
				lpObj->m_ViewState = 0;
				BuffEffectC.DelEffect(lpObj,75);
	...
	else if(lpObj->m_ColdBeattackCount > 0)
	{
		...
		if(lpObj->m_ColdBeattackCount <= 0)
		{
			...
			lpObj->m_ViewSkillState[eVS_ICE] = 0;
			lpObj->m_ViewState = 0;
			BuffEffectC.DelEffect(lpObj,eVS_ICE);
	...
	else if(lpObj->m_SummonerSkillSleepTime > 0)
	{
		lpObj->m_SummonerSkillSleepTime--;

		if(lpObj->m_SummonerSkillSleepTime <= 0)
		{
			lpObj->m_ViewSkillState[eVS_SKILL_SLEEP] = 0;
			lpObj->m_ViewState = 0;

			BuffEffectC.DelEffect(lpObj,eVS_SKILL_SLEEP);
	...
	else if(lpObj->m_ImmuneToMagicCount > 0)
	{
		...
		if(lpObj->m_ImmuneToMagicCount <= 0)
		{
			...
			BuffEffectC.DelEffect(lpObj,eVS_INMUNE_TO_MAGIC);
		}
	}
	else if(lpObj->m_ImmuneToHarmCount > 0)
	{
		...
		if(lpObj->m_ImmuneToHarmCount <= 0)
		{
			...
			BuffEffectC.DelEffect(lpObj,eVS_DEFENSE_AURA);
		}
	}
}
...
BOOL gObjMoveGate(int aIndex, int gt)
{
		...
		|| DS_MAP_RANGE(mapNumber))
	{
		if(lpObj->m_iSkillNPCHelpTime > 0)
		{
			...
			BuffEffectC.DelEffect(lpObj,eVS_NPC_HELP);
		}
	}
	...
	{
		if(g_Raklion.GetRaklionState() >= 7)
		{
			GCServerMsgStringSend("Sorry event is not open!", lpObj->m_Index, 1);
	...
	{
		if(!g_Imperial.UsePortalGate(lpObj, gt))
		{
	...
	{
		if(gt == 282 && g_Raklion.GetRaklionState() >= 7)
		{
			GCServerMsgStringSend("Sorry event is not started!", lpObj->m_Index, 1);
	...
	{
		RAKLION_UTIL.NotifyRaklionCurrentState(aIndex, g_Raklion.GetRaklionState(), g_Raklion.GetRaklionDetailState());
	}
	...
	if(mapNumber == MAP_INDEX_ICARUS)
	{
		if(!(lpObj->pInventory[8].m_Type == ITEMGET(13,3)
			...
			|| (lpObj->pInventory[7].m_Type >= ITEMGET(12,0)
				&& lpObj->pInventory[7].m_Type <=ITEMGET(12,134)))
	...
	/*
	if(IT_MAP_RANGE(mapNumber) && !IT_MAP_RANGE(lpObj->MapNumber))
	{
		int iIllusionTempleIndex = lpObj->m_cIllusionTempleIndex;
		
		if( iIllusionTempleIndex != -1 )
		{
			g_IllusionTemple.SearchUserDropTorchItem(lpObj->m_Index);
			lpObj->m_Change = -1;
		}
		else
		{
			g_IllusionTemple.SearchUserDeleteTorchItem(lpObj->m_Index);
		}
	}
	*/
	...
	if(g_BlockRaklionMapEnter == 1)
	{
		...
		if(gGateC.GetGate(gt,(short &)x,(short &)y,(BYTE &)mapNumber,(BYTE &)dir,(short &)level) != -1)
		{
			if(mapNumber == MAP_INDEX_RAKLION && mapNumber == MAP_INDEX_RAKLIONBOSS)
			{
				GCServerMsgStringSend(lMsg.Get(1604),lpObj->m_Index,1);
				x = lpObj->X;
				y = lpObj->Y;
	...
	if(movefail == 0)
	{
		...
		char ITFlag = false;
		if(IT_MAP_RANGE(gObj[aIndex].MapNumber) && gObj[aIndex].m_cIllusionTempleIndex >= 0)
		{
			if(!IT_MAP_RANGE(mapNumber))
			{
				...
				if(g_IllusionTemple.GetHeroIndex(gObj[aIndex].m_cIllusionTempleIndex) == aIndex)
				{
					g_IllusionTemple.DropSacredItemFromHero(gObj[aIndex].m_cIllusionTempleIndex);
				}
			}
		}
		...
		if(lpObj->m_Change >= 0 || ITFlag == true)
		{
			gObjViewportListProtocolCreate(&gObj[aIndex]);
		}
...
void gObjTeleport(int aIndex, int map, int x, int y)
{
	...
	gObj[aIndex].TX = x;
	gObj[aIndex].TY = y;	//  #error Change to TY
...
int skill = -1,reqeng,reqleadership;
	...
	if(g_EnergyCheckOff == 1)
	{
		...
		if(reqeng < 0)
		{
			LogAdd("required energy < 0");
			return -1;
		}

		if((lpObj->Energy + lpObj->AddEnergy) < reqeng)
		{
			LogAdd("short of energy");
			return -1;
		}
	}
	...
	if(MagicDamageC.SkillGetRequireClass(lpObj->Class,lpObj->ChangeUP,skill) < 1 && !MasterLevelSystem.IsMasterSkill(Type))
	{
		LogAdd("Its not his class");
		return -1;
	}
	...
	for(int n = 0; n < MAGIC_SIZE; n++)
	{
		if(lpObj->Magic[n].IsMagic() == 1)
		{
			...
			if(skill < 0)
			{
				LogAdd("null skills %s%s",__FILE__,__LINE__);
				return -1;
			}

			if(lpObj->Magic[n].m_Skill == skill)
			{
				LogAdd("skill already exists");
				return -1;
			}
		}
	}
	...
	LogAdd("gObjMagicAdd %s %s error" ,__FILE__,__LINE__);
	return -1;
}
...
int  gObjMagicAdd(LPOBJ lpObj, WORD aSkill, BYTE Level)
{
	...
	if(MagicDamageC.SkillGetRequireClass(lpObj->Class,lpObj->ChangeUP,aSkill) < 1 && !MasterLevelSystem.IsMasterSkill(aSkill))
	{
		//LogAdd(Lang.GetText(0, 162));
		return -1;
	}
	...
	for(int n = 0; n < MAGIC_SIZE; n++)
	{
		if(lpObj->Magic[n].IsMagic() == 1)
		{
			if(lpObj->Magic[n].m_Skill == aSkill)
			{
			//	LogAdd(Lang.GetText(0, 167), lpObj->Magic[n].m_Skill,aSkill);
				return -1;
			}
		}
	}
	...
	if(aSkill == 62)
	{
		for(int n = 0; n < MAGIC_SIZE; n++)
		{
			if(lpObj->Magic[n].IsMagic() == 1)
			{
				if(lpObj->Magic[n].m_Skill >= 515 && lpObj->Magic[n].m_Skill <= 519)
				{
				//	LogAdd(Lang.GetText(0, 167), lpObj->Magic[n].m_Skill,aSkill);
					return -1;
				}
	...
	for(int n = 0; n < MAGIC_SIZE; n++)
	{
		if(lpObj->Magic[n].IsMagic() == 0)
		{
			...
			if(skill < 0)
			{
				LogAdd("error %s %s", __FILE__, __LINE__);
				//..LogAdd(Lang.GetText(0, 165),__FILE__,__LINE__);
				return -1;
			}
			else
			{
	...
	LogAdd("gObjMagicAdd %s %s",__FILE__,__LINE__);
	return -1;
}
...
BOOL gObjItemLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	lpObj->pInventory[target].Convert(
			...
			CURRENT_DB_VERSION,
			 &lpObj->pInventory[target].m_ItemSocket[0]);
...
BOOL gObjItemLuckUp(LPOBJ lpObj, int source, int target)
{
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
...
//	int _r = rand()%100;
	...
	LogAddTD("[%s][%s] Use jewel of Luck",lpObj->AccountID,lpObj->Name);
	return true;
}
...
BOOL gObjCheckMaxZen(int aIndex, int nAddZen)
{
	...
	if ( _Money > (__int64)MAX_ZEN || _Money < 0 )
	{
...
void gObjUseBlessAndSoulPotion(int aIndex, int iItemLevel)
{
	...
	{
		BuffEffectC.EnableBuff(lpObj->m_Index, 10, 120, 0, 0, 0);
	}
	else if ( iItemLevel == 1 ) // Soul Potion
	{
		BuffEffectC.EnableBuff(lpObj->m_Index, 11, 60, 0, 0, 0);
...
bool gObjIsMasteringLevel(LPOBJ lpObj)
{
	...
	if(lpObj->Type == OBJ_USER)
	{
		...
		case CLASS_ELF:
		case CLASS_SUMMONER:
			return (bool)(lpObj->ChangeUP == 2 ? true : false);
		case CLASS_DARKLORD:
		case CLASS_MAGUMSA:
			return (bool)(lpObj->ChangeUP == 1 ? true : false);
		default:
			return false;
		}
	...
	{
		LogAddC(2, "gObjIsMasteringLevel Error: Object isn't a player!");
		return false;
	}

}

bool gObjIsMasteringLevel(int aIndex)
{

	if(OBJMAX_RANGE(aIndex))
	{
		return gObjIsMasteringLevel(&gObj[aIndex]);
	}
	else
	{
		LogAddC(2, "[ERROR] gObjIsMasteringLevel Error: index out of range!");
		return false;
	}
...
void gObjReady4Relife(LPOBJ lpObj)
{
	...
	GCMoneySend(lpObj->Money,lpObj->Money);
	//GCStateInfoSend(lpObj,1,43);
	if(lpObj->MapNumber == 6)
	{
		gObjTeleport(lpObj->m_Index,0,128,128);
		MsgOutput(lpObj->m_Index,"[Death] Died in Arena -> Regen Lorencia");
...
void gObjZenFix(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	
	if(lpObj->Money < -1 || lpObj->Money > MAX_ZEN)
	{
		lpObj->Money = MAX_ZEN;
		GCMoneySend(aIndex,lpObj->Money);
		MsgOutput(aIndex,"[ZENFIX] Please becareful, ZEN Fixed");
		return;
	}
	else
		return;
...
bool gObjCheckMagic(LPOBJ lpObj, WORD aSkill, BYTE Level)
{
