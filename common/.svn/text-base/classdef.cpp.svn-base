...
void classdef::Init()
{
	...
	// Summoner
    this->SetCharacter( CLASS_SUMMONER, 21, 21, 18, 23, 70.0f , 40.0f, 1.0f, 1.5f, 2.0f, 1.7f, 0 );

	// Rage Figher
	this->SetCharacter( CLASS_RAGEFIGHTER, 30, 30, 25, 20, 90, 60, 2.0f, 1.5f, 1.5f, 2.0f, 0);
...
void classdef::SetEquipment(int Class)
{
	...
	int right_type=0;
	int n;
	BYTE SOCKETOPT[5] = {0xFF,0xFF,0xFF,0xFF,0xFF};
	...
	{
		if ( Class == CLASS_ELF || Class == CLASS_MAGUMSA || Class == CLASS_SUMMONER || Class == CLASS_DARKLORD)
		{
			this->DefClass[Class].Equipment[0].m_Level = 0;
			this->DefClass[Class].Equipment[1].m_Level = 0;
			this->DefClass[Class].Equipment[0].Convert(right_type, 0,0,0,0,0,0,CURRENT_DB_VERSION,&SOCKETOPT[0]);
			this->DefClass[Class].Equipment[1].Convert(def_type, 0,0,0,0,0,0,CURRENT_DB_VERSION,&SOCKETOPT[0]);
		}
		else
		{
			this->DefClass[Class].Equipment[0].m_Level = 0;
			this->DefClass[Class].Equipment[0].Convert(def_type, 0,0,0,0,0,0,CURRENT_DB_VERSION,&SOCKETOPT[0]);
		}
	}
	...
	if ( this->m_BabubBanJiCreate == TRUE )
	{
		...
		this->DefClass[Class].Equipment[12].Convert(def_type, 0,0,0,0,0,0,CURRENT_DB_VERSION,&SOCKETOPT[0]);
		
		def_type = ItemGetNumberMake(13, 20);
		this->DefClass[Class].Equipment[13].m_Level = 2;
		this->DefClass[Class].Equipment[13].Convert(def_type, 0,0,0,0,0,0,CURRENT_DB_VERSION,&SOCKETOPT[0]);
	}
