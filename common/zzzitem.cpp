...
#include "..\SOURCE\SocketItemSystem.h"
...
void CItem::StoreSockets(BYTE* Socket)
{
	for(int i=0;i<5;i++)
	{
		if(Socket[i] < 0xFF)
		{
			this->m_ItemSocket[i] = Socket[i];
			LogAddTD("Socket DB Save %d",Socket[i]);
		}
	}
	
}
void CItem::Convert(int type, BYTE Option1, BYTE Option2, BYTE Option3,  BYTE Attribute2, BYTE SetOption, BYTE ItemEffectEx,  BYTE DbVersion, BYTE* SOCKETOPT)
{
	int _type;
	int ItemLevel;
	LPITEM_ATTRIBUTE p;
	...
	if ( p->RequireLevel != 0 )
	{
		...
		else if ( _type >= ITEMGET(12,36) && _type <= ITEMGET(12,43) )
		{
			this->m_RequireLevel = p->RequireLevel;
		}
	...
	if ( _type == ITEMGET(13,30) || _type == ITEMGET(12,40) )
	{
	...
	if ( Option3 != 0 )
	{
		...
		if ( _type == ITEMGET(13,30) || _type == ITEMGET(12,40) ) // Cape of lord 
		{
			this->m_Special[this->m_SpecialNum] = 80;
			this->m_Option3 = Option3;
			this->m_RequireStrength +=  Option3 * 4;
		...
		if ( _type == ITEMGET(12,0)  || _type == ITEMGET(12,42)  )	// Wing elf
		{
			this->m_Special[this->m_SpecialNum] = 85;
			this->m_Option3 = Option3;
		}
		...
		else if( _type == ITEMGET(12,38) )
		{
			this->m_Option3 = Option3 * 4;
			this->m_RequireStrength += Option3 * 4;
		...
		if ( _type == ITEMGET(13,30)  || _type == ITEMGET(12,40) ) // Cape ofLord
		{
			this->m_Option3 = Option3;
			this->m_RequireStrength +=  Option3 * 4;
	...
	if( gItemSocket.IsSeason4Item(this->m_Type))
	{
		for(int i = 0;i<5;i++)
		{
			if(SOCKETOPT[i] < 0xFF)
			{
				this->m_ItemSocket[i] = SOCKETOPT[i];
			}
		}
	}
	//this->StoreSockets(&this->m_ItemSocket[0]);
	//this->m_ItemSocket[0] = 0xFE;
}
...
void CItem::Value()
{
	...
	else if( this->m_Type >= ITEMGET(13,71) && this->m_Type <= ITEMGET(13,75)) // moss merchabnt
	{
		Gold = 1000000;
	}
	...
	else if ( this->m_Type == ITEMGET(14,110) ) // Sign of doblegoer
	{
		Gold = ((10000.0f) * this->m_Durability) * 3.0f;
	}
	...
	else
	{
		switch ( this->m_Level )
		{
			...
			case 14:	Level2 += 315;  break;
			case 15:	Level2 += 395;  break;
		}
...
{ // GS-N 1.00.66   0x00526020
	if ((aClass < 0) || (aClass >= MAX_TYPE_PLAYER ))
	{
		LogAdd("Å¬·¡½º ¿¡·¯ : %d (%s %d)", aClass, __FILE__, __LINE__); // Å¬·¡½º ¿¡·¯ : %d (%s %d)
		return 0;
	}
...
void CItem::SetPetItemInfo(int petlevel, int petexp)
{
	if ( this->m_Type == ITEMGET(13,4) )
	{
		...
		this->Convert(this->m_Type, this->m_Option1, this->m_Option2,
			this->m_Option3, this->m_NewOption, this->m_SetOption, this->m_ItemOptionEx, CURRENT_DB_VERSION, &this->m_ItemSocket[0]);
		this->PetValue();
	...
	if ( this->m_Durability == 0.0f )
	{
		if ( this->m_CurrentDurabilityState != 1.0f )
		{
			...
			this->Convert(this->m_Type, this->m_Option1, this->m_Option2, this->m_Option3, this->m_NewOption,
				this->m_SetOption, this->m_ItemOptionEx, CURRENT_DB_VERSION,  &this->m_ItemSocket[0]);
			return true;
		}
	}
	else if ( this->m_Durability < this->m_DurabilityState[2] )
...
// ak tinha  n+=5 se nao me engano n eh isso q to vendo n perai, aqui ta 100%, o problema eh o charset msm
	...
	//só tem 10 bytes aqui... ta faltando coisa aqui n? ou esse negocio do socket ta errado? essa parte sempre foi assim so fiz add as info do socket aqui n tem isso n... eu acho do socket
}
...
{
	//o call tem aquele arg no fim msm? q arg?
	memset(buf, 0, MAX_ITEM_INFO);
	int n = 0;
	...
	//eh aqui msm q ta corrompendo a stack, ta escrevendo no byte 7 qnd só vai até 6 haha
}
...
void ItemByteConvert16(LPBYTE buf, CItem * item , int maxitem)
{
	...
	for (int index=0; index < maxitem ;index++)
	{
		...
		else
		{
			...
			buf[n++] = item[index].m_Durability; //2
...
int ItemGetDurability(int index, int itemLevel, int ExcellentItem, int SetItem)
{
	...
	else if ( itemLevel >= 5 )
	{
		...
		{
			dur=ItemAttribute[index].Durability + itemLevel*2+9;
		}
		else if (itemLevel == 15 )
		{
			dur=ItemAttribute[index].Durability + itemLevel*2+12;
		}
		else
		{
	...
	if ( (index < ITEMGET(12,3) || index > ITEMGET(12,6) ) &&
		  ...
		  index != ITEMGET(2,13) &&
		  index != ITEMGET(13,30) &&
		  index < ITEMGET(12,36) || index > ITEMGET(12,43))
	{
		if ( SetItem != 0 )
			dur +=20;
...
{
	if(this->m_Type < ITEMGET(12,36) || this->m_Type > ITEMGET(12,43))
		return FALSE;
	 ...
	 while(true)
		 {
			 ...
			 if(Token == NUMBER)
			 {
				 ...
				 while(true)
				 {
					...
					// store data
					gObjResetItem[aIndex].itemIndex = ITEMGET(iCat,iIndex);
					gObjResetItem[aIndex].iResets = Resets;
					aIndex++;
				 }
			 }
		 }
		 ...
		 LogAddTD("%s file load succesfuly");
}